...
running under the dispatcher (env var ``CORTEX_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``athena chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``athena kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``athena kanban complete …``
   inside the container, where ``athena`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.cortex/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``athena kanban …``), the dashboard
(``athena dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``CORTEX_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``athena chat`` without the kanban toolset see zero
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("CORTEX_KANBAN_TASK")
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``CORTEX_KANBAN_TASK`` set
    ...
    ``CORTEX_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``HERMES_KANBAN_DB`` → ``CORTEX_KANBAN_BOARD`` env → current symlink
    → ``default``). Per-tool ``board`` lets a Telegram-side agent override
    the env-pinned active board without restarting Athena.
    """
...
#   - No-op outside dispatcher-spawned worker context (no ``CORTEX_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``CORTEX_KANBAN_TASK`` — task id (required; absence means no-op)
    ...
    tid = os.environ.get("CORTEX_KANBAN_TASK")
    if not tid:
        return False
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set CORTEX_KANBAN_TASK in the env)"
        )
...
def _handle_create(args: dict, **kw) -> str:
    ...
    # Otherwise, a dispatcher-spawned worker (CORTEX_KANBAN_TASK set)
    ...
    # (kanban toolset, no CORTEX_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                _self_tid = os.environ.get("CORTEX_KANBAN_TASK")
                if _self_tid:
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to CORTEX_KANBAN_TASK from the env "
...
_DESC_BOARD = (
    ...
    "CORTEX_KANBAN_BOARD env → the 'current' symlink under the kanban "

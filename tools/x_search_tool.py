...
* ``XAI_API_KEY`` is set in ``~/.cortex/.env`` or the process environment
  (paid xAI API key), OR
* The user is signed in via xAI Grok OAuth — SuperGrok subscription —
  i.e. ``athena auth add xai-oauth`` has been run and the stored refresh
  token still works.
...
def check_x_search_requirements() -> bool:
    ...
    :func:`cortex_cli.auth.resolve_xai_oauth_runtime_credentials` which

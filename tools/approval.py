...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("CORTEX_YOLO_MODE", ""))
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from cortex_cli.plugins import invoke_hook
    except Exception:
...
# via shell expansions like $HOME or $CORTEX_HOME, or by the resolved absolute
# active profile home path such as /home/hermes/.hermes/config.yaml. The
# resolved-absolute form is folded into the ~/.cortex/ patterns at detection
...
# go stale when CORTEX_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_HERMES_ENV_PATH = (
    r'(?:~\/\.hermes/|'
    r'(?:\$home|\$\{home\})/\.hermes/|'
    r'(?:\$cortex_home|\$\{cortex_home\})/)'
    r'\.env\b'
)
# ~/.cortex/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _HERMES_ENV_PATH; matches the CORTEX_HOME override form as
# well as ~/.cortex/.
_HERMES_CONFIG_PATH = (
    r'(?:~\/\.hermes/|'
    r'(?:\$home|\$\{home\})/\.hermes/|'
...
DANGEROUS_PATTERNS = [
    ...
    # that should always require user consent, just like `athena gateway restart`
    ...
    # In-place edit of a Athena-managed security file (~/.cortex/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Athena config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Athena config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Athena config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.cortex/ form so the Athena config/env patterns catch it. In Docker and
    ...
    # ~, $HOME, or $CORTEX_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live CORTEX_HOME even when that is set
    # after this module is imported — as the hermetic test conftest does.
    command = _rewrite_resolved_cortex_home(command)
    return command


def _rewrite_resolved_cortex_home(command: str) -> str:
    """Rewrite the resolved absolute Athena home prefix to ``~/.cortex/``.

    Resolves the active ``CORTEX_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.cortex/`` so the static ``_HERMES_CONFIG_PATH`` / ``_HERMES_ENV_PATH``
    patterns match. No-op when the path can't be resolved or doesn't appear.
    """
    try:
        from cortex_constants import get_cortex_home
        home = get_cortex_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate CORTEX_HOME (e.g. "/" or "") rewriting
        ...
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.cortex/")
    return command

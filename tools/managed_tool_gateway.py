...
def auth_json_path():
    """Return the Athena auth store path, respecting CORTEX_HOME overrides."""
    return get_cortex_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`athena tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    ...
    try:
        from cortex_cli.auth import resolve_nous_access_token

...
selectable via ``athena tools`` → Image Generation; the active model is
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    ...
    # Athena cache roots can be translated without side effects. SSH can still
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`athena setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `athena tools` "
        "→ Image Generation (run `athena plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)

...
`mcp_tool.py`, and `cortex_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# AthenaMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------
...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class AthenaMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`AthenaTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, AthenaTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import AthenaTokenStorage
                        if isinstance(storage, AthenaTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import AthenaTokenStorage
            if not isinstance(storage, AthenaTokenStorage):
                return
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`AthenaMCPOAuthProvider` directly using the helpers
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``athena mcp remove <name>`` and (indirectly) by
        ``athena mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:

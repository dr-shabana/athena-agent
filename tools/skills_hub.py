#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Athena Skills Hub.
...
Used by cortex_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("athena", {})
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.cortex/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hermes_meta = meta_block.get("athena", {})
                if isinstance(hermes_meta, dict):
                    tags = hermes_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Athena centralized index source
# ---------------------------------------------------------------------------

HERMES_INDEX_URL = "https://athena-agent.nousresearch.com/docs/api/skills-index.json"
...
def _load_hermes_index() -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("Athena index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Athena index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class AthenaIndexSource(SkillSource):
    """Skill source backed by the centralized Athena Skills Index.
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        AthenaIndexSource(auth=auth), # Centralized index (search + resolved install paths)

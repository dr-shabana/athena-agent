...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.cortex/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Athena profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    ...
    * ``label=athena-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Athena process whose reuse path will pick them
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Athena
    process leaves its container behind permanently because there's no
    subsequent Athena process scheduled to reuse that exact (task, profile)
    pair.
    """
...
#   SETUID/SETGID - the image's init drops from root to the 'athena'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``athena-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.cortex/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. athena-agent:latest) already use /init as PID 1
        ...
        #   * the orphan reaper (`athena-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`hermes-task-id`, `hermes-profile`)
        #   * operators running `docker ps --filter label=athena-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Athena profile is captured at
        ...
        # container shared across sessions").  If a prior Athena process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Athena secret blocklist. Only implicit passthrough
        # keys are filtered.
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        # container) but can happen if a previous Athena process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Athena exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Athena process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Athena startup. That covers the
        ...
        # outlives Athena processes, processes inside it stay alive, and
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``athena /quit``

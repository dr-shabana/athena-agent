...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``athena tools`` repaint, tool-registration scans,
    ...
    2. ``~/.cortex/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_cortex_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.cortex/.env`` first, then ``os.environ``.

    Wraps :func:`cortex_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from cortex_cli.config import get_env_value as _hermes_get_env_value
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers Athena-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``cortex_cli.config.get_env_value`` so keys
    stored in ``~/.cortex/.env`` (the standard Athena location) are honored —

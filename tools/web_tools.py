...
Backend is selected during ``athena tools`` setup (web.backend in config.yaml).
When available, Athena can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Athena config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Athena' config/.env layer (``athena config set``, ``athena tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.cortex/config.yaml."""
    try:
...
def _get_backend(capability: str = "search") -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``athena tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `athena tools` repaint.
        try:
...
def _register_bundled_web_providers_directly() -> None:
    ...
    (:func:`cortex_cli.plugins._ensure_plugins_discovered`), which auto-loads
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `athena tools` to set one up."
                ),
            }
        else:
            logger.info(

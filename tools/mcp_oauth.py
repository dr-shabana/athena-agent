...
This module provides the glue:
    - ``AthenaTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Athena Agent"
"""
...
from cortex_constants import secure_parent_dir
...
def _get_token_dir() -> Path:
    ...
    Uses CORTEX_HOME so each profile gets its own OAuth tokens.
    Layout: ``CORTEX_HOME/mcp-tokens/``
    """
    try:
        from cortex_constants import get_cortex_home
        base = Path(get_cortex_home())
    except ImportError:
        base = Path(os.environ.get("CORTEX_HOME", str(Path.home() / ".hermes")))
...
# ---------------------------------------------------------------------------
# AthenaTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class AthenaTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        CORTEX_HOME/mcp-tokens/<server_name>.json         -- tokens
        CORTEX_HOME/mcp-tokens/<server_name>.client.json   -- client info
        CORTEX_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Athena records an absolute wall-clock ``expires_at`` alongside the
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://athena-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `athena mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = AthenaTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Athena Agent")
    scope = cfg.get("scope")

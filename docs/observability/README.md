# Athena Observer Hooks

Athena observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Athena' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Athena catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Athena only builds

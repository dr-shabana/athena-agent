...
needs, and a profile is just a CORTEX_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.cortex/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — CORTEX_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with CORTEX_HOME override |
...
### Seam #1 — hub-skill install cannot use the CORTEX_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = CORTEX_HOME / "skills"` at **module
import time**. The context-local `set_cortex_home_override()` swap (which makes
...
A fresh subprocess re-imports `skills_hub` with the profile's CORTEX_HOME bound
...
2. write model (synchronous, CORTEX_HOME override)
3. write selected MCP servers (synchronous, CORTEX_HOME override)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, CORTEX_HOME override
...
page afterward. Mirror `_write_profile_model`'s CORTEX_HOME-override helper for
...
- Backend E2E with isolated CORTEX_HOME: POST a full create body

...
Athena middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Athena'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Athena calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Athena retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Athena guardrails,
...
Athena stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Athena runs
them as a nested chain in registration order. Middleware failures are fail-open:
Athena logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Athena applies middleware in this order:
...
For each tool call, Athena applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Athena pre-execution path against the effective arguments:
...
For isolated local testing, use one `CORTEX_HOME` for plugin enablement and the
agent run:

```bash
export CORTEX_HOME=/tmp/hermes-middleware-test
mkdir -p "$CORTEX_HOME"
hermes plugins enable <plugin-name>
...
Return the same response shape Athena expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Athena treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Athena preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Athena does not convert downstream

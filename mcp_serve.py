"""
Athena MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Athena-specific extra)
...
logger = logging.getLogger("cortex.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using CORTEX_HOME."""
    try:
        from cortex_constants import get_cortex_home
        return get_cortex_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("CORTEX_HOME", Path.home() / ".hermes")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from cortex_constants import get_cortex_home
        directory_file = get_cortex_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("CORTEX_HOME", Path.home() / ".hermes")
...
class EventBridge:
    ...
    This is the Athena equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from cortex_constants import get_cortex_home
            db_file = get_cortex_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("CORTEX_HOME", Path.home() / ".hermes")) / "state.db"

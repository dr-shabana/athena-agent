"""
Timezone-aware clock for Athena.
...
Resolution order:
  ...
  2. ``timezone`` key in ``~/.cortex/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Athena never
...
from cortex_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Athena requires 3.9+)

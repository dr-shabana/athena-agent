#!/usr/bin/env python3
"""Live test harness for Athena Agent's Tool Search feature.
...
ORIGINAL_HOME = os.environ.get("CORTEX_HOME")
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.cortex/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.cortex/.env`` so
    ...
    cortex_home = home_dir / ".hermes"
    cortex_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, cortex_home / "auth.json")
    ...
    if real_env_file.exists():
        shutil.copy(real_env_file, cortex_home / ".env")
        ...
        load_hermes_dotenv(cortex_home=str(Path.home() / ".hermes"))
    ...
    (cortex_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return cortex_home
...
def reset_module_state():
    """Drop cached modules so the new CORTEX_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "cortex_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original CORTEX_HOME
    if ORIGINAL_HOME is not None:
        os.environ["CORTEX_HOME"] = ORIGINAL_HOME
    else:

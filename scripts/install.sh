...
# Configuration
REPO_URL_SSH="git@github.com:dr-shabana/athena-agent.git"
REPO_URL_HTTPS="https://github.com/dr-shabana/athena-agent.git"
CORTEX_HOME="${CORTEX_HOME:-$HOME/.hermes}"
...
#   code at /usr/local/lib/athena-agent, command at /usr/local/bin/hermes,
#   data still at /root/.hermes (CORTEX_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$CORTEX_HOME/.no-bundled-skills so future"
            ...
            echo "                   default (non-root):  ~/.cortex/athena-agent"
            echo "                   default (root, Linux): /usr/local/lib/athena-agent"
            echo "  --hermes-home PATH  Data directory (default: ~/.hermes, or \$CORTEX_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Hermes installs the code under"
            echo "  /usr/local/lib/athena-agent and links the command into"
            echo "  /usr/local/bin/hermes (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$CORTEX_HOME"
            echo "  (default /root/.hermes).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$CORTEX_HOME/athena-agent are preserved in-place."
...
emit_manifest() {
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Athena Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hermes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $CORTEX_HOME/athena-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $CORTEX_HOME/athena-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/athena-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $CORTEX_HOME/athena-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in CORTEX_HOME.
    if is_termux; then
        INSTALL_DIR="$CORTEX_HOME/athena-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$CORTEX_HOME/athena-agent/.git" ]; then
            INSTALL_DIR="$CORTEX_HOME/athena-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/athena-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/athena-agent"
        ...
        log_info "  Data:    $CORTEX_HOME (unchanged)"
...
install_uv() {
    ...
    # Hermes owns its own uv at $CORTEX_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.sh and `hermes update` stay in sync.
    local _managed_uv="$CORTEX_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $CORTEX_HOME/bin ..."
    mkdir -p "$CORTEX_HOME/bin"
    ...
    # directly into $CORTEX_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$CORTEX_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_node() {
    ...
    if [ -x "$CORTEX_HOME/node/bin/node" ] && node_satisfies_build "$("$CORTEX_HOME/node/bin/node" --version)"; then
        export PATH="$CORTEX_HOME/node/bin:$PATH"
        log_success "Node.js $("$CORTEX_HOME/node/bin/node" --version) found (Hermes-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.cortex/node/..."
    ...
    # Place into ~/.cortex/node/ and symlink binaries into the same bin dir
    # the hermes command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$CORTEX_HOME/node"
    mkdir -p "$CORTEX_HOME"
    mv "$extracted_dir" "$CORTEX_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$CORTEX_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$CORTEX_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$CORTEX_HOME/node/bin/npx"  "$node_link_dir/npx"

    export PATH="$CORTEX_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$CORTEX_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.cortex/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Athena Agent itself does not require or retain root access."
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"athena-agent\[([\w-]+)\]", s)
        if m:
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# Athena Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Athena Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Athena Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
copy_config_templates() {
    ...
    mkdir -p "$CORTEX_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.cortex/.env (top level, easy to find)
    if [ ! -f "$CORTEX_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$CORTEX_HOME/.env"
            log_success "Created ~/.cortex/.env from template"
        else
            touch "$CORTEX_HOME/.env"
            log_success "Created ~/.cortex/.env"
        fi
    else
        log_info "~/.cortex/.env already exists, keeping it"
    fi
    ...
    # Create config.yaml at ~/.cortex/config.yaml (top level, easy to find)
    if [ ! -f "$CORTEX_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$CORTEX_HOME/config.yaml"
            log_success "Created ~/.cortex/config.yaml from template"
        fi
    else
        log_info "~/.cortex/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$CORTEX_HOME/SOUL.md" ]; then
        cat > "$CORTEX_HOME/SOUL.md" << 'SOUL_EOF'
...
-->
SOUL_EOF
        log_success "Created ~/.cortex/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.cortex/"

    # Seed bundled skills into ~/.cortex/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        ...
        printf '%s\n' \
            ...
            > "$CORTEX_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $CORTEX_HOME/.no-bundled-skills"
        log_info "  Future 'hermes update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.cortex/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.cortex/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$CORTEX_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$CORTEX_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.cortex/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$CORTEX_HOME/.env"
    ...
    {
        echo ""
        echo "# Athena Agent browser tools — use the system Chrome/Chromium binary."
...
maybe_start_gateway() {
    ...
    WHATSAPP_SESSION="$CORTEX_HOME/whatsapp/session/creds.json"
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            nohup $HERMES_CMD gateway > "$CORTEX_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.cortex/logs/gateway.log"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $CORTEX_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $CORTEX_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $CORTEX_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'athena' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'athena' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'athena' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$CORTEX_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$CORTEX_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$CORTEX_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$CORTEX_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$CORTEX_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            # provisioned during prerequisites/node-deps (at $CORTEX_HOME/node/bin)
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$CORTEX_HOME/.install_method"
            ;;
        *)

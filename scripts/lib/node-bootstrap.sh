...
#   2. ~/.cortex/node/ from a prior Hermes-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.cortex/node/ (always works, zero shell rc edits)
#
# Usage:
...
#   CORTEX_HOME               (default: $HOME/.hermes)
# ============================================================================
...
_nb_install_bundled_node() {
    ...
    _nb_log "Extracting to $CORTEX_HOME/node/..."
    ...
    mv "$extracted" "$CORTEX_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    mkdir -p "$_link_dir"
    ln -sf "$CORTEX_HOME/node/bin/node" "$_link_dir/node"
    ln -sf "$CORTEX_HOME/node/bin/npm"  "$_link_dir/npm"
    ln -sf "$CORTEX_HOME/node/bin/npx"  "$_link_dir/npx"
    export PATH="$CORTEX_HOME/node/bin:$PATH"

    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $CORTEX_HOME/node/"
    return 0
}
...
ensure_node() {
    ...
    if [ -x "$CORTEX_HOME/node/bin/node" ]; then
        export PATH="$CORTEX_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then

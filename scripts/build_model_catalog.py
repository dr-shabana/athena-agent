#!/usr/bin/env python3
"""Build the Athena Model Catalog — a centralized JSON manifest of curated models.
...
Athena CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Athena release.
...
# Ensure CORTEX_HOME is set for imports that touch it at module level.
os.environ.setdefault("CORTEX_HOME", os.path.join(os.path.expanduser("~"), ".hermes"))

from cortex_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS  # noqa: E402
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "athena-agent repo",
            "docs": "https://athena-agent.nousresearch.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {

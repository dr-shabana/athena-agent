"""
LINE Messaging API platform adapter for Athena Agent.
...
to Athena Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``CORTEX_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            cortex_home = Path.home().joinpath(".hermes").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``athena status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``athena status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``athena setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.cortex/.env`` via ``cortex_cli.config``.
    """
    print()
    ...
    try:
        from cortex_cli.config import get_env_var, set_env_var
    except ImportError:
        print("cortex_cli.config not available; set LINE_* vars manually in ~/.cortex/.env")
        return

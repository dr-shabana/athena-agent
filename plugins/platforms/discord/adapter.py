...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.cortex/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Athena gateway
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_cortex_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Athena session")
        ...
        @tree.command(name="status", description="Show Athena session status")
        ...
        @tree.command(name="stop", description="Stop the running Athena agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.cortex/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Athena Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Athena gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Athena session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Athena in the thread",
        ...
        # cortex_cli/commands.py automatically appear as Discord slash
        ...
        try:
            from cortex_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from cortex_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Athena skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from cortex_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Athena session in the thread
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Athena: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``athena update --gateway``
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``athena update`` prompts.
...
# is not in this process (e.g. ``athena cron`` running standalone) and no live
...
def interactive_setup() -> None:
    ...
    from cortex_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``cortex_cli.gateway.get_env_value`` at call time (not via
    ...
    """
    import cortex_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'athena-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # cortex_cli/setup.py::_setup_discord function.  Same shape as Teams.

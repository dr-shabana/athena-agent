...
This plugin connects Athena Agent to iMessage (and other Spectrum
...
TypeScript-only, Athena runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Athena-Sidecar-Token`.
...
2. **Find or create** the `Athena Agent` project on the Photon dashboard.
...
There is no separate `login` command; like every other Athena channel,
...
Runtime SDK credentials live in `~/.cortex/.env` (the same place every other
...
Management metadata lives in `~/.cortex/auth.json` under `credential_pool`:

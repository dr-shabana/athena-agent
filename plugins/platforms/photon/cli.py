"""
``athena photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Athena
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `athena photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Athena Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``athena photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Athena Agent" project.
    ...
    #    and persist both (runtime creds -> ~/.cortex/.env, ids -> auth.json).
    try:
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `athena photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`athena photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.cortex/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `athena photon telemetry on` / `athena photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.cortex/.env)")
...
#
# `athena gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``athena photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `athena gateway setup` wizard."""

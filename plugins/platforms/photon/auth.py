...
Credential storage mirrors every other Athena channel:

    * runtime SDK creds  -> ``~/.cortex/.env``  (``PHOTON_PROJECT_ID`` =
      spectrumProjectId, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.cortex/auth.json`` under
...
# registers Athena as its own client_id.
...
# Default name of the project Athena provisions for the operator.
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of athena-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.cortex/auth.json`` honouring the active Athena profile."""
    try:
        from cortex_constants import get_cortex_home
        return Path(get_cortex_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.cortex")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.cortex/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.cortex/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.cortex/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: cortex_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `athena photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `athena photon setup`)"
    )

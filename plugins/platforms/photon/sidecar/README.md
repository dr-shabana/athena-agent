...
Small Node helper that bridges Athena Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Athena is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Athena-Sidecar-Token`)
...
The Athena plugin's `hermes photon setup` command runs `npm install`
here automatically.

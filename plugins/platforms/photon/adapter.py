"""
Photon Spectrum (iMessage) platform adapter for Athena Agent.
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `athena photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `athena setup` / status.
        return False
    return True
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Athena defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Athena-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Athena checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `athena photon setup`)"
            )
...
def register(ctx) -> None:
    ...
    # gateway-setup hook and the `athena photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        # Surfaces Photon in `athena gateway setup` alongside every other
    ...
    # Register CLI subcommands — `athena photon ...`

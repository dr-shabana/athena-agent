...
# ``model_tools`` import time, so ``athena status``, ``athena chat``, etc.
...
#     fresh "Athena is thinking…" card during the small window between
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${CORTEX_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _cortex_home = _get_cortex_home()
        except (ModuleNotFoundError, ImportError):
            _cortex_home = _Path.home() / ".hermes"
        self._thread_count_store = _ThreadCountStore(
            _cortex_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Athena is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("CORTEX_HOME", str(_Path.home() / ".hermes"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${CORTEX_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Athena is thinking…' marker message.
        ...
        "Athena is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Athena is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Athena is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Athena is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Athena is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``athena setup``.

    The setup wizard at ``cortex_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.cortex/.env`` so the next
    ...
    print()
    print_success("Google Chat configuration saved to ~/.cortex/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``athena cron`` running as a
    separate process from ``athena gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Athena plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'athena-agent[google_chat]'",
        ...
        # the "Athena is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Athena is thinking…' marker message "

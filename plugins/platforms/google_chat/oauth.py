...
- Per-user tokens (keyed by sender email):
    ``${CORTEX_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${CORTEX_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${CORTEX_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${CORTEX_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${CORTEX_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's CORTEX_HOME helper so the token follows the user's
# profile (e.g. tests can override via CORTEX_HOME=/tmp/...).
try:
    from cortex_constants import display_cortex_home, get_cortex_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where cortex_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _cortex_home.py shim).
...
def _cortex_home() -> Path:
    """Resolve CORTEX_HOME at call time (NOT module import).

    Tests and ``CORTEX_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.cortex/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _cortex_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _cortex_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _cortex_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _cortex_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _cortex_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install athena-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'athena-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into CORTEX_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_cortex_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_cortex_home()}/google_chat_user_token.json"
    )

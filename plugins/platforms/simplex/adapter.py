"""SimpleX Chat platform adapter (Athena plugin).
...
This adapter ships as a Athena platform plugin under
``plugins/platforms/simplex/``. The Athena plugin loader scans the
...
discoverable and ``athena setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Athena — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``athena setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.cortex/.env`` via
    ``cortex_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "cortex_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.cortex/.env"
        )
        return

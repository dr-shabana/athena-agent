"""ntfy platform adapter (Athena plugin).
...
``plugins/platforms/ntfy/``. The Athena plugin loader scans the
...
_ECHO_TAG = "athena-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``athena cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Athena dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `athena gateway status` without

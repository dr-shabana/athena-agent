"""
IRC Platform Adapter for Athena Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Athena agent.  Zero external dependencies — uses
...
def check_requirements() -> bool:
    ...
    # athena setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `athena gateway setup` flow for the IRC platform.

    Lazy-imports ``cortex_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Athena to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    print()
    print_success("IRC configuration saved to ~/.cortex/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Athena Agent (cron)")

"""langfuse — Athena plugin for Langfuse observability.

Traces Athena conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Athena plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``athena plugins enable
observability/langfuse`` or ``athena tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``athena tools`` or ~/.cortex/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Athena plugin system —
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Athena branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Athena also has a turn-scoped
    ...
    # messages is a list only for legacy Athena branches that fired
    # pre_llm_call with API messages directly. Current Athena fires
...
def register(ctx) -> None:
    ...
    # Athena versions.  pre_api_request / post_api_request fire per API

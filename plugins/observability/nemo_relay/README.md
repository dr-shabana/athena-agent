...
Optional Athena observability plugin that maps Athena observer hooks to
...
not replace Athena Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Athena emit NeMo Relay lifecycle events for
the work Athena already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Athena Agent can:

- Preserve Athena execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
For isolated test homes, enable the plugin in the same `CORTEX_HOME` that the
agent run will use:

```bash
env CORTEX_HOME=/tmp/hermes-nemo-relay-test \
  hermes plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`CORTEX_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`CORTEX_HOME` is the Athena profile/config home used by both
`hermes plugins enable ...` and the later `hermes chat ...` run. If unset,
Athena uses the user's default home, usually `~/.hermes`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export CORTEX_HOME=/tmp/hermes-nemo-relay-test
hermes plugins enable observability/nemo_relay
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/cortex_agent-*.whl
...
`HERMES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Athena does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Athena keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Athena routes LLM and tool
...
For the full generic Athena middleware contract, see
...
export CORTEX_HOME=/tmp/hermes-nemo-relay-docs/hermes-home
mkdir -p "$CORTEX_HOME"

cat > "$CORTEX_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Athena session, delegates to a child subagent, has the
...
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='Athena Agent E2E'
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Athena Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Athena dispatches the read-only tools as one batch, and NeMo Relay
...
- Athena sessions map to `agent` scopes.
- Athena API request hooks map to `llm` scope start/end events.
- Athena tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Athena execution middleware to hand
...
When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Athena routes execution
through these boundaries:

```text
Athena provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Athena provider adapter next_call(...)

Athena tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Athena tool dispatcher next_call(...)
```
...
middleware for a local Athena run. This path requires a NeMo Relay runtime that
...
```bash
export CORTEX_HOME=/tmp/hermes-middleware-test/hermes-home
mkdir -p "$CORTEX_HOME" /tmp/hermes-middleware-test/nemo-relay
...
agent_name = "Athena Middleware E2E"
agent_version = "local"

"""disk_cleanup — ephemeral file cleanup for Athena Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under CORTEX_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly CORTEX_HOME and /tmp/hermes-*
Never touches: ~/.cortex/logs/ or any system directory.
"""
...
try:
    ...
    def get_cortex_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("CORTEX_HOME") or "").strip()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$CORTEX_HOME/logs/``."""
    return get_cortex_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$CORTEX_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under CORTEX_HOME or ``/tmp/hermes-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    cortex_home = get_cortex_home()
    try:
        path.resolve().relative_to(cortex_home)
        return True
...
# Paths under $CORTEX_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so CORTEX_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        cortex_home = get_cortex_home()
        for parent in ("cron", "cronjobs"):
            base = cortex_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside CORTEX_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under CORTEX_HOME (but leave CORTEX_HOME itself and
    ...
    empty_removed = 0
    try:
        for dirpath in sorted(cortex_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == cortex_home:
                continue
            try:
                rel_parts = dirpath.relative_to(cortex_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    cortex_home = get_cortex_home()
    try:
        rel = path.resolve().relative_to(cortex_home)
        ...
        # Path isn't under CORTEX_HOME (e.g. /tmp/hermes-*) — fall through.
        pass

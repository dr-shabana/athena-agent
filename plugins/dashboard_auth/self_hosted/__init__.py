...
A standards-compliant OpenID Connect Relying Party for the ``athena dashboard``
...
:class:`~cortex_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``cortex_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~cortex_cli.dashboard_auth.Session`.
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Athena does not call

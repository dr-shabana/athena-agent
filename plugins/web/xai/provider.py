...
``{title, url, description, position}`` rows every other Athena web
provider produces.
...
prefers Athena-managed xAI Grok OAuth (via ``athena auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.cortex/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``athena tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `athena auth` to sign in with "

...
Launched by ``athena meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$CORTEX_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``athena meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_cortex_home()) / "workspace" / "meetings" / "node_token.json"

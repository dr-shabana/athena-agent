...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with athena-agent)
2. User-installed providers: ``$CORTEX_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$CORTEX_HOME/plugins/`` or None if unavailable."""
    try:
        from cortex_constants import get_cortex_home
        d = get_cortex_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$CORTEX_HOME/plugins/<name>/``) directories.  Bundled takes

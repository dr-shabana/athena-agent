...
Or via $CORTEX_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Athena can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $CORTEX_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_cortex_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "athena": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "athena"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Athena config."""
    profile = config.get("profile", "athena")
    return str(profile or "athena")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Athena profile name (from agent_identity)
      {workspace} — Athena workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``hermes-{user}`` with no user becomes ``athena``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Athena Agent and the User"
    ...
    def save_config(self, values, cortex_home):
        """Write config to $CORTEX_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(cortex_home) / "hindsight"
    ...
    def post_setup(self, cortex_home: str, config: dict) -> None:
        ...
        from cortex_cli.memory_setup import _curses_select
        ...
        self.save_config(provider_config, cortex_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(cortex_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(cortex_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "athena"},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Athena Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "athena"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "athena"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "athena", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "athena")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Athena Agent and the User")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See athena-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Athena created/used that client on the shared async loop,

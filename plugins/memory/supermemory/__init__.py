...
def _load_supermemory_config(cortex_home: str) -> dict:
    config = _default_config()
    config_path = Path(cortex_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, cortex_home: str) -> None:
    config_path = Path(cortex_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "athena"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Athena writes into the "Athena" Space in the Supermemory
        ...
        merged = {"sm_source": "athena", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `athena memory setup`.
        # All other options are documented for $CORTEX_HOME/supermemory.json
    ...
    def save_config(self, values, cortex_home):
        ...
        _save_supermemory_config(sanitized, cortex_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from cortex_constants import get_cortex_home
        self._cortex_home = kwargs.get("cortex_home") or str(get_cortex_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._cortex_home)

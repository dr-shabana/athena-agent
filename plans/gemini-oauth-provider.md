...
## Token Lifecycle
- Store at `~/.cortex/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
- athena-agent Copilot OAuth: `hermes_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)

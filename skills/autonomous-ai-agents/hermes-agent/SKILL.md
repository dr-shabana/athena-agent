...
metadata:
  hermes:
    ...
    homepage: https://github.com/dr-shabana/athena-agent
...
~/.cortex/.env              API keys and secrets (under $CORTEX_HOME if set)
...
Config: `~/.cortex/config.yaml` (settings), `~/.cortex/.env` (API keys) — both under `$CORTEX_HOME` when it is set.

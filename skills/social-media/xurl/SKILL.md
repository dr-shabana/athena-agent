...
version: 1.1.1
author: xdevplatform + openclaw + Athena Agent
license: MIT
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Athena tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Athena Docker layout, `/opt/data` is `CORTEX_HOME`, but Athena tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Athena-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Athena subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Athena adaptation: reformatted for Athena skill conventions; safety guardrails preserved verbatim.

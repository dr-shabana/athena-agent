...
`$CORTEX_HOME/config.yaml` (defaults to `~/.cortex/config.yaml` when `CORTEX_HOME` is unset)
...
Restart the Athena session for changes to take effect. The MCP connection is

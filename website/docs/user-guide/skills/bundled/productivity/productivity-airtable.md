...
4. Store the token in `~/.cortex/.env` (or via `hermes setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.cortex/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.

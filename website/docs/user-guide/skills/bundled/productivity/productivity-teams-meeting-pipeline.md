...
description: "Operate the Teams meeting summary pipeline via Athena CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions"
---
...
Operate the Teams meeting summary pipeline via Athena CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions.
...
Before using the pipeline, verify these are set in `~/.cortex/.env`:

...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`athena-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-athena-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Athena terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Athena itself, `model.provider: openai-codex` uses Athena-managed Codex
OAuth from `~/.cortex/auth.json` after `hermes auth add openai-codex`. For the
...
## Athena Gateway Caveat

When invoking the Codex CLI from a Athena gateway/service context (for example,

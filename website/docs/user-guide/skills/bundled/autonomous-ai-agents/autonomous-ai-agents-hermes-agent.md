---
title: "Athena Agent — Configure, extend, or contribute to Athena Agent"
sidebar_label: "Athena Agent"
description: "Configure, extend, or contribute to Athena Agent"
---
...
Configure, extend, or contribute to Athena Agent.
...
| Path | `skills/autonomous-ai-agents/athena-agent` |
| Version | `2.1.0` |
| Author | Athena Agent + Teknium |
| License | MIT |
...
Athena Agent is an open-source AI agent framework by Nous Research that runs in your terminal, messaging platforms, and IDEs. It belongs to the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Athena works with any LLM provider (OpenRouter, Anthropic, OpenAI, DeepSeek, local models, and 15+ others) and runs on Linux, macOS, and WSL.

What makes Athena different:

- **Self-improving through skills** — Athena learns from experience by saving reusable procedures as skills. When it solves a complex problem, discovers a workflow, or gets corrected, it can persist that knowledge as a skill document that loads into future sessions. Skills accumulate over time, making the agent better at your specific tasks and environment.
- **Persistent memory across sessions** — remembers who you are, your preferences, environment details, and lessons learned. Pluggable memory backends (built-in, Honcho, Mem0, and more) let you choose how memory works.
- **Multi-platform gateway** — the same agent runs on Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Email, and 10+ other platforms with full tool access, not just chat.
- **Provider-agnostic** — swap models and providers mid-workflow without changing anything else. Credential pools rotate across multiple API keys automatically.
- **Profiles** — run multiple independent Athena instances with isolated configs, sessions, skills, and memory.
- **Extensible** — plugins, MCP servers, custom tools, webhook triggers, cron scheduling, and the full Python ecosystem.

People use Athena for software development, research, system administration, data analysis, content creation, home automation, and anything else that benefits from an AI agent with persistent context and full system access.

**This skill helps you work with Athena Agent effectively** — setting it up, configuring features, spawning additional agent instances, troubleshooting issues, finding the right commands and settings, and understanding how the system works when you need to extend or contribute to it.

**Docs:** https://athena-agent.nousresearch.com/docs/
...
```
hermes mcp serve            Run Athena as an MCP server
...
(`hermes mcp install <name>`): `skill_view(name="athena-agent", file_path="references/native-mcp.md")`.
...
Platform docs: https://athena-agent.nousresearch.com/docs/user-guide/messaging/
...
patterns: `skill_view(name="athena-agent", file_path="references/webhooks.md")`.
...
hermes uninstall            Uninstall Athena
```
...
authoritative list or see the [live slash commands reference](https://athena-agent.nousresearch.com/docs/reference/slash-commands).
...
/snapshot [sub]      Create or restore state snapshots of Athena config/state (CLI)
/background <prompt> Run prompt in background
/queue <prompt>      Queue for next turn
/steer <prompt>      Inject a message after the next tool call without interrupting
/agents (/tasks)     Show active agents and running tasks
/resume [name]       Resume a named session
/goal [text|sub]     Set a standing goal Athena works on across turns until achieved
...
/busy [sub]          Control what Enter does while Athena is working (CLI)
...
/reload-skills       Re-scan ~/.cortex/skills/ for added/removed skills
...
/update              Update Athena to latest (gateway)
...
```
~/.cortex/config.yaml       Main configuration
~/.cortex/.env              API keys and secrets
$CORTEX_HOME/skills/        Installed skills
~/.cortex/sessions/         Gateway routing index, request dumps, *.jsonl transcripts (and optional per-session JSON snapshots when sessions.write_json_snapshots: true)
~/.cortex/state.db          Canonical session store (SQLite + FTS5)
~/.cortex/logs/             Gateway and error logs
~/.cortex/auth.json         OAuth tokens and credential pools
~/.cortex/athena-agent/     Source code (if git-installed)
```

Profiles use `~/.cortex/profiles/<name>/` with the same layout.
...
Full config reference: https://athena-agent.nousresearch.com/docs/user-guide/configuration
...
Full provider docs: https://athena-agent.nousresearch.com/docs/integrations/providers
...
Common "why is Athena doing X to my output / tool calls / commands?" toggles — and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `hermes` invocation) because they're read once at startup.
...
By default (`approvals.mode: manual`), Athena prompts the user before running shell commands flagged as destructive (`rm -rf`, `git reset --hard`, etc.). The modes are:
...
- `hermes --yolo …`
- `export CORTEX_YOLO_MODE=1`

Note: YOLO / `approvals.mode: off` does NOT turn off secret redaction. They are independent.

### Shell hooks allowlist

Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.cortex/shell-hooks-allowlist.json` — prompted interactively the first time a hook wants to run.
...
## Spawning Additional Athena Instances

Run additional Athena processes as fully independent subprocesses — separate sessions, tools, and environments.
...
Athena uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:

```
# Start
terminal(command="tmux new-session -d -s agent1 -x 120 -y 40 'athena'", timeout=10)
...
User docs: https://athena-agent.nousresearch.com/docs/user-guide/features/cron
...
- **Telemetry:** sidecar at `~/.cortex/skills/.usage.json` holds
  per-skill `use_count`, `view_count`, `patch_count`,
  `last_activity_at`, `state`, `pinned`.

Config: `curator.*` (`enabled`, `interval_hours`, `min_idle_hours`,
`stale_after_days`, `archive_after_days`, `backup.*`).
User docs: https://athena-agent.nousresearch.com/docs/user-guide/features/curator

### Kanban (multi-agent work queue)

Durable SQLite board for multi-profile / multi-worker collaboration.
Users drive it via `hermes kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `CORTEX_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `CORTEX_KANBAN_BOARD` pinned in env); tenant is a soft namespace
  within a board for workspace-path + memory-key isolation.

User docs: https://athena-agent.nousresearch.com/docs/user-guide/features/kanban

---

## Windows-Specific Quirks

Athena runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
Athena's own env scrubber dropping `SYSTEMROOT` / `WINDIR` / `COMSPEC`
...
POSIX venv layouts (`.venv/bin/activate`). The Athena-installed venv at
...
every Athena tool and most Windows APIs. Prefer forward slashes in code
...
Check logs first:
```bash
grep -i "failed to send\|error" ~/.cortex/logs/gateway.log | tail -20
```
...
|----------------|----------|
| Config options | `hermes config edit` or [Configuration docs](https://athena-agent.nousresearch.com/docs/user-guide/configuration) |
| Available tools | `hermes tools list` or [Tools reference](https://athena-agent.nousresearch.com/docs/reference/tools-reference) |
| Slash commands | `/help` in session or [Slash commands reference](https://athena-agent.nousresearch.com/docs/reference/slash-commands) |
| Skills catalog | `hermes skills browse` or [Skills catalog](https://athena-agent.nousresearch.com/docs/reference/skills-catalog) |
| Provider setup | `hermes model` or [Providers guide](https://athena-agent.nousresearch.com/docs/integrations/providers) |
| Platform setup | `hermes gateway setup` or [Messaging docs](https://athena-agent.nousresearch.com/docs/user-guide/messaging/) |
| MCP servers | `hermes mcp list` or [MCP guide](https://athena-agent.nousresearch.com/docs/user-guide/features/mcp) |
| Profiles | `hermes profile list` or [Profiles docs](https://athena-agent.nousresearch.com/docs/user-guide/profiles) |
| Cron jobs | `hermes cron list` or [Cron docs](https://athena-agent.nousresearch.com/docs/user-guide/features/cron) |
| Memory | `hermes memory status` or [Memory docs](https://athena-agent.nousresearch.com/docs/user-guide/features/memory) |
| Env variables | `hermes config env-path` or [Env vars reference](https://athena-agent.nousresearch.com/docs/reference/environment-variables) |
| CLI commands | `hermes --help` or [CLI reference](https://athena-agent.nousresearch.com/docs/reference/cli-commands) |
| Gateway logs | `~/.cortex/logs/gateway.log` |
| Session files | `hermes sessions browse` (reads state.db) |
| Source code | `~/.cortex/athena-agent/` |

---

## Contributor Quick Reference

For occasional contributors and PR authors. Full developer docs: https://athena-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.cortex/config.yaml` (settings), `~/.cortex/.env` (API keys).
...
All handlers must return JSON strings. Use `get_cortex_home()` for paths, never hardcode `~/.hermes`.
...
- Tests auto-redirect `CORTEX_HOME` to temp dirs — never touch real `~/.cortex/`
- Run full suite before pushing any change
- Use `-o 'addopts='` to clear any baked-in pytest flags

**Windows contributors:** `scripts/run_tests.sh` currently looks for POSIX venvs (`.venv/bin/activate` / `venv/bin/activate`) and will error out on Windows where the layout is `venv/Scripts/activate` + `python.exe`. The Athena-installed venv at `venv/Scripts/` also has no `pip` or `pytest` — it's stripped for end-user install size. Workaround: install pytest + pytest-xdist + pyyaml into a system Python 3.11 user site (`/c/Program Files/Python311/python -m pip install --user pytest pytest-xdist pyyaml`), then run tests directly:
...
- Use `get_cortex_home()` from `hermes_constants` for all paths (profile-safe)

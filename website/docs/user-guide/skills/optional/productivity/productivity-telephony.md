---
title: "Telephony — Give Athena phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Athena phone capabilities without core tool changes"
---
...
Give Athena phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
...
This optional skill gives Athena practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.cortex/.env`
...
It does **not** turn Athena into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Athena memory
...
### 1) "I want Athena to own a real phone number"
Use **Twilio**.
...
- pairs well with Athena `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.cortex/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
Then save credentials into Athena:
...
3. Buy it and save it into `~/.cortex/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Athena calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Athena's existing `text_to_speech` support.

Use this when:
- you want the call to use Athena's configured TTS voice rather than Twilio `<Say>`
...
Recommended Athena TTS -> Twilio Play workflow:

1. Generate the audio with Athena `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Athena to create the message audio with `text_to_speech`
...
Important note:
- Athena TTS is great for prerecorded outbound messages
...
- Athena STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Athena memory.
...
- Do not store arbitrary third-party phone numbers in Athena memory.
...
3. persist that number to `~/.cortex/.env`

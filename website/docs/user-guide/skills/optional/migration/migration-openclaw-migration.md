---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Athena Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Athena Agent"
---
...
Migrate a user's OpenClaw customization footprint into Athena Agent. Imports Athena-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Version | `1.0.0` |
| Author | Athena Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Athena`, `Memory`, `Persona`, `Import` |
...
Use this skill when a user wants to move their OpenClaw setup into Athena Agent with minimal manual cleanup.
...
- import `SOUL.md` into the Athena home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Athena memory entries
- merge OpenClaw command approval patterns into Athena `command_allowlist`
- migrate Athena-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Athena working-directory configuration
- copy OpenClaw skills into `~/.cortex/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Athena workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.cortex/tts/`
- archive non-secret docs that do not have a direct Athena destination
...
- `~/.cortex/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.cortex/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.cortex/skills/migration/openclaw-migration/`.
...
Athena CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Athena. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Athena workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Athena skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.cortex/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```
...
```bash
python3 ~/.cortex/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.cortex/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.cortex/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.cortex/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Athena unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Athena targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Athena destination. Unsupported auth blobs must still be reported as skipped.
...
- Athena persona state imported
- Athena memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.cortex/skills/openclaw-imports/`

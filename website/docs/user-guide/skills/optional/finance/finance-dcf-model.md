...
**If you encounter errors or unreasonable results, read [TROUBLESHOOTING.md](https://github.com/dr-shabana/athena-agent/blob/main/optional-skills/finance/dcf-model/TROUBLESHOOTING.md) for detailed debugging guidance.**
...
3. **Check output**:
   ...
   - If `status` is `"errors_found"` → Check `error_summary` and read [TROUBLESHOOTING.md](https://github.com/dr-shabana/athena-agent/blob/main/optional-skills/finance/dcf-model/TROUBLESHOOTING.md) for debugging guidance
...
Many passages below say "use the S&P Kensho MCP / Daloopa MCP / FactSet MCP". Those are commercial financial-data MCPs from the original Cowork plugin context. In Athena:

- **If you have any structured financial-data MCP configured** (Athena supports MCP — see `native-mcp` skill), prefer it for point-in-time comps, precedent transactions, and filings.

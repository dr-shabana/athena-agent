...
12. **execute_code sandbox has no env vars** — When Athena runs auto_jailbreak via execute_code, the sandbox doesn't inherit `~/.cortex/.env`. Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.cortex/.env"))`

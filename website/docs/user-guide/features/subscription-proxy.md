...
chat completions — use your Athena-managed provider subscription as their
...
| Use case | "Use Athena as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Athena stores
the refresh token in `~/.cortex/auth.json` — the same place all Athena
...
Model:      Athena-4-70B    # or Athena-4.3-36B, Athena-4-405B
```

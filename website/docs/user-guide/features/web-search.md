...
The summary keeps quotes, code blocks, and key facts in their original formatting — it's a content compressor, not a paraphraser. If summarization fails or times out, Athena falls back to the first ~5 000 chars of raw content rather than a useless error.
...
Then select SearXNG as the search backend in `~/.cortex/config.yaml`:
...
If no backend is explicitly configured, Athena picks the first available one based on which credentials are set:

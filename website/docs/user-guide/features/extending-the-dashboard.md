...
description: "Build themes and plugins for the Athena web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Athena web dashboard (`hermes dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.cortex/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.cortex/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.cortex/dashboard-themes
```

```yaml
# ~/.cortex/dashboard-themes/neon.yaml
name: neon
label: Neon
...
|-------|---------|------------|--------|
| **Athena Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Athena Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Athena Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
```yaml
# ~/.cortex/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Athena plugins in `~/.cortex/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.
...
```bash
mkdir -p ~/.cortex/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.cortex/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.cortex/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
#### Calling built-in Athena endpoints
...
| `header-left` | Before the Athena brand in the top bar. |
...
```json
// ~/.cortex/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.cortex/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__HERMES_PLUGIN_SDK__;
  const { React } = SDK;
...
A reference plugin (`example-dashboard` in [`hermes-example-plugins`](https://github.com/dr-shabana/hermes-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
```python
# ~/.cortex/plugins/my-plugin/dashboard/plugin_api.py
...
#### Accessing Athena internals

Backend routes run inside the dashboard process, so they can import from the athena-agent codebase directly:
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.cortex/plugins/<name>/dashboard/` | `user` |
...
The [`strike-freedom-cockpit`](https://github.com/dr-shabana/hermes-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `hermes-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/dr-shabana/hermes-example-plugins.git

# Theme
cp hermes-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.cortex/dashboard-themes/

# Plugin
cp -r hermes-example-plugins/strike-freedom-cockpit ~/.cortex/plugins/
```

Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Athena Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
Check that the file is in `~/.cortex/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.cortex/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.cortex/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
4. Tail `~/.cortex/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.cortex/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.

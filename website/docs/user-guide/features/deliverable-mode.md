...
When Athena Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.cortex/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.cortex/config.yaml` (switchable per session
via `/personality`).
...
If you use Athena' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.cortex/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Athena Agent's deliverable

...
Type `@` followed by a reference to inject content directly into your message. Athena expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Athena env: `$CORTEX_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$CORTEX_HOME/skills/.hub/`

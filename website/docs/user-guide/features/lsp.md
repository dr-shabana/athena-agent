...
Athena runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Athena
...
1. Athena captures a baseline of current diagnostics for the file.
...
…). Athena auto-detects the binary on PATH or in
`<CORTEX_HOME>/lsp/bin/`.
...
`node_modules` tree — Athena installs both packages together when you
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <CORTEX_HOME>/lsp/bin
...
When `install_strategy: auto`, Athena installs binaries into
`<CORTEX_HOME>/lsp/bin/`. NPM packages land in
`<CORTEX_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Athena-owned and is
...
Servers are kept alive for the life of the Athena process. There's
...
The binary isn't on PATH and isn't in `<CORTEX_HOME>/lsp/bin/`. Run
...
`~/.cortex/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.cortex/logs/agent.log` for `[agent.lsp.client]` entries —

...
description: "Customize Athena Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Athena Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `CORTEX_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Athena is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Athena now seeds a default `SOUL.md` automatically in:

```text
~/.cortex/SOUL.md
```

More precisely, it uses the current instance's `CORTEX_HOME`, so if you run Athena with a custom home directory, it will use:
...
- Athena creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Athena loads `SOUL.md` only from `CORTEX_HOME`
- Athena does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Athena falls back to a built-in default identity
...
If Athena loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `CORTEX_HOME`, the personality belongs to the Athena instance itself.

That also makes it easier to teach users:
- "Edit `~/.cortex/SOUL.md` to change Athena' default personality."
...
- how Athena should handle uncertainty, disagreement, or ambiguity
...
## What Athena injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Athena falls back to a built-in default identity ("You are Athena Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Athena ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Athena, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Athena its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.cortex/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.cortex/SOUL.md`
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Athena speaks
- `display.skin` and `/skin` affect how Athena looks in the terminal

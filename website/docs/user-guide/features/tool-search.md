...
**Tool Search** is Athena' opt-in progressive-disclosure layer for that
...
:::info Built-in Athena tools never defer
The tools that make up Athena' core capability set (`terminal`,
...
When the model invokes `tool_call`, Athena **unwraps the bridge** and
...
- **No JS sandbox.** Athena uses the simpler "structured tools" mode

...
Athena Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
4. Start a session with the toolset enabled:
   ...
   ```
   or add `computer_use` to your enabled toolsets in `~/.cortex/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Athena refreshes the binary in two
places so you don't get stuck on a stale release:

- **`hermes update`** — when you update Athena itself, if `cua-driver` is
...
Athena applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.cortex/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Athena applies four layers of optimisation:
...
- [Universal skill: `macos-computer-use`](https://github.com/dr-shabana/athena-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)

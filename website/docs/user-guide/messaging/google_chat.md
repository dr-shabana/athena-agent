...
sidebar_position: 12
title: "Google Chat"
description: "Set up Athena Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Athena Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Athena
...
download the file. Save it somewhere only Athena can read (e.g.,
`~/.cortex/google-chat-sa.json`, `chmod 600`).
...
Also grant `Pub/Sub Viewer` on the same subscription — Athena calls
...
- **App name**: whatever you want users to see ("Athena" is reasonable).
...
`ADDED_TO_SPACE` event that Athena uses to cache the bot's own `users/{id}` for
...
Add the Google Chat section to `~/.cortex/.env`:
...
Install the dependencies the Google Chat adapter needs (no Athena extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Athena is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Athena detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Athena session.
...
3. Download the JSON. Move it onto the host that runs Athena.
4. Register the client with Athena (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's Athena home (e.g.
`~/.cortex/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.cortex/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.cortex/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Athena isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: Athena will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.cortex/google_chat_user_tokens/<sanitized_email>.json` (filesystem

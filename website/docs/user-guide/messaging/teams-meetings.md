...
Use the Teams meeting pipeline when you want Athena to ingest Microsoft Graph meeting events, fetch transcripts first, fall back to recordings plus STT when needed, and deliver a structured summary to downstream sinks.
...
Add Graph app-only credentials to `~/.cortex/.env`:
...
Use this when you want Athena to post the summary through Microsoft Graph into a Teams chat or channel.
...
Start Athena normally after updating config:

```bash
hermes gateway run
```

Or, if you run Athena in Docker, start the gateway the same way you already do for your deployment.

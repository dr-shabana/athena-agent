...
Connect Athena to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Athena — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Athena runs it in a
...
2. **Finds or creates** the `Athena Agent` project on your account.
...
Runtime credentials are written to `~/.cortex/.env`
...
(device token, dashboard project id) lives in `~/.cortex/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Athena
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Athena replies with a pairing code. Approve it with:
...
**Pre-authorize specific numbers** (in `~/.cortex/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.cortex/.env`):
...
By default Athena responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Athena` and
`@Athena agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Athena will reply.
...
assigned iMessage line Athena uses. When a Photon token and dashboard project
...
- **Outbound attachments are supported.** Athena sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Athena wake words  | JSON list / comma / newline regex patterns for group mentions |

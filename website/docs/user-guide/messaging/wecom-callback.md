...
Connect Athena to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Athena supports two WeCom integration modes:
...
3. Athena decrypts the message, queues it for the agent
...
console. A mismatch between the token configured in Athena and the token the
...
in `~/.cortex/.env` values around `=` will also break signature checks. After

...
A **profile distribution** packages a complete Athena agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a Athena agent meant sending someone:
...
The tradeoff: recipients need git installed. On any machine running Athena in 2026, that's already true.
...
# Edit ~/.cortex/profiles/research-bot/SOUL.md
...
Create `~/.cortex/profiles/research-bot/distribution.yaml`:
...
```bash
cd ~/.cortex/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
5. Copies distribution-owned files into `~/.cortex/profiles/research-bot/` (or wherever the manifest's `name` resolves).
...
```
# Environment variables required by this Athena distribution.
...
```bash
cp ~/.cortex/profiles/research-bot/.env.EXAMPLE ~/.cortex/profiles/research-bot/.env
...
Path:    ~/.cortex/profiles/research-bot
...
```bash
# Engineering lead
cd ~/.cortex/profiles/pr-reviewer
# ... build and tune ...
...
```bash
# You
cd ~/.cortex/profiles/polymarket-trader
...
You built Athena-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
```yaml
# ~/.cortex/profiles/research-bot/local/my-overrides.yaml
...
# Iterate locally in ~/.cortex/profiles/forked-research-bot/
...
hermes profile install ~/.cortex/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
hermes profile delete research-bot-test --yes
hermes profile install ~/.cortex/profiles/research-bot --name research-bot-test
```
...
- [Using SOUL with Athena](../guides/use-soul-with-hermes.md) — authoring personalities

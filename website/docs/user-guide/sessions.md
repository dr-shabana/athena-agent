...
1. **SQLite database** (`~/.cortex/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.cortex/config.yaml`:
...
Athena automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `hermes sessions list` or `hermes sessions browse`.
...
Athena provides a full set of session management commands via `hermes sessions`:
...
|------|------|-------------|
| SQLite database | `~/.cortex/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.cortex/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.cortex/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.cortex/sessions/`. They are no longer written or
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Athena process in the same `CORTEX_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.cortex/config.yaml`:

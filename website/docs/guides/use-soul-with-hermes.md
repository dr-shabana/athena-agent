---
sidebar_position: 7
title: "Use SOUL.md with Athena"
description: "How to use SOUL.md to shape Athena Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Athena

`SOUL.md` is the **primary identity** for your Athena instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Athena to feel like the same assistant every time you talk to it — or if you want to replace the Athena persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Athena should be
- what Athena should avoid stylistically
- how Athena should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Athena is and how Athena speaks
...
Athena now uses only the global SOUL file for the current instance:

```text
~/.cortex/SOUL.md
```

If you run Athena with a custom home directory, it becomes:
...
Athena automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Athena does not overwrite it
- if the file exists but is empty, Athena adds nothing from it to the prompt

## How Athena uses it

When Athena starts a session, it reads `SOUL.md` from `CORTEX_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Athena feels.
...
Athena already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Athena should sound.

# Avoid
What Athena should not do.

# Defaults
How Athena should behave when ambiguity appears.
```
...
Then restart Athena or start a new session.
...
### I edited SOUL.md but Athena still sounds the same

Check:
- you edited `~/.cortex/SOUL.md` or `$CORTEX_HOME/SOUL.md`
...
### Athena is ignoring parts of my SOUL.md

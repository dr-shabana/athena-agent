---
sidebar_position: 6
title: "Use MCP with Athena"
description: "A practical guide to connecting MCP servers to Athena Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Athena

This guide shows how to actually use MCP with Athena Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Athena tool
- you want Athena to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Athena to internal APIs, databases, or company systems without modifying Athena core

Do not use MCP when:
- a built-in Athena tool already solves the job well
...
- Athena remains the agent
- MCP servers contribute tools
- Athena discovers those tools at startup or reload time
...
If you installed Athena with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Athena banner/status should show MCP integration when configured
- ask Athena what tools it has available
...
## WSL2: bridge Athena in WSL to Windows Chrome

This is the practical setup when:

- Athena runs inside WSL2
...
In this setup, Athena does **not** connect to Chrome directly. Instead:

- Athena runs in WSL
- Athena starts a local stdio MCP server
...
```text
Athena (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Athena stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Athena core browser transport
...
Then start a fresh Athena session or run:
...
Once loaded, Athena can use the MCP-prefixed browser tools directly. For example:
...
If Athena runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Athena from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Athena from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Athena:
...
2. Athena-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Athena will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Athena to reason over a bounded workspace.
...
Now Athena can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Athena core.
...
Because Athena now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.

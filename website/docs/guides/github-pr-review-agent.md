...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Athena Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Athena instantly when PRs are opened or updated.
:::
...
- **Athena Agent installed** — see the [Installation guide](/getting-started/installation)
...
Use `deliver: "local"` to save reviews to `~/.cortex/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Athena can access GitHub. Start a chat:
...
```
Run: gh pr list --repo dr-shabana/athena-agent --state open --limit 3
```
...
Still in the chat, ask Athena to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo dr-shabana/athena-agent
```
...
A skill gives Athena consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.cortex/skills/code-review
```

Create `~/.cortex/skills/code-review/SKILL.md`:
...
This is what makes the reviewer actually useful. Start a session and teach Athena your team's standards:
...
2. Teach Athena your conventions via memory (Step 4)

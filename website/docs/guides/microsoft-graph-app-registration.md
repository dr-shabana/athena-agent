...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.cortex/.env` at the end.
...
4. Fill in:
   - **Name:** `Athena Teams Meeting Pipeline` (or any name you'll recognize).
...
Put the three values you collected into `~/.cortex/.env`:
...
Athena ships a Graph auth smoke-test. From your Athena install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.cortex/.env` with the new value.

---
sidebar_position: 8
title: "Use Voice Mode with Athena"
description: "A practical guide to setting up and using Athena voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Athena
...
- you want Athena sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Athena.
...
## Step 1: make sure normal Athena works first
...
Athena supports both local and cloud speech stacks.
...
If you choose NeuTTS in the setup wizard, Athena checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Athena transcribes and responds
...
- asking Athena to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Athena loop.

## Tuning CLI behavior

### Silence threshold

If Athena starts/stops too aggressively, tune:
...
Athena stays a normal chat bot, but can speak replies.
...
- you want Athena to function like a portable research or ops assistant
...
Athena joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Athena detects speech boundaries
- transcripts are posted in the associated text channel
- Athena responds in text and audio
...
1. get text Athena working
2. install `athena-agent[voice]`

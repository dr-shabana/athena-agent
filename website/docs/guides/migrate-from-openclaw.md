...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Athena Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`hermes claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Athena. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.cortex/` (by default a single restore-point archive is written before apply, under `~/.cortex/backups/pre-migration-*.zip`; restorable with `hermes import`). |
...
| What | OpenClaw source | Athena destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.cortex/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.cortex/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.cortex/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.cortex/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Athena destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.cortex/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.cortex/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.cortex/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.cortex/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Athena skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Athena destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.cortex/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Athena config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Athena config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Athena field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.cortex/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Athena `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Athena path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct Athena equivalent)

These are saved to `~/.cortex/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Athena |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Athena has built-in tool instructions |
...
| Multi-agent list | `archive/agents-list.json` | Use Athena profiles |
...
Imported skills land in `~/.cortex/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

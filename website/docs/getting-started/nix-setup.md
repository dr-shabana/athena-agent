...
After `nix profile install`, `hermes`, `athena-agent`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `hermes setup` walks you through provider selection, `hermes gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.cortex/`.
...
```bash
nix profile install github:dr-shabana/athena-agent#messaging
```

For all optional extras (voice, all providers, all platforms):

```bash
nix profile install github:dr-shabana/athena-agent#full
```
...
Setting `addToSystemPackages = true` does two things: puts the `hermes` CLI on your system PATH **and** sets `CORTEX_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `hermes` in your shell creates a separate `~/.cortex/` directory.
:::
...
This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$CORTEX_HOME/config.yaml` on each activation.
...
| Give the agent a personality | `${services.athena-agent.stateDir}/.hermes/SOUL.md` | manage the file directly |
...
| Override the hermes package | `package` | `inputs.athena-agent.packages.${system}.default.override { ... }` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$CORTEX_HOME/.env` at activation time (`nixos-rebuild switch`). Athena reads this file on every startup, so changes take effect with a `systemctl restart athena-agent` — no container recreation needed.
...
The agent identity file is separate: Athena loads its primary `SOUL.md` from `$CORTEX_HOME/SOUL.md`, which in the NixOS module is `${services.athena-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
:::tip
Environment variables in `env` values are resolved from `$CORTEX_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
Tokens are stored in `$CORTEX_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.
...
2. **`.managed` marker file** in `CORTEX_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it athena-agent hermes config set ...` is also blocked)
...
Plugins are symlinked into `$CORTEX_HOME/plugins/` at activation time. Athena discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.

### Entry-Point Plugins (`extraPythonPackages`)

For pip-packaged plugins that register via `[project.entry-points."cortex_agent.plugins"]` (e.g., [rtk-hermes](https://github.com/ogallotti/rtk-hermes)):
...
| Check | What it tests |
|---|---|
| `package-contents` | `hermes` and `athena-agent` binaries exist and `hermes version` runs |
...
| `stateDir` | `str` | `"/var/lib/hermes"` | State directory (`CORTEX_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `hermes` CLI to system PATH and set `CORTEX_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$CORTEX_HOME/.env` at activation time |
...
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$CORTEX_HOME/plugins/`. Each must contain `plugin.yaml` |
...
| `hermes version` shows old version | Container not restarted | `systemctl restart athena-agent` |

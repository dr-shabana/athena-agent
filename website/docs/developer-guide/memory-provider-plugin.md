...
description: "How to build a memory provider plugin for Athena Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Athena Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          cortex_home (str): Active CORTEX_HOME path. Use for storage.
        """
...
| `save_config(values, cortex_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
the `messages` parameter; Athena will continue calling them with the legacy
signature.
...
All storage paths **must** use the `cortex_home` kwarg from `initialize()`, not hardcoded `~/.hermes`:

```python
# CORRECT — profile-scoped
from hermes_constants import get_cortex_home
data_dir = get_cortex_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.cortex/my-provider").expanduser()
```

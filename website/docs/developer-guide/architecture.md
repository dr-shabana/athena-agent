...
sidebar_position: 1
title: "Architecture"
description: "Athena Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Athena Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── hermes_constants.py       # CORTEX_HOME, profile-aware paths
...
Exposes Athena as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`hermes -p <name>`) gets its own CORTEX_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |

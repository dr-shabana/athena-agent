...
Athena Agent uses a SQLite database (`~/.cortex/state.db`) to persist session
...
```
~/.cortex/state.db (SQLite, WAL mode)
...
db = SessionDB()                           # Default: ~/.cortex/state.db
...
Default path: `~/.cortex/state.db`

This is derived from `hermes_constants.get_cortex_home()` which resolves to
`~/.cortex/` by default, or the value of `CORTEX_HOME` environment variable.

...
| `--profile <name>`, `-p <name>` | Select which Athena profile to use for this invocation. Overrides the sticky default set by `hermes profile use`. |
| `--resume <session>`, `-r <session>` | Resume a previous session by ID or title. |
| `--continue [name]`, `-c [name]` | Resume the most recent session, or the most recent session matching a title. |
| `--worktree`, `-w` | Start in an isolated git worktree for parallel-agent workflows. |
| `--yolo` | Bypass dangerous-command approval prompts. |
| `--pass-session-id` | Include the session ID in the agent's system prompt. |
| `--ignore-user-config` | Ignore `~/.cortex/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
...
| `hermes secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.cortex/.env`. |
...
| `hermes backup` | Back up Athena home directory to a zip file. |
| `hermes checkpoints` | Inspect / prune / clear `~/.cortex/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
| `hermes import` | Restore a Athena backup from a zip file. |
...
| `hermes acp` | Run Athena as an ACP server for editor integration. |
| `hermes mcp` | Manage MCP server configurations and run Athena as an MCP server. |
| `hermes plugins` | Manage Athena Agent plugins (install, enable, disable, remove). |
...
| `hermes profile` | Manage profiles — multiple isolated Athena instances. |
| `hermes completion` | Print shell completion scripts (bash/zsh/fish). |
| `hermes version` | Show version information. |
| `hermes update` | Pull latest code and reinstall dependencies (git installs), or check PyPI and `pip install --upgrade` (pip installs). `--check` previews without installing; `--backup` takes a pre-pull `CORTEX_HOME` snapshot. |
| `hermes uninstall` | Remove Athena from the system. |
...
| `--ignore-user-config` | Ignore `~/.cortex/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
...
hermes chat --safe-mode -q "Is this bug mine or Athena'?"
```
...
Per-run overrides (no mutation to `~/.cortex/config.yaml`):
...
**`hermes model`** (run from your terminal, outside any Athena session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.
...
**If you need to add a new provider:** Exit your Athena session first (`Ctrl+C` or `/quit`), then run `hermes model` from your terminal prompt.
:::
...
| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `CORTEX_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `hermes update`. |
...
hermes slack manifest --write      # write to ~/.cortex/slack-manifest.json
...
|------|---------|---------|
| `--write [PATH]` | stdout | Write to a file instead of stdout. Bare `--write` writes `$CORTEX_HOME/slack-manifest.json`. |
...
Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.cortex/.env` + `~/.cortex/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.
...
Pull API keys from an external secret manager at process startup instead of storing them in `~/.cortex/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
> Not to be confused with `hermes claw migrate` (one-shot import of OpenClaw configuration into Athena) — `hermes migrate` is the top-level config-rewrite command.
...
On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Athena venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.cortex/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
hermes auth spotify                                      # Authenticate Athena with Spotify via PKCE
```
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.cortex/kanban/current`). |
...
Board resolution order (highest precedence first): `--board <slug>` flag → `CORTEX_KANBAN_BOARD` env var → `~/.cortex/kanban/current` file → `default`.
...
Subscriptions persist to `~/.cortex/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart.
...
| **Identity** | Active profile name, CORTEX_HOME path |
...
The report includes system info (OS, Python version, Athena version), recent agent and gateway logs (512 KB limit per file), and redacted API key status. Keys are always redacted — no secrets are uploaded.
...
Inspect and manage the shadow git store at `~/.cortex/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.
...
View, tail, and filter Athena log files. All logs are stored in `~/.cortex/logs/` (or `<profile>/logs/` for non-default profiles).
...
Athena uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `hermes logs list` subcommand shows all log files including rotated ones.
...
Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.cortex/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.cortex/skill-bundles/` and report added/removed bundles |
...
| `backup` | Take a manual tar.gz snapshot of `~/.cortex/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.cortex/skills/` from a snapshot (defaults to newest) |
...
Inspect shell-script hooks declared in `~/.cortex/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.cortex/shell-hooks-allowlist.json`.
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Athena](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server).
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Athena Plugin](../guides/build-a-hermes-plugin.md).
...
Migrate your OpenClaw setup to Athena. Reads from `~/.openclaw` (or a custom path) and writes to `~/.hermes`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.cortex/` (by default a single restore-point archive is written to `~/.cortex/backups/pre-migration-*.zip` before apply; restorable with `hermes import`). |
...
**git installs:** `hermes update` pulls the configured update branch (default: `main`). If your checkout is on another branch, Athena may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
| `--backup` | Create a labeled pre-update snapshot of `CORTEX_HOME` (config, auth, sessions, skills, pairing data) before pulling. Default is **off** — the previous always-backup behavior was adding minutes to every update on large homes. Flip it on permanently via `updates.pre_update_backup: true` in `config.yaml`. |
...
- **Gateway restart.** After a successful update, Athena attempts to restart all running gateway profiles automatically so they pick up the new code. Use `hermes gateway restart` when you want to restart a gateway without applying an update.
...
- **npm lockfile churn.** Before stashing or switching branches, Athena makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `hermes update`.
- **Pairing data snapshot.** Even when `--backup` is off, `hermes update` takes a lightweight snapshot of `~/.cortex/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `hermes backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `hermes.service` warning.** If Athena detects a pre-rename `hermes.service` systemd unit (instead of the current `hermes-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| `hermes postinstall` | Internal bootstrap. Runs once after `pip install athena-agent` (or `hermes update` on pip installs) to install non-Python dependencies that pip cannot provide — Node.js runtime, headless browser, ripgrep, ffmpeg — and then trigger `hermes setup` if the profile has not been configured yet. Safe to re-run idempotently. |
| `hermes uninstall [--full] [--gui] [--yes]` | Remove Athena, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--yes` skips prompts. |

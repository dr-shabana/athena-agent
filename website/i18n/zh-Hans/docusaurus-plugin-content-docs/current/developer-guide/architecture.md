...
sidebar_position: 1
title: "架构"
description: "Athena Agent 内部结构——主要子系统、执行路径、数据流及延伸阅读指引"
---

# 架构

本页是 Athena Agent 内部结构的顶层导图。用它在代码库中定位自己，然后深入各子系统专项文档了解实现细节。
...
├── hermes_constants.py       # CORTEX_HOME、感知 profile 的路径
...
三种发现来源：`~/.cortex/plugins/`（用户级）、`.hermes/plugins/`（项目级）和 pip entry point。插件通过上下文 API 注册工具、hook 和 CLI 命令。存在两种专用插件类型：记忆提供者（`plugins/memory/`）和上下文引擎（`plugins/context_engine/`）。两者均为单选——每种同时只能激活一个，通过 `hermes plugins` 或 `config.yaml` 配置。
...
通过 stdio/JSON-RPC 将 Athena 作为编辑器原生 agent 暴露给 VS Code、Zed 和 JetBrains。
...
| **Profile 隔离** | 每个 profile（`hermes -p <name>`）拥有独立的 CORTEX_HOME、配置、记忆、会话和 gateway PID。多个 profile 可并发运行。 |

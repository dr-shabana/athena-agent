...
完成本指南需要**租户管理员权限**（或由管理员代为授予同意）。请记录收集到的值——最终需要填入 `~/.cortex/.env`。
...
4. 填写以下内容：
   - **Name：**`Athena Teams Meeting Pipeline`（或任何你能识别的名称）。
...
```powershell
# Create a policy scoped to the Athena app
New-CsApplicationAccessPolicy `
  -Identity "Athena-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Athena meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Athena-Meeting-Pipeline-Policy" `
...
Athena 内置了 Graph 身份验证冒烟测试。在 Athena 安装目录下执行：
...
2. 用新值更新 `~/.cortex/.env` 中的 `MSGRAPH_CLIENT_SECRET`。

...
description: "使用 Nix 安装和部署 Athena Agent——从快速 `nix run` 到完全声明式的 NixOS 模块（含容器模式）"
---

# Nix & NixOS 安装配置

Athena Agent 提供了一个 Nix flake，支持三个层级的集成：
...
```bash
# 直接运行（首次使用时构建，之后使用缓存）
nix run github:dr-shabana/athena-agent -- setup
nix run github:dr-shabana/athena-agent -- chat

# 或持久化安装
nix profile install github:dr-shabana/athena-agent
hermes setup
hermes chat
```

执行 `nix profile install` 后，`hermes`、`athena-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`hermes setup` 引导你完成提供商选择，`hermes gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.cortex/`。
...
# /etc/nixos/flake.nix（或你的系统 flake）
{
  inputs = {
    ...
    athena-agent.url = "github:dr-shabana/athena-agent";
  };
...
```nix
services.athena-agent.environmentFiles = [ "/var/lib/hermes/env" ];
```
:::

:::tip addToSystemPackages
设置 `addToSystemPackages = true` 有两个作用：将 `hermes` CLI 添加到系统 PATH，**并**在系统范围内设置 `CORTEX_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行 `hermes` 会创建独立的 `~/.cortex/` 目录。
:::
...
:::note 模型命名
`settings.model.default` 使用你的提供商所期望的模型标识符。使用 [OpenRouter](https://openrouter.ai)（默认）时，格式如 `"anthropic/claude-sonnet-4"` 或 `"google/gemini-3-flash"`。如果直接使用提供商（Anthropic、OpenAI），请将 `settings.model.base_url` 指向其 API，并使用其原生模型 ID（例如 `"claude-sonnet-4-20250514"`）。未设置 `base_url` 时，Athena 默认使用 OpenRouter。
:::
...
```nix
services.athena-agent.configFile = /etc/hermes/config.yaml;
```

这会完全绕过 `settings`——不合并，不生成。每次激活时，该文件会原样复制到 `$CORTEX_HOME/config.yaml`。
...
| 给 Agent 设置个性 | `${services.athena-agent.stateDir}/.hermes/SOUL.md` | 直接管理该文件 |
...
| 覆盖 hermes 包 | `package` | `inputs.athena-agent.packages.${system}.default.override { ... }` |
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$CORTEX_HOME/.env` 中。Athena 在每次启动时读取此文件，因此更改在 `systemctl restart athena-agent` 后生效——无需重建容器。
...
`documents` 选项将文件安装到 Agent 的工作目录（即 `workingDirectory`，Agent 将其作为工作区读取）。Athena 按约定查找特定文件名：

- **`USER.md`** — 关于 Agent 正在交互的用户的上下文信息。
- 你放置在此处的任何其他文件对 Agent 都可见，作为工作区文件。

Agent 身份文件是独立的：Athena 从 `$CORTEX_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.athena-agent.stateDir}/.hermes/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
...
:::tip
`env` 值中的环境变量在运行时从 `$CORTEX_HOME/.env` 解析。使用 `environmentFiles` 注入密钥——切勿将 token 直接放入 Nix 配置。
:::
...
对于使用 OAuth 2.1 的服务器，设置 `auth = "oauth"`。Athena 实现了完整的 PKCE 流程——元数据发现、动态客户端注册、token 交换和自动刷新。
...
Token 存储在 `$CORTEX_HOME/mcp-tokens/<server-name>.json` 中，在重启和重建后持久保留。

<details>
<summary><strong>无头服务器上的初始 OAuth 授权</strong></summary>

首次 OAuth 授权需要基于浏览器的同意流程。在无头部署中，Athena 将授权 URL 打印到 stdout/日志，而不是打开浏览器。
...
# 原生模式
sudo -u hermes CORTEX_HOME=/var/lib/hermes/.hermes \
  hermes mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
hermes mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.cortex/mcp-tokens/my-oauth-server{,.client}.json \
...
2. **`.managed` 标记文件**，位于 `CORTEX_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it athena-agent hermes config set ...` 也会被屏蔽）
...
|---|---|---|---|---|
| `systemctl restart athena-agent` | 否 | 保留 | 保留 | 保留 |
...
插件在激活时以符号链接方式安装到 `$CORTEX_HOME/plugins/`。Athena 通过其正常的目录扫描发现它们。从列表中移除插件并运行 `nixos-rebuild switch` 会删除符号链接。

### 入口点插件（`extraPythonPackages`）

对于通过 `[project.entry-points."cortex_agent.plugins"]` 注册的 pip 打包插件（例如 [rtk-hermes](https://github.com/ogallotti/rtk-hermes)）：
...
对于已在 athena-agent 的 `pyproject.toml` 中声明的可选 extras（例如 `hindsight` 或 `honcho` 等记忆提供商），使用 `extraDependencyGroups` 在构建时将其包含到封闭的 venv 中：
...
```nix
{
  inputs.athena-agent.url = "github:dr-shabana/athena-agent";
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `hermes` 和 `athena-agent` 二进制文件存在且 `hermes version` 可运行 |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `enable` | `bool` | `false` | 启用 athena-agent 服务 |
| `package` | `package` | `athena-agent` | 使用的 athena-agent 包 |
| `user` | `str` | `"athena"` | 系统用户 |
| `group` | `str` | `"athena"` | 系统组 |
| `createUser` | `bool` | `true` | 自动创建用户/组 |
| `stateDir` | `str` | `"/var/lib/hermes"` | 状态目录（`CORTEX_HOME` 的父目录） |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent 工作目录（`MESSAGING_CWD`） |
| `addToSystemPackages` | `bool` | `false` | 将 `hermes` CLI 添加到系统 PATH 并在系统范围内设置 `CORTEX_HOME` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | 包含密钥的 env 文件路径。激活时合并到 `$CORTEX_HOME/.env` |
...
| `extraPlugins` | `listOf package` | `[]` | 以符号链接方式安装到 `$CORTEX_HOME/plugins/` 的目录插件包。每个包必须包含 `plugin.yaml` |
...
| 容器路径 | 主机路径 | 模式 | 说明 |
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Athena 二进制文件 + 所有 Nix 依赖 |
...
# 容器模式
docker exec athena-agent cat /data/.hermes/.env
```
...
| `hermes version` 显示旧版本 | 容器未重启 | `systemctl restart athena-agent` |
...
| `no container with name or ID "athena-agent"`（Podman） | Podman rootful 容器对普通用户不可见 | 为 podman 添加免密 sudo（参见[容器模式](#container-mode)章节） |
| `unable to find user hermes` | 容器仍在启动中（入口点尚未创建用户） | 等待几秒后重试——CLI 会自动重试 |
| 通过 `extraPackages` 添加的工具在终端中找不到 | 需要 `nixos-rebuild switch` 更新每用户 profile | 重建并重启：`nixos-rebuild switch && systemctl restart athena-agent` |
...
sidebar_position: 13
title: "Webhooks"
description: "接收来自 GitHub、GitLab 等服务的事件以触发 Athena agent 运行"
---

# Webhooks

接收来自外部服务（GitHub、GitLab、JIRA、Stripe 等）的事件，并自动触发 Athena agent 运行。Webhook 适配器运行一个 HTTP 服务器，接受 POST 请求、验证 HMAC 签名、将 payload（载荷）转换为 agent prompt（提示词），并将响应路由回来源或其他已配置的平台。
...
按照上方示例，将 `github-pr` 路由添加到 `~/.cortex/config.yaml`。
...
在仓库中打开一个 pull request。webhook 触发后，Athena 处理事件并在 PR 上发布审查评论。
...
- 订阅存储在 `~/.cortex/webhook_subscriptions.json`

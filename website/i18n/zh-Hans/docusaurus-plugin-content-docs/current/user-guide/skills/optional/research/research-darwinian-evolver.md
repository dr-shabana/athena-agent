...
| 版本 | `0.1.0` |
| 作者 | Bihruze (Asahi0x), Athena Agent |
| 许可证 | MIT |
| 平台 | linux, macos |
...
调用来调用它（纯聚合方式）。**不得**将上游类导入 Athena 本身。
...
```bash
mkdir -p ~/.cortex/cache/darwinian-evolver && cd ~/.cortex/cache/darwinian-evolver
...
```bash
cd ~/.cortex/cache/darwinian-evolver/darwinian_evolver \
...
在浏览器中打开 `~/.cortex/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
并加载 JSON 日志，即可查看进化树。
...
SKILL_DIR=~/.cortex/skills/research/darwinian-evolver
DE_DIR=~/.cortex/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL 协议。** 永远不要在 Athena 核心中使用 `from darwinian_evolver import ...`。`~/.cortex/skills/...` 下的自定义驱动脚本属于用户侧，没有问题。

...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/dr-shabana/athena-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    hermes_meta = (fm.get("metadata") or {}).get("athena") or {}
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `athena skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Athena Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Athena ships with a large built-in skill library copied into `~/.cortex/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Athena also syncs bundled skills on `athena update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.cortex/skills/` tree, it is still shipped with Athena; restore it with `athena skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with athena-agent — install via athena skills install official/<category>/<skill>"',
        "---",
        "",
        ...
        "",
        "```bash",
        "athena skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "athena skills install official/blockchain/solana",
        "athena skills install official/mlops/flash-attention",
        "```",
        "",

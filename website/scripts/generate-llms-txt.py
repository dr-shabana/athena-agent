#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Athena docs site.
...
Both publish at:
  https://athena-agent.nousresearch.com/docs/llms.txt
  https://athena-agent.nousresearch.com/docs/llms-full.txt
...
SITE_BASE = "https://athena-agent.nousresearch.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Athena as a Python Library", None),
        ("guides/use-mcp-with-hermes", "Use MCP with Athena", None),
        ("guides/use-voice-mode-with-hermes", "Use Voice Mode with Athena", None),
        ("guides/use-soul-with-hermes", "Use SOUL.md with Athena", None),
        ("guides/build-a-hermes-plugin", "Build a Athena Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Athena"),
...
def emit_llms_index() -> str:
    ...
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/dr-shabana/"
        "athena-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/dr-shabana/athena-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Athena Agent — Full Documentation\n",
        (
            "This file is the entire Athena Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Athena, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://athena-agent.nousresearch.com/docs\n",
        "Short index: https://athena-agent.nousresearch.com/docs/llms.txt\n",
        "\n---\n\n",
    ]

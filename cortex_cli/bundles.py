"""Implementation of the ``athena bundles`` CLI subcommand.

Mirrors the structure of ``cortex_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `athena bundles list | grep …` doesn't
...
def register_cli(subparser) -> None:
    """Build the ``athena bundles`` argparse tree.

    Called from ``cortex_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `athena bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``athena bundles <subcommand>`` to the right handler."""

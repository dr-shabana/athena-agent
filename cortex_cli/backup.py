...
`athena backup` creates a zip archive of the entire ~/.cortex/ directory
(excluding the athena-agent repo and transient files).

`athena import` restores from a backup zip, overlaying onto the current
CORTEX_HOME root.
"""
...
from cortex_constants import get_default_hermes_root, get_cortex_home, display_cortex_home
...
# ``athena-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/athena-agent/``
# are not accidentally excluded.
_EXCLUDED_DIRS = {
    "athena-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``athena-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/athena-agent/`` — must be preserved.
        if part == "athena-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside CORTEX_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Athena home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Athena home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    # Collect files
    print(f"Scanning {display_cortex_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``athena-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "athena-agent" and not is_root)
        ]
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Athena backup.
    ...
    if not found:
        return False, (
            "zip does not appear to be a Athena backup "
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_cortex_home()}")
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Athena configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # cortex_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: athena profile list  (after installing hermes)")

        # Guidance
        print()
        if not (hermes_root / "athena-agent").is_dir():
            print("Note: The athena-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: athena update")
        ...
        print("Done. Your Athena configuration has been restored.")
...
# Critical state files to include in quick snapshots (relative to CORTEX_HOME).
...
# — `athena update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(cortex_home: Optional[Path] = None) -> Path:
...
def create_quick_snapshot(
    ...
    cortex_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(cortex_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside CORTEX_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``athena update``.
    ...
    Args:
        ...
        cortex_home: Override for the Athena home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(cortex_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_cortex_home()}/state-snapshots/")
...
def _pre_update_backup_dir(cortex_home: Optional[Path] = None) -> Path:
    home = cortex_home or get_cortex_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of CORTEX_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<CORTEX_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``athena update``) should continue even if the backup fails.
    """
    hermes_root = cortex_home or get_default_hermes_root()
    if not hermes_root.is_dir():
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `athena claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of CORTEX_HOME under ``backups/`` before a
    ``athena claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``athena import <archive>``.  Writes to
    ``<CORTEX_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `athena import` and the

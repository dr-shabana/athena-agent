...
Pure display functions with no AthenaCLI state dependency.
"""
...
from cortex_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
_UPSTREAM_REPO_URL = "https://github.com/dr-shabana/athena-agent.git"
...
def check_for_updates() -> Optional[int]:
    """Check whether a Athena update is available.
    ...
    cache_file = cortex_home / ".update_check"
    ...
    # and `athena update` correctly refuses to run in-place inside the
    ...
    if embedded_rev:
        ...
        # $CORTEX_HOME/athena-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = cortex_home / "athena-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Athena git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$CORTEX_HOME/athena-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``cortex_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from cortex_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/dr-shabana/athena-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Athena checkout. Cached per-process. Release URL always points at the
    canonical dr-shabana/athena-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Athena Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        from cortex_cli.codex_runtime_switch import get_current_runtime
        from cortex_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from cortex_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
    ...
    # Pip-install warning — `pip install athena-agent` is not the supported

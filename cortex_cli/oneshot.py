...
configured for "cli" in `athena tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (CORTEX_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `athena chat`:
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``athena -z`` / oneshot mode.

    Oneshot bypasses ``AthenaCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    from cortex_cli.models import detect_provider_for_model
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from cortex_cli import model_switch as _ms
    ...
    agent = AIAgent(
        ...
        #   - dangerous-command approval → bypassed via CORTEX_YOLO_MODE=1

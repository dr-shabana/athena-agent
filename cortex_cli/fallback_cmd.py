...
Subcommands:
  ...
  hermes fallback add      Pick provider + model via the same picker as `athena model`,
...
Storage: ``fallback_providers`` in ``~/.cortex/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from cortex_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `athena model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    print("  Docs: https://athena-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `athena model`, then append the selection to the chain."""
    from cortex_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `athena model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `athena fallback list` to view, or `athena fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``athena fallback [subcommand]``."""

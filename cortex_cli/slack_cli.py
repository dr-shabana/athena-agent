"""``athena slack ...`` CLI subcommands.

Today only ``athena slack manifest`` is implemented — it generates the
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Athena. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Athena deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from cortex_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Athena agent on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with Athena in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``cortex_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$CORTEX_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Athena")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "Athena"
    description = getattr(args, "description", None) or "Your Athena agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("CORTEX_HOME") or str(Path.home() / ".hermes")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Athena app\n"

"""Migrate Athena' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Athena' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Athena mcp_servers.<n>.command/args/env  → codex stdio transport
  Athena mcp_servers.<n>.url/headers       → codex streamable_http transport
  Athena mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Athena mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Athena-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Athena' own
...
MIGRATION_MARKER = (
    "# managed by athena-agent — `athena codex-runtime migrate` regenerates this section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Athena config.")
...
# Athena keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Athena' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one Athena MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # Athena' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in hermes_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown Athena key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (CORTEX_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Athena)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Athena' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Athena' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Athena
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a CORTEX_HOME pointing
    ...
    ``~/.cortex`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Athena' own
    ...
    CORTEX_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # CORTEX_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_cortex_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever CORTEX_HOME its launcher sets
    ...
    # — that would override the launcher's CORTEX_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("CORTEX_HOME", tmp_path) would
    ...
    cortex_home = os.environ.get("CORTEX_HOME") or ""
    if cortex_home and _looks_like_test_tempdir(cortex_home):
        cortex_home = ""
    if cortex_home:
    ...
    env["HERMES_QUIET"] = "1"
    env["CORTEX_REDACT_SECRETS"] = env.get("CORTEX_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    """Translate Athena mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hermes_config: full ~/.cortex/config.yaml dict
        ...
        expose_hermes_tools: when True (default), register Athena' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Athena for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Athena config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject Athena' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Athena for the tools codex

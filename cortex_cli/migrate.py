"""CLI handlers for ``athena migrate ...``.

Currently exposes only ``athena migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``athena migrate <subtype>``."""
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `athena migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `athena doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0

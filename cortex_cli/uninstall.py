"""
Athena Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.cortex/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Athena PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing athena-agent or hermes PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Athena Agent" comment and following line
                if '# Athena Agent' in line or '# athena-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('athena' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hermes
                if 'athena' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains cortex_cli reference)
                content = wrapper.read_text()
                if 'cortex_cli' in content or 'athena-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(cortex_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``athena`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Athena home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (cortex_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``athena gateway run`` processes
    ...
    try:
        from cortex_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from cortex_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``CORTEX_HOME`` and ``HERMES_GIT_BASH_PATH``
...
#   4. On the ``athena dashboard`` + gateway paths, drops files into
...
def _hermes_path_markers(cortex_home: Path) -> list[str]:
    """Path-entry substrings that identify Athena-owned User-PATH entries."""
    root = str(cortex_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare athena-agent install dir.
    markers = [root + "\\athena-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if CORTEX_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "athena".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-hermes as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(cortex_home: Path) -> list[str]:
    """Strip Athena-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _hermes_path_markers(cortex_home)
            kept: list[str] = []
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete CORTEX_HOME and HERMES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("CORTEX_HOME", "HERMES_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(cortex_home: Path) -> list[Path]:
...
def _is_default_cortex_home(cortex_home: Path) -> bool:
    """Return True when ``cortex_home`` points at the default (non-profile) root."""
    try:
        from cortex_constants import get_default_hermes_root
        return cortex_home.resolve() == get_default_hermes_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its CORTEX_HOME directory.

    We shell out to ``athena -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current CORTEX_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m cortex_cli.main` so we don't depend on a `athena`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "cortex_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's CORTEX_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``athena uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$CORTEX_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from cortex_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(cortex_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Athena Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Athena Chat GUI installation was found.")
        print(f"  Checked: {cortex_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Athena agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(cortex_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Athena agent at {cortex_home / 'athena-agent'}")
        print(f"  • Your config, sessions, and secrets under {cortex_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Athena agent is still installed. Run 'athena' to use the CLI,")
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.cortex/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.cortex/ for future reinstall
    """
    project_root = get_project_root()
    cortex_home = get_cortex_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie CORTEX_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_cortex_home(cortex_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.cortex data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Athena Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {cortex_home / 'config.yaml'}")
    print(f"  Secrets: {cortex_home / '.env'}")
    print(f"  Data:    {cortex_home / 'cron/'}, {cortex_home / 'sessions/'}, {cortex_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their CORTEX_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Athena data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Athena code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``athena`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$CORTEX_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in cortex_home so the marker matching is
        ...
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(cortex_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Athena-owned PATH entries in User environment")

        log_info("Removing CORTEX_HOME / HERMES_GIT_BASH_PATH User env vars...")
        removed_env = remove_hermes_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Athena-set User env vars to remove")
    ...
    #     (only when they still point into this Athena home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Athena-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(cortex_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Athena-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(cortex_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE CORTEX_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from cortex_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(cortex_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.cortex/, just remove the athena-agent subdir
            if cortex_home in project_root.parents or project_root.parent == cortex_home:
    ...
    #     under CORTEX_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(cortex_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(cortex_home)
    ...
    # 5. Optionally remove ~/.cortex/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile CORTEX_HOME dirs live under
        ...
        try:
            if cortex_home.exists():
                shutil.rmtree(cortex_home)
                log_success(f"Removed {cortex_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {cortex_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {cortex_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://athena-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://athena-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Athena Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m cortex_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import cortex_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``cortex_constants`` + ``cortex_cli.colors``
    (and lazily ``cortex_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m cortex_cli.uninstall")
    parser.add_argument(
        "--mode",

...
the existing module-level functions in cortex_cli.gateway and
cortex_cli.gateway_windows directly. This protocol is a thin facade
...
``athena gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``athena gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    detection always fails. Since every Athena runtime call inside the
...
# in ``cortex_cli.gateway`` (systemd/launchd) and ``cortex_cli.gateway_windows``
...
# active (set via the ``athena -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in cortex_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from cortex_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from cortex_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from cortex_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from cortex_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in cortex_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from cortex_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from cortex_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from cortex_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from cortex_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``cortex_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from cortex_cli import gateway_windows
        from cortex_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `athena profile create`
...
# UID/GID of the in-image ``athena`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    (the ``athena`` user — UID 10000 — running every Athena runtime
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``athena -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`athena profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources CORTEX_HOME (and any extra env) via with-contenv —
             so e.g. ``-e CORTEX_HOME=/data/hermes`` is honored at run
          ...
          4. Drops to the hermes user and exec's
             ``athena -p <profile> gateway run`` (or just ``hermes
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``athena gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root CORTEX_HOME profile" (the implicit profile that exists at
        the top of $CORTEX_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``cortex_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$CORTEX_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``CORTEX_HOME``,
        ...
        allocator in ``cortex_cli.profiles._allocate_gateway_port``).
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${CORTEX_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the CORTEX_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e CORTEX_HOME=/data/hermes``
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``athena logs`` reads and what
        ...
        return (
            ...
            f': "${{CORTEX_HOME:=/opt/data}}"\n'
            f'log_dir="$CORTEX_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``athena gateway

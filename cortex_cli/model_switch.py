...
- ``cortex_cli.providers``        -- canonical provider identity + overlays
- ``cortex_cli.model_normalize``  -- per-provider name formatting
...
_HERMES_MODEL_WARNING = (
    "Athena Agent 3 & 4 models are NOT agentic and are not designed "
    "for use with Athena Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Athena Agent 3 / Athena 4 chat families.
# The previous substring check (`"athena" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hermes-brain:qwen3-14b-ctx16k`` that just
# happen to carry "athena" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   dr-shabana/Athena-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Athena 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Athena 3/4 chat model."""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``athena config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from cortex_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'athena model' for available providers, or define it "
        ...
        from cortex_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from cortex_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from cortex_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in cortex_cli.runtime_provider.resolve_runtime_provider;
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from cortex_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from cortex_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from cortex_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # https://athena-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Athena release. Falls back to the in-repo
    ...
    # --- 1. Check Athena-mapped providers ---
    from cortex_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from cortex_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from cortex_cli.auth import _load_auth_store
        ...
        # /model picker sees the SAME list `athena model` would build, with
    ...
    # --- 2. Check Athena-only providers (nous, openai-codex, copilot, opencode-go) ---
    from cortex_cli.providers import HERMES_OVERLAYS
    from cortex_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Athena provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Athena IDs ("copilot").
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # Resolve Athena slug — e.g. "github-copilot" → "copilot"
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `athena model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from cortex_cli.auth import get_provider_auth_state as _nous_state
    ...
    # with `athena model`).
    try:
        from cortex_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Athena's canonical write key (matches
            ...
            # Athena writes ``models:`` as a dict keyed by model id
            # (see cortex_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Athena's own writer uses; a hyphen variant is accepted
            ...
            # active model. Athena's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, cortex_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`cortex_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from cortex_cli.models import fetch_openrouter_models

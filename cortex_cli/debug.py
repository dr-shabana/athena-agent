"""``athena debug`` debug tools for Athena Agent.
...
Currently supports:
    hermes debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.cortex/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.cortex/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``athena debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``athena debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Athena version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `athena debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "athena-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``athena debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.cortex/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``athena debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "athena-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----AthenaDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from cortex_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_cortex_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``athena dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``athena dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``athena debug share`` (CLI) and the
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `athena debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"\nShare these links with the Athena team for support.")
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``athena debug`` call.
    ...
    # best-effort — any failure is swallowed so ``athena debug`` stays

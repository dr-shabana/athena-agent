...
Profile-aware location: ``$CORTEX_HOME/logs/dashboard-auth.log``.
...
from ``cortex_constants`` or other cortex_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$CORTEX_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``cortex_constants.get_cortex_home`` semantics: env var wins,
    else ``~/.cortex``. A local copy avoids an import cycle with the
    middleware which lives below ``cortex_cli``.
    """
    home = os.environ.get("CORTEX_HOME") or str(Path.home() / ".hermes")

...
manual BotFather token copy-paste. Athena talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.athena-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "AthenaSetupBot"

DEFAULT_BOT_NAME = "Athena Agent"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Athena Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Athena Telegram onboarding service.")

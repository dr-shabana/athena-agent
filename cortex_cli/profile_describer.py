...
Used by ``athena profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``cortex_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Athena Agent kanban board.
...
Rules:
  ...
  - Never write "Athena Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_cortex_home())
        else:

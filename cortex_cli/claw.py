...
from cortex_cli.config import get_cortex_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Athena may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Athena gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Athena gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Athena")
...
def _cmd_migrate(args):
    """Run the OpenClaw → Athena migration."""
    ...
    print_info(f"Target:      {cortex_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.cortex/. "
    ...
    # ── Phase 2b: Pre-apply backup of the Athena home ─────────
    # Delegates to cortex_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `athena import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from cortex_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(cortex_home=cortex_home)
            if backup_archive:
            ...
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Athena home."
            )
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: athena claw cleanup")
                return
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        if skipped_keys:
            ...
            print_info("  athena config set OPENROUTER_API_KEY sk-or-v1-...")

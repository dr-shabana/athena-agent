"""
Unified self-relaunch for Athena CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``athena sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``athena`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from cortex_cli._parser import (
...
def resolve_hermes_bin() -> Optional[str]:
    ...
    Priority:
      ...
      2. ``shutil.which("athena")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m cortex_cli.main``.
    ...
    ``hermes.exe`` on PATH or the ``sys.executable -m cortex_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "cortex_cli.main"]
...
def relaunch(
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m cortex_cli.main``) or a ``.cmd`` batch file, and both
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``athena`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nAthena relaunch failed: {exc}\n"

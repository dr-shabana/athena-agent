"""CLI subcommand: ``athena send`` — pipe text from shell scripts to any
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `athena gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.cortex/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.cortex/.env`` AND bridge top-level
    ...
    startup that ``athena send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.cortex/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.cortex/config.yaml`` into
       ...
       via ``athena config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``athena send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.cortex/.env and ~/.cortex/config.yaml into os.environ so the
    ...
    # Import lazily so `athena send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Athena "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.cortex/.env + ~/.cortex/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `athena send --list telegram`.",
    )

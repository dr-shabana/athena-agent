"""CLI for the Athena Kanban board — ``athena kanban …`` subcommand.
...
from cortex_cli import kanban_swarm as ks
from cortex_cli.profiles import get_active_profile_name, get_profile_dir, seed_profile_skills
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this CORTEX_HOME and its
    ...
    Used by ``athena kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`athena kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Athena profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://athena-agent.nousresearch.com/docs/user-guide/features/kanban "
    ...
    # CORTEX_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `athena kanban boards switch <slug>` or the "
            "CORTEX_KANBAN_BOARD env var). Use `athena kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `athena gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.cortex/profiles/ and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``athena kanban …`` argparse dispatch.
    ...
    # env-var pin for the duration of this call. Using CORTEX_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `athena kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # CORTEX_HOME. Previously only `init` and `daemon` triggered
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``athena kanban boards <action>``.
    ...
    DB, so a fresh CORTEX_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `athena kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `athena kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `athena kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `athena kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # the kanban dispatcher can use them without a separate `athena profile
    ...
    if profiles:
        ...
        print("No profiles found under ~/.cortex/profiles/.")
        print("Create one with `athena -p <name> setup` before assigning tasks.")
    print()
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `athena -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from cortex_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``athena kanban show <id>``
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    if os.environ.get("CORTEX_KANBAN_TASK") != task_id:
        return None
...
def _cmd_daemon(args: argparse.Namespace) -> int:
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`athena kanban list --status ready` / "
                    f"`athena kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Athena profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from cortex_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from cortex_cli import kanban_decompose as decomp

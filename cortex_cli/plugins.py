"""
Athena Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with athena-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.cortex/plugins/<name>/``
3. **Project plugins** – ``./.hermes/plugins/<name>/`` (opt-in via
   ``HERMES_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``cortex_agent.plugins``
   entry-point group.
...
from cortex_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
...
# stderr in addition to ~/.cortex/logs/agent.log. Aimed at plugin authors
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other Athena loggers.
    """
...
ENTRY_POINTS_GROUP = "cortex_agent.plugins"
...
class PluginManifest:
    ...
    #              in ~/.cortex/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``athena plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``athena honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``athena <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`cortex_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from cortex_cli.dashboard_auth import (
    ...
    def register_slack_action_handler(
        ...
        Athena' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``athena model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: cortex_cli.main imports plugins indirectly
        from cortex_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.cortex/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to cortex_cli/. Two layouts are
        ...
        # 2. User plugins (~/.cortex/plugins/)
        user_dir = get_cortex_home() / "plugins"
        ...
        for manifest in winners.values():
            ...
            # for the same reason: every platform Athena ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `athena plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `athena plugins list`.
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``athena tools`` TUI so plugin-provided toolsets appear

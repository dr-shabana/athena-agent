"""Athena middleware contract helpers.
...
def apply_llm_request_middleware(
    ...
    **context: Any,
    ...
    provider kwargs before Athena sends them.
    """
...
def _invoke_middleware(kind: str, **kwargs: Any) -> List[Any]:
    from cortex_cli.plugins import invoke_middleware

    return invoke_middleware(kind, **middleware_payload(**kwargs))


def _has_middleware(kind: str) -> bool:
    from cortex_cli.plugins import has_middleware

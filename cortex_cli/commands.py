"""Slash command definitions and autocomplete for the Athena CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Athena config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Athena works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Athena is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.cortex/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Athena Agent to the latest version", "Info"),
    CommandDef("version", "Show Athena Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`cortex_cli.plugins.PluginContext.register_command`. They behave
...
Telegram only displays a small BotCommand menu in practice.  The full Athena
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``athena skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``athena skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``athena skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("athena", "Talk to Athena or run a subcommand", "[subcommand] [args]"))
    seen.add("athena")
...
def slack_app_manifest(request_url: str = "https://athena-agent.local/slack/commands") -> dict[str, Any]:
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from cortex_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]

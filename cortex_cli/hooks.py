...
Consent records live under ``~/.cortex/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.cortex/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``athena hooks`` — dispatches to the requested action."""
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.cortex/config.yaml.")
        print("See `athena hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `athena hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `athena hooks test` and `athena hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Athena wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `athena hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `athena hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `athena hooks doctor`.")

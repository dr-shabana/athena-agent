...
TIPS = [
    ...
    "athena config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    ...
    "Save money: athena tools disables unused tools, hermes skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "athena models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "athena skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Gateway & Messaging ---
    "Athena runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "MCP servers auto-generate toolsets at runtime — athena tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.cortex/checkpoints/ — your project's .git is never touched.",
    ...
    "Cron scripts live in ~/.cortex/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "The skills audit log at ~/.cortex/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at CORTEX_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "CORTEX_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in CORTEX_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.cortex/interrupt_debug.log with timestamps.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via athena tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.cortex/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.cortex/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Athena auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Athena config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Athena is working.',
    ...
    '/reload reloads ~/.cortex/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.cortex/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.cortex/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'athena curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "athena curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'athena curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'athena auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'hermes dashboard embeds the full Athena TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.cortex/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.cortex/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.cortex/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.cortex/skills/ so drop-in skills appear without restarting the session.',
    ...
    'athena setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Athena block commands when the tirith scanner itself errors out.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.cortex/dashboard-plugins/.',
]

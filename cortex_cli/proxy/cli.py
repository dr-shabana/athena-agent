"""CLI handlers for the ``athena proxy`` subcommand."""
...
from cortex_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def cmd_proxy_start(args: Any) -> int:
    ...
    print(
        f"Starting Athena proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Athena proxy upstream adapters\n")
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``athena proxy <subcommand>``."""

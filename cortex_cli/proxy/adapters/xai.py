...
# xAI's public API is OpenAI-compatible for the endpoints Athena commonly
# uses. The Responses endpoint is included because Athena' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Athena-managed OAuth credentials."""
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`athena auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`athena auth reset xai-oauth` or re-authenticate with "
                    "`athena auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `athena auth add xai-oauth --type oauth`."
            )

"""
Athena Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m cortex_cli.main web          # Start on http://127.0.0.1:9119
    python -m cortex_cli.main web --port 8080
"""
...
try:
    ...
    # running `athena dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``athena gateway run`` — but the
    desktop app spawns a ``athena dashboard`` backend, not a gateway, so a cron
    ...
    CORTEX_HOME — whichever process grabs the lock first wins the tick.
    """
...
app = FastAPI(title="Athena Agent", version=__version__, lifespan=_lifespan)
...
_SESSION_HEADER_NAME = "X-Athena-Session-Token"
...
#
# This list is defined in ``cortex_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from cortex_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Athena-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
async def _dashboard_auth_gate(request: Request, call_next):
    from cortex_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Athena (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``athena model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Athena provider slug — fine.
    ...
    1. Vendor-name → Athena-provider mapping: when the provider string is
       not a known Athena provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from cortex_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from cortex_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from cortex_cli.models import _AGGREGATOR_PROVIDERS
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("CORTEX_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Athena root is actually /opt/data
...
async def get_status():
    ...
    # and which providers are registered so ``athena status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from cortex_cli.dashboard_auth import list_providers as _list_providers
    ...
    if not auth_required:
        status.update({
            "cortex_home": str(get_cortex_home()),
...
async def get_system_stats():
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_cortex_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from cortex_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from cortex_cli.nous_subscription import get_nous_subscription_features
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from cortex_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.cortex/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_cortex_home() / "logs"
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``athena <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``cortex_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "cortex_cli.main", *subcommand]
...
def _spawn_gateway_restart() -> Tuple[subprocess.Popen, bool]:
    """Spawn ``athena gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``athena gateway restart`` children race each other on the
...
async def restart_gateway():
    """Kick off a ``athena gateway restart`` in the background."""
    try:
...
async def update_hermes():
    """Kick off ``athena update`` in the background."""
...
async def check_hermes_update(force: bool = False):
    """Report whether a Athena update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/hermes/update`` actually runs ``athena update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Athena version string
    ...
    try:
        ...
        if force:
            try:
                (get_cortex_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.cortex/config.yaml`` under ``tts.``.
    """
...
async def get_profiles_sessions(
    ...
    from cortex_state import SessionDB
    from cortex_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Athena surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Athena supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in cortex_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None):
    ...
    """
    try:
        from cortex_cli.inventory import build_models_payload, load_picker_context
        ...
        # onboarding picker) sees the SAME full provider universe `athena model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `athena model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from cortex_cli.auth import get_provider_auth_state
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.cortex/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from cortex_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (cortex_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from cortex_cli.nous_subscription import apply_nous_managed_defaults
                from cortex_cli.tools_config import _get_platform_tools
        ...
        # endpoint, mirroring the ``athena model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from cortex_cli.main import _auto_provider_name, _save_custom_provider
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "email": {
        ...
        "docs_url": "https://athena-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Athena to DingTalk groups (钉钉).",
    ...
    "webhook": {
        ...
        "docs_url": "https://athena-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.athena-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"AthenaDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Athena Agent").strip() or "Athena Agent"
...
def _restart_gateway_after_telegram_onboarding() -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Athena appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's CORTEX_HOME.
...
# the canonical ``athena auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.cortex/.anthropic_oauth.json`` — Athena-managed PKCE flow (what
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Athena PKCE ({_HERMES_OAUTH_FILE})",
    ...
    try:
        ...
        pass
    try:
        from cortex_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Athena even
    when they also have a separate Athena-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://athena-agent.nousresearch.com/docs/guides/xai-grok-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from cortex_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Athena-managed PKCE file
    ...
    if provider_id in {"anthropic", "claude-code"}:
        ...
        try:
            from cortex_cli.auth import clear_provider_auth
...
#          → persists to ~/.cortex/.anthropic_oauth.json AND credential pool
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Athena file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``athena auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
# the tokens exactly like `athena auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `athena auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from cortex_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from cortex_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `athena sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `athena sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_cortex_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from cortex_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_cortex_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from cortex_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, CORTEX_HOME)."""
    from cortex_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["cortex_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process CORTEX_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (cortex_cli.mcp_config), so
# servers managed here show up under `athena mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from cortex_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from cortex_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from cortex_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from cortex_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # CORTEX_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds CORTEX_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (cortex_cli.webhook); the webhook
...
async def list_webhooks():
    import cortex_cli.webhook as wh
...
# spawn the real `athena gateway <verb>` so behaviour matches the CLI exactly.
...
async def list_credential_pool():
    ...
    from cortex_cli.auth import read_credential_pool
...
# `athena memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `athena memory setup` to configure a new one.",
            )
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `athena import`. The spawned action runs with
...
async def list_hooks():
    ...
    form can offer them.
    """
    from cortex_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from cortex_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <cortex_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_cortex_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``athena`` subprocess, and
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from cortex_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`athena -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the CORTEX_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_cortex_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "athena setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local CORTEX_HOME override so the write lands in the target
    ...
    """
    from cortex_constants import set_cortex_home_override, reset_cortex_home_override

    token = set_cortex_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
        ...
        reset_cortex_home_override(token)
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local CORTEX_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the CORTEX_HOME override. Returns the
    number of skills newly disabled.
    """
    from cortex_constants import set_cortex_home_override, reset_cortex_home_override
    from cortex_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's CORTEX_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``athena profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from CORTEX_HOME).
    """
    from cortex_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``athena profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the CORTEX_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``athena profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from cortex_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_cortex_home()`` at call
       time — the context-local override from ``set_cortex_home_override``
    ...
    retargeted to the *current* ``get_cortex_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a CORTEX_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
    ...
    from cortex_constants import (
        get_cortex_home,
        set_cortex_home_override,
        reset_cortex_home_override,
    )
...
async def get_skills(profile: Optional[str] = None):
    ...
    from cortex_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def get_toolsets(profile: Optional[str] = None):
    from cortex_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``athena tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``athena tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``athena tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.cortex/.env`` via ``save_env_value`` —
    the same store ``athena tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``athena tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``athena -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same CORTEX_HOME the rest of the
...
#
# The endpoint spawns the same ``athena --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from cortex_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from cortex_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from cortex_cli.dashboard_auth.audit import AuditEvent, audit_log
        from cortex_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``athena --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``HERMES_TUI_RESUME`` env var —
    matching what ``cortex_cli.main._launch_tui`` does for the CLI path.
    ...
    ``CORTEX_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_cortex_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``athena -p <name>`` performs. The in-process
    ...
    """
    from cortex_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    if profile_dir is not None:
        env["CORTEX_HOME"] = str(profile_dir)
    ...
    # inherits the profile CORTEX_HOME set above.
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from cortex_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Athena inside WSL2 to use the dashboard's /chat "
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`cortex_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from cortex_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    # (the MC Pages app), not the Athena backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Athena Teal",         "description": "Classic dark teal — the canonical Athena look"},
    {"name": "default-large", "label": "Athena Teal (Large)", "description": "Athena Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.cortex/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.cortex/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_cortex_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.cortex/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as cortex_cli.plugins):
    1. User plugins:    ~/.cortex/plugins/<name>/dashboard/manifest.json
    ...
    from cortex_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_cortex_home() / "plugins", "user"),
    ...
    # ``cortex_cli/plugins.py`` and the documented user contract.
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from cortex_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_cortex_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from cortex_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from cortex_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from cortex_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from cortex_cli.plugins_cmd import dashboard_remove_user_plugin
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.cortex/plugins/ if you trust it)",
...
from cortex_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from cortex_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing HERMES_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Athena Agent exposes a
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"  Athena Web UI → http://{host}:{actual_port}")

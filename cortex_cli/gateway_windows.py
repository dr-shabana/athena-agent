...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m cortex_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Athena_Gateway"
_TASK_DESCRIPTION = "Athena Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Athena profile."""
    from cortex_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "cortex_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    Default profile: ``Athena_Gateway``
    Named profile X: ``Athena_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during cortex_cli boot.
    from cortex_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    (or ``<CORTEX_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Athena installs stay self-contained).
    """
    _assert_windows()
    from cortex_cli.config import get_cortex_home

    script_dir = Path(get_cortex_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``CORTEX_HOME`` whenever it
    ...
    source checkout only if ``CORTEX_HOME`` cannot be resolved yet.
    """
    from cortex_cli.config import get_cortex_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports CORTEX_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m cortex_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "CORTEX_HOME={cortex_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "HERMES_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports cortex_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "cortex_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    cortex_home = str(Path(get_cortex_home()).resolve())
    profile_arg = _profile_arg(cortex_home)

    content = _build_gateway_cmd_script(python_path, working_dir, cortex_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Athena builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "cortex_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m cortex_cli.main gateway run``
    ...
    log_dir = Path(get_cortex_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from cortex_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `athena -p <profile> gateway install` must be safe.
    ...
    from cortex_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Athena gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: athena gateway status")
                return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from cortex_cli.config import get_cortex_home
        print(f"    type {Path(get_cortex_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_cortex_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from cortex_cli.config import get_cortex_home

    cortex_home = Path(get_cortex_home()).resolve()
    print()
    print("Next steps:")
    print("  hermes gateway status                      # Check status")
    print(f"  type {cortex_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Athena gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: athena gateway status")
                    return
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_cortex_home()).resolve()
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: athena gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: athena gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from cortex_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid

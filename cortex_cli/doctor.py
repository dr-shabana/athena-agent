...
Diagnoses issues with Athena Agent setup.
"""
...
from cortex_cli.config import get_project_root, get_cortex_home, get_env_path
...
_DHH = display_cortex_home()  # user-facing display path (e.g. ~/.cortex or ~/.cortex/profiles/coder)

# Load environment variables from ~/.cortex/.env so API key checks work
_env_path = get_env_path()
load_hermes_dotenv(cortex_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from cortex_cli.colors import Colors, color
from cortex_cli.models import _HERMES_USER_AGENT
from cortex_constants import OPENROUTER_MODELS_URL
...
from cortex_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.cortex/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("CORTEX_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from cortex_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from cortex_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches cortex_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``athena --version`` reporting a stale version while
    ...
    """
    try:
        from cortex_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != cortex_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'hermes update', or set "
            "cortex_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `athena profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from cortex_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `athena`.
    os.environ.setdefault("HERMES_INTERACTIVE", "1")

    # Handle `athena doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.cortex/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Athena Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`athena doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and cortex_cli/__init__.py versions
    ...
    # Check ~/.cortex/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'athena setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'athena setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'athena setup' to create one")
                issues.append("Run 'athena setup' to create .env")
    
    # Check ~/.cortex/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from cortex_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from cortex_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'athena config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from cortex_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.cortex/.env or run 'athena setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'athena setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'athena config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from cortex_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from cortex_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'athena setup' to migrate config")
                else:
                    issues.append("Run 'hermes doctor --fix' or 'athena setup' to migrate config")
            else:
        ...
        try:
            import yaml
            from cortex_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            if drift:
                check_warn(
                    ...
                    "(stale ghost from an earlier `athena setup` run)",
                )
                if should_fix:
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Athena' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = cortex_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Athena a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Athena Agent Persona\n\n"
                "<!-- Edit this file to customize how Athena communicates. -->\n\n"
    ...
    memories_dir = cortex_home / "memories"
    ...
    state_db_path = cortex_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from cortex_state import is_malformed_db_error, repair_state_db_schema
    ...
    wal_path = cortex_home / "state.db-wal"
    if wal_path.exists():
        try:
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "athena"
        ...
        _cmd_link = _cmd_link_dir / "athena"
    ...
    try:
        from cortex_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `athena doctor` invocation.
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Athena one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'athena config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``athena doctor``.
    ...
    # never the legitimate source for `athena doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'athena setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = CORTEX_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: athena skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = CORTEX_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: athena memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: athena memory setup",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: athena memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: athena memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from cortex_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re

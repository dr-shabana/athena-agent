...
Shows the status of all Athena Agent components.
"""
...
from cortex_cli.auth import AuthError, resolve_provider
from cortex_cli.colors import Colors, color
from cortex_cli.config import get_env_path, get_env_value, get_cortex_home, load_config
from cortex_cli.models import provider_label
from cortex_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from cortex_cli.nous_subscription import get_nous_subscription_features
from cortex_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``athena config``'s
...
def show_status(args):
    """Show status of all Athena Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Athena Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: athena portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: athena model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: athena auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: athena auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: athena model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from cortex_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from cortex_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_cortex_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_cortex_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'athena setup' to configure", Colors.DIM))
    print()

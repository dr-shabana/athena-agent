...
Config files are stored in ~/.cortex/ for easy access:
- ~/.cortex/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.cortex/.env         - API keys and secrets

This module provides:
- hermes config          - Show current configuration
- hermes config edit     - Open config in editor
- athena config set      - Set a specific value
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``athena config set`` (which rewrites
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.cortex/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``athena logs``
...
#   the next ``subprocess.run([...])`` Athena makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Athena itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``athena update``, the TUI build.
...
#   on every plugin install / ``athena update``.
...
# * ``CORTEX_HOME`` / ``HERMES_PROFILE`` / ``HERMES_CONFIG`` /
#   ``HERMES_ENV`` — Athena runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Athena runtime location — never via dashboard env writer.
    # NOT a HERMES_* blanket: integration credentials (HERMES_GEMINI_*,
    # HERMES_LANGFUSE_*, HERMES_SPOTIFY_*, ...) ARE allowed.
    "CORTEX_HOME", "HERMES_PROFILE", "HERMES_CONFIG", "HERMES_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Athena runtime location "
            "(CORTEX_HOME, HERMES_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `athena plugins enable
    # observability/langfuse` or `athena tools → Langfuse`); credentials gate
...
from cortex_cli.colors import Colors, color
from cortex_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_cortex_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Athena is running in package-manager-managed mode.

    Two signals: the HERMES_MANAGED env var (set by the systemd service),
    or a .managed marker file in CORTEX_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade athena-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Athena was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.cortex/.install_method`` file (written by installers)
    ...
    container detection here:
      ...
      - the published ``nousresearch/athena-agent`` image stamps ``docker``
    ...
    so ``athena update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_cortex_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.cortex/.install_method."""
    stamp = get_cortex_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Athena lives in a ``uv tool`` layout.

    ``uv tool install athena-agent`` places the install at
    ``.../uv/tools/athena-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``athena-agent`` happens to be uv-tool-installed on the machine while
    the *active* Athena is a regular pip/venv install, causing
    ``athena update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/athena-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade athena-agent"
    if method == "docker":
        return "docker pull nousresearch/athena-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade athena-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade athena-agent"
        return "pip install --upgrade athena-agent"
...
# Long-form text for ``athena update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# cortex_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Athena, it doesn't update
...
✗ ``athena update`` doesn't apply inside the Docker container.

Athena Agent runs as a published image (nousresearch/athena-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/athena-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate athena-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/athena-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/athena-agent/tags
  • Your config and session history live under ``$CORTEX_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``athena update`` inside Docker.
...
def format_managed_message(action: str = "modify this Athena installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Athena installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.athena-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Athena installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade athena-agent"
        )

    return (
        f"Cannot {action}: this Athena installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Athena."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CORTEX_HOME/.container-mode.
    ...
    from cortex_constants import is_container
    if is_container():
        return None

    container_mode_file = get_cortex_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "athena-agent")
    exec_user = info.get("exec_user", "athena")
...
# Re-export from cortex_constants — canonical definition lives there.
from cortex_constants import get_cortex_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_cortex_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_cortex_home() / ".env"
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Athena commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level CORTEX_HOME once, but
    subdirectories created at runtime by ``ensure_cortex_home()`` (especially
...
def _chown_to_hermes_uid(path) -> None:
    ...
    directories created by :func:`ensure_cortex_home` on Docker deployments.
    See #34107.
    """
...
def _secure_dir(path):
    ...
    The mode can be overridden via the CORTEX_HOME_MODE environment variable
    (e.g. CORTEX_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse CORTEX_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("CORTEX_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Athena runs in a container with volume-mounted config files, forcing
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CORTEX_HOME if the user doesn't have one yet."""
...
def ensure_cortex_home():
    """Ensure ~/.cortex directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_cortex_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_cortex_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"CORTEX_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Athena-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Athena
        ...
        # app, ACP) in a code workspace, Athena adds a coding operating brief
    ...
    "terminal": {
        ...
        # skipped. When empty, Athena auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Athena sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Athena
        ...
        # bundled Athena image, which drops to the `athena` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Athena sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Athena should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``athena chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.cortex/checkpoints/`` size (MB).  When
    ...
    # single read_file page exceeds these limits, Athena truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``athena kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``athena kanban decompose`` and the kanban
        ...
        # ``athena profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `athena model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `athena` (and `athena chat`) launches by default:
        ...
        "interface": "cli",
        # When true, `athena --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `athena -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERMES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `athena --tui` drops a one-time hint
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Athena appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.cortex/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.cortex/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Athena feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Athena auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `athena curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `athena update`, so pruning them only
        ...
        # skipped), snapshot ~/.cortex/skills/ into
        # ~/.cortex/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `athena curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Athena Agent*" header.
    ...
    # stored approval from ~/.cortex/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `athena doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``cortex_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Athena to lazy-install opt-in backend packages from PyPI
    ...
    # todos to ready, and fires `athena -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `athena` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `athena kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     athena-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Athena tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.cortex/logs/.
    ...
    # update model picker lists without shipping a athena-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://athena-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `athena model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.cortex/.env,
        ...
        # behavior: files must live under the Athena cache, under
        ...
        # uploaded as native gateway attachments. Files inside the Athena
        # cache (~/.cortex/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.cortex/state.db.
    ...
    "sessions": {
        ...
        # default of ``athena sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.cortex/sessions/session_{sid}.json`` on every turn
    ...
    # ``athena update`` behaviour.
    "updates": {
        # Run a full ``athena backup``-style zip of CORTEX_HOME before every
        # ``athena update``.  Backups land in ``<CORTEX_HOME>/backups/`` and
        # can be restored with ``athena import <path>``.  Off by default —
        # on large CORTEX_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `athena update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<CORTEX_HOME>/lsp/bin/`` on first use.
    ...
    # `athena tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.cortex/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.cortex/.env (or your shell) and never in config.yaml.
            ...
            # ~/.cortex/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'athena model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Athena auth store)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'athena-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Athena API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Athena API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Athena API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `athena update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Athena versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``cortex_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'athena setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Athena won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("CORTEX_HOME", "~/.cortex")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$CORTEX_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_cortex_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `athena plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `athena curator status` has no
    ...
    #   3. Creates `~/.cortex/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_cortex_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_cortex_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: athena config set <key> <value>")
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _cortex_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.cortex/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    ...
    (which change ``CORTEX_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.cortex/.env.
    ...
    interactive menu render (`athena tools`, `athena setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `athena tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Athena env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.cortex/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.cortex/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.cortex/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.cortex/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Athena — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Athena vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.cortex/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Athena Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from cortex_cli.auth import get_anthropic_key
    ...
    print(color("  athena config set <key> <value>", Colors.DIM))
    print(color("  athena setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: athena config set <key> <value>")
            print()
            print("Examples:")
            print("  athena config set model anthropic/claude-sonnet-4")
            print("  athena config set terminal.backend docker")
            print("  athena config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        print("  athena config set <key> <value>   Set a config value")
...
# ``_inject_profile_env_vars`` surfaces them in ``athena config`` UI so users

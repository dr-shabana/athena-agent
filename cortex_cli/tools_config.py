"""
Unified tool configuration for Athena Agent.

`athena tools` and `athena setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.cortex/config.yaml under
...
from cortex_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from cortex_cli.nous_account import format_nous_portal_entitlement_message
...
from cortex_cli.cli_output import (  # noqa: E402 — late import block
...
# who want it opt in via `athena tools` → Video Generation, which walks
...
# `athena tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from cortex_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `athena tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``athena tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from cortex_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``athena tools`` checklist actually offers
    for ``platform``.
    ...
    so ``athena tools`` never claims to add or remove a toolset the user was
...
from cortex_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Athena routes X searches through xAI's built-in x_search "
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``athena update`` and
      by ``athena computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `athena update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Athena process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_cortex_home()}/athena-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in athena-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/athena-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.cortex/config.yaml")
        ...
        # Run the full `athena auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.cortex/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from cortex_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            ...
            # toolset enable; they can retry with `athena auth spotify`.
        ...
        try:
            from cortex_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
        ...
        _print_info("    Restart Athena for tracing to take effect.")
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `athena auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``athena tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``athena tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: athena tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # through ``athena tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `athena tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `athena tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `athena tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by athena tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `athena tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `athena tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `athena model`).
    if managed_feature:
        from cortex_cli.nous_subscription import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `athena tools` and `athena setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Athena Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://athena-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from cortex_constants import display_cortex_home
    print(color(f"  Tool configuration saved to {display_cortex_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'athena' or gateway restart.", Colors.DIM))
    print()

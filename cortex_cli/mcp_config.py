"""
MCP Server Management CLI — ``athena mcp`` subcommand.

Implements ``athena mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.cortex/config.yaml under the ``mcp_servers`` key.
"""
...
from cortex_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_cortex_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from cortex_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.cortex/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``athena mcp login`` to distinguish a genuine authentication
    ...
    try:
        ...
        return AthenaTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # cortex_cli/main.py for why the dest is renamed.
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_cortex_home()}/.env as {env_key}")
    ...
    print()
    _success(f"Saved '{name}' to {display_cortex_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `athena mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `athena mcp remove` + `athena mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `athena mcp login " + name + "`.")
            return
        if tools:
...
def mcp_command(args):
    """Main dispatcher for ``athena mcp`` subcommands."""
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `athena plugin`.
        from cortex_cli.mcp_picker import run_picker
        run_picker()

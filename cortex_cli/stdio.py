...
on Japanese locales, etc.).  Athena's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``cortex_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Athena is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Athena-managed Git install directories so
    ...
    # install without Athena-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Athena's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Athena's
    ``athena config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Athena (User env var in Windows Settings, or
    export in a PowerShell profile) and Athena picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Athena-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``athena`` from right after install) retain their old PATH.

    Any subprocess Athena spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Athena-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "athena", "git", "cmd"),
        os.path.join(local_appdata, "athena", "git", "bin"),
        os.path.join(local_appdata, "athena", "git", "usr", "bin"),
        # Athena venv Scripts directory — host of the hermes.exe shim itself,
        ...
        os.path.join(local_appdata, "athena", "athena-agent", "venv", "Scripts"),

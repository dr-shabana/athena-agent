...
Outputs a compact, plain-text summary of the user's Athena setup
...
from cortex_cli.config import get_cortex_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    ``HERMES_GIT_SHA`` build-arg (see ``cortex_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``athena dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from cortex_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(cortex_home: Path) -> int:
...
def _cron_summary(cortex_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = cortex_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"cortex_home:      {display_cortex_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `athena auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `athena auth list` shows it (#42130).
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(cortex_home)}")
    lines.append(f"  skills:             {_count_skills(cortex_home)}")

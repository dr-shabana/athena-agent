"""
Profile management for multiple isolated Athena instances.

Each profile is a fully independent CORTEX_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.cortex/profiles/<name>/`` by default.

The "default" profile is ``~/.cortex`` itself — backward compatible,
...
_PROFILE_DIRS = [
    ...
    # See cortex_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.cortex``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   athena-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `athena backup` archives
...
# Marker file written by `athena profile create --no-skills`.  When present in
...
# `athena skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Athena infrastructure directories that only the default profile
         (``~/.cortex``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_cortex_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.cortex) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "athena-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "athena", "default", "test", "tmp", "root", "sudo",
})

# Athena subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hermes root, NOT to the current CORTEX_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where CORTEX_HOME points outside
    ``~/.cortex``, profiles live under ``CORTEX_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_cortex_home() / "profiles"


def _get_default_cortex_home() -> Path:
    """Return the default (pre-profile) CORTEX_HOME path.

    In standard deployments this is ``~/.cortex``.
    In Docker/custom deployments where CORTEX_HOME is outside ``~/.cortex``
    (e.g. ``/opt/data``), returns CORTEX_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_cortex_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Athena installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its CORTEX_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_cortex_home()
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``athena -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``athena profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``athena profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Athena config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_cortex_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``athena update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.cortex)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.cortex/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `athena -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Athena profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `athena update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`athena profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `athena update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `athena profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `athena -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches CORTEX_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``athena profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "CORTEX_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_cortex_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.cortex).\n"
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``CORTEX_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from cortex_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set CORTEX_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("CORTEX_HOME")
    try:
        os.environ["CORTEX_HOME"] = str(profile_dir)
        from cortex_cli.gateway import get_service_name, get_launchd_plist_path
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.cortex/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    """Infer the current profile name from CORTEX_HOME.

    Returns ``"default"`` if CORTEX_HOME is not set or points to ``~/.cortex``.
    Returns the profile name if CORTEX_HOME points into ``~/.cortex/profiles/<name>``.
    Returns ``"custom"`` if CORTEX_HOME is set to an unrecognized path.
    """
    from cortex_constants import get_cortex_home
    cortex_home = get_cortex_home()
    resolved = cortex_home.resolve()

    default_resolved = _get_default_cortex_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.cortex itself — its parent is ~/ and its
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.cortex itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.cortex). "
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_cortex_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a CORTEX_HOME path string.

    Called early in the CLI entry point, before any hermes modules
    are imported, to set the CORTEX_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)

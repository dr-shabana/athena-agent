...
# display_cortex_home is imported lazily at call sites to avoid ImportError
# when cortex_constants is cached from a pre-update version during `athena update`.
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``athena gateway status`` from falsely counting the ``athena`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``athena gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "cortex_cli.main gateway",
        "cortex_cli.main --profile",
        "cortex_cli.main -p",
        "cortex_cli/main.py gateway",
        "cortex_cli/main.py --profile",
        "cortex_cli/main.py -p",
        "hermes gateway",
    ...
    current_home = str(get_cortex_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"cortex_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. CORTEX_HOME
        ...
        # CORTEX_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "cortex_home=" in command_lc
            and f"cortex_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``athena update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Athena profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Athena profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from cortex_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "cortex_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``athena update`` users
    # with no running gateway until they re-invoke ``athena gateway``
    ...
    from cortex_cli._subprocess_compat import (
...
def _sync_cortex_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``CORTEX_HOME``.

    Under ``sudo``, ``CORTEX_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_cortex_home` falls back to ``/root/.hermes`` — the wrong
    profile. The unit file pins ``CORTEX_HOME`` for the actual gateway
    ...
    unit_home = env.get("CORTEX_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("CORTEX_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["CORTEX_HOME"] = unit_home
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from cortex_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `athena gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``athena gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from cortex_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (CORTEX_HOME-scoped).
...
from cortex_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Athena Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``athena gateway run`` must remain interruptible from
...
SERVICE_DESCRIPTION = "Athena Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current CORTEX_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (CORTEX_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from cortex_constants import get_default_hermes_root

    home = get_cortex_home().resolve()
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary CORTEX_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(cortex_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when CORTEX_HOME is a named profile.

    For ``~/.cortex/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        cortex_home: Optional explicit CORTEX_HOME path. Defaults to the current
            ``get_cortex_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from cortex_constants import get_default_hermes_root

    home = Path(cortex_home or str(get_cortex_home())).resolve()
    default = get_default_hermes_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this CORTEX_HOME.

    Default ``~/.cortex`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.cortex/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other CORTEX_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Athena installs that predate the
...
# "athena" units are never matched.
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Athena gateway units.

    Detects unit files installed by older Athena versions that used a
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Athena gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Athena gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Athena gateway unit(s) detected from an older install:")
...
def remove_legacy_hermes_units(
    ...
    """Stop, disable, and remove legacy Athena gateway unit files.
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Athena gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Athena gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Athena can't create it from this user session."
            )
            if run_as_user:
                print_info(
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Athena often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.cortex`` → ``ai.hermes.gateway.plist`` (backward compatible).
    Profile ``~/.cortex/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.hermes/athena-agent  -> /home/alice/.hermes/athena-agent
...
def _cortex_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current CORTEX_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_cortex_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hermes                    → /home/alice/.hermes
      /root/.hermes/profiles/coder     → /home/alice/.hermes/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes = get_cortex_home().resolve()
    current_default = (Path.home() / ".hermes").resolve()
    target_default = Path(target_home_dir) / ".hermes"

    # Default ~/.cortex → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.cortex → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.cortex) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    cortex_home = get_cortex_home()
    hermes_node = cortex_home / "node" / "bin"
    if _is_dir(hermes_node):
        candidates.append(str(hermes_node))
    hermes_nm = cortex_home / "node_modules" / ".bin"
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m cortex_cli.main``, so module
    ...
    dir, or a clone that ``athena update`` later relocates/removes — the path
    ...
    ``CORTEX_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if CORTEX_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        cortex_home = _cortex_home_for_target_user(home_dir)
        profile_arg = _profile_arg(cortex_home)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's CORTEX_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(cortex_home) if cortex_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m cortex_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
"""
    ...
    cortex_home = str(get_cortex_home().resolve())
    profile_arg = _profile_arg(cortex_home)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir CORTEX_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved CORTEX_HOME
    ...
    exported a throwaway ``CORTEX_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``athena gateway restart``
    with ``CORTEX_HOME=/tmp/hermes-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="CORTEX_HOME=..."`` lines and launchd
    ``<key>CORTEX_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'CORTEX_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>CORTEX_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp CORTEX_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: CORTEX_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported CORTEX_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only CORTEX_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp CORTEX_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="CORTEX_HOME=..."``. Writing that to the developer's
    ...
    # CORTEX_HOME into the unit (manual E2E homes like /tmp/hermes-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Athena install"
    )
    return True
...
def systemd_stop(system: bool = False):
    ...
    _sync_cortex_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `athena gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `athena gateway status` or logs for final state."
            )
            return
...
# process invocation (each ``athena gateway start/stop/status`` call).
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m cortex_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active CORTEX_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "cortex_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    """
    from cortex_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_cortex_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from cortex_constants import display_cortex_home as _dhh
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Athena install"
    )
    return True
...
def launchd_stop():
    ...
    # `athena gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Athena install")
    else:
        print("⚠ Service definition is stale relative to the current Athena install")
        print("  Run: athena gateway start")
    ...
    if deep:
        log_file = get_cortex_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Athena gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'athena' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Athena command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$CORTEX_HOME and break later non-root dashboard/gateway runs."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `athena gateway run` still
    ...
    # /restart) rather than through `athena gateway restart` which already
    ...
    # `athena gateway start/restart` — leaving the gateway vulnerable to
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Athena will log in directly",
    ...
    },
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Athena delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Athena agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Athena will use (e.g., hermes@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Athena connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Athena will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``athena setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.cortex/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_cortex_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from cortex_cli.telegram_managed_bot import (
...
def _setup_whatsapp():
    ...
    from cortex_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from cortex_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from cortex_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Athena will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Athena will store the returned account_id/token in ~/.cortex/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `athena gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_cortex_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `athena pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `athena pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "AthenaAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``cortex_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from cortex_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.cortex/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.cortex/.env: {', '.join(required)}")
    else:
        print_info(
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`cortex_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from cortex_cli.service_manager import (
    ...
    if profile is None:
        ...
        # CORTEX_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``athena gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``athena gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    # `docker logs` and ${CORTEX_HOME}/logs/gateways/<profile>/current,
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print(
                "  nohup hermes gateway run > ~/.cortex/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
        ...
        # slot — start them individually via `athena -p <name> gateway
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `athena gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `athena gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup hermes gateway run > ~/.cortex/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'hermes gateway run'         # persistent via tmux"
        ...
        # Stop, disable, and remove legacy Athena gateway unit files from

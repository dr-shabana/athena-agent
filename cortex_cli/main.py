...
Usage:
    ...
    athena setup               # Interactive setup wizard
    ...
    hermes honcho migrate                  # Step-by-step migration guide: OpenClaw native → Athena + Honcho
    hermes version             Show version
    hermes update              Update to latest version
    hermes uninstall           Uninstall Athena Agent
...
# IMPORTANT: cortex_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``cortex_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``athena update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``cortex_bootstrap`` but the editable install's
...
# ``athena update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import cortex_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'athena' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``athena tools`` or
    ...
    try:
        ...
        setproctitle.setproctitle("athena")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"athena", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"athena")
...
# that run *before* cortex_cli.config is importable. Mirrors the explicit
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`athena --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from cortex_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Athena Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``athena --version`` before config/logging imports on Termux."""
...
from cortex_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from cortex_cli.subcommands.cron import build_cron_parser
from cortex_cli.subcommands.gateway import build_gateway_parser
from cortex_cli.subcommands.profile import build_profile_parser
from cortex_cli.subcommands.model import build_model_parser
from cortex_cli.subcommands.setup import build_setup_parser
from cortex_cli.subcommands.postinstall import build_postinstall_parser
from cortex_cli.subcommands.whatsapp import build_whatsapp_parser
from cortex_cli.subcommands.slack import build_slack_parser
from cortex_cli.subcommands.login import build_login_parser
from cortex_cli.subcommands.logout import build_logout_parser
from cortex_cli.subcommands.auth import build_auth_parser
from cortex_cli.subcommands.status import build_status_parser
from cortex_cli.subcommands.webhook import build_webhook_parser
from cortex_cli.subcommands.hooks import build_hooks_parser
from cortex_cli.subcommands.doctor import build_doctor_parser
from cortex_cli.subcommands.security import build_security_parser
from cortex_cli.subcommands.dump import build_dump_parser
from cortex_cli.subcommands.debug import build_debug_parser
from cortex_cli.subcommands.backup import build_backup_parser
from cortex_cli.subcommands.import_cmd import build_import_cmd_parser
from cortex_cli.subcommands.config import build_config_parser
from cortex_cli.subcommands.version import build_version_parser
from cortex_cli.subcommands.update import build_update_parser
from cortex_cli.subcommands.uninstall import build_uninstall_parser
from cortex_cli.subcommands.dashboard import build_dashboard_parser
from cortex_cli.subcommands.gui import build_gui_parser
from cortex_cli.subcommands.logs import build_logs_parser
from cortex_cli.subcommands.prompt_size import build_prompt_size_parser
from cortex_cli.subcommands.memory import build_memory_parser
from cortex_cli.subcommands.acp import build_acp_parser
from cortex_cli.subcommands.tools import build_tools_parser
from cortex_cli.subcommands.insights import build_insights_parser
from cortex_cli.subcommands.skills import build_skills_parser
from cortex_cli.subcommands.pairing import build_pairing_parser
from cortex_cli.subcommands.plugins import build_plugins_parser
from cortex_cli.subcommands.mcp import build_mcp_parser
from cortex_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (athena tools, athena setup, athena model) use
...
#
# Many modules cache CORTEX_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("CORTEX_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.cortex/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set CORTEX_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `athena mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Athena' own profile selector.
        """
        try:
    ...
    # after the subcommand (`athena chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors cortex_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If CORTEX_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.cortex/profiles/coder or
    # /opt/data/profiles/coder).  If CORTEX_HOME points to the hermes root
    # instead (e.g. systemd hardcodes CORTEX_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `athena profile use` and the gateway should honour that choice.
    # See issue #22502.
    cortex_home_env = os.environ.get("CORTEX_HOME", "")
    if profile_name is None and cortex_home_env:
        if Path(cortex_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the hermes root
    if profile_name is None:
        try:
            from cortex_constants import get_default_hermes_root
    ...
    # 3. If we found a profile, resolve and set CORTEX_HOME
    if profile_name is not None:
        try:
            from cortex_cli.profiles import resolve_profile_env

            cortex_home = resolve_profile_env(profile_name)
            ...
            return
        os.environ["CORTEX_HOME"] = cortex_home
...
# Load .env from ~/.cortex/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from cortex_cli.config import get_cortex_home
from cortex_cli.env_loader import load_hermes_dotenv

load_hermes_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → CORTEX_REDACT_SECRETS env
# var BEFORE cortex_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_cortex_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "CORTEX_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["CORTEX_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `athena` subcommands
...
try:
    from cortex_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from cortex_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if cortex_constants not importable yet
...
from cortex_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to cortex_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (cortex_cli.main._model_flow_*) keep resolving unchanged.
from cortex_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_cortex_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    from cortex_cli.config import get_env_path, get_cortex_home, load_config
    from cortex_cli.auth import get_auth_status

    # Determine whether Athena itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from cortex_cli.config import DEFAULT_CONFIG
    ...
    from cortex_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_cortex_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Athena has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Athena to use their tokens.
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from cortex_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'athena')
    """
...
def _ensure_tui_node() -> None:
    ...
    cortex_home = os.environ.get("CORTEX_HOME") or str(Path.home() / ".hermes")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "CORTEX_HOME": cortex_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(cortex_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from cortex_cli.dep_ensure import ensure_dependency
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from cortex_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from cortex_cli.config import apply_terminal_config_to_env
    ...
    # in the user's shell would otherwise make a plain `athena --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `athena update` so
    ...
    if code == 42:
        from cortex_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``CORTEX_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``athena kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``athena kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("CORTEX_KANBAN_BOARD"):
        return
    try:
        from cortex_cli.kanban_db import get_current_board

        os.environ["CORTEX_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.cortex/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Athena — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Athena still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    try:
        from cortex_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from cortex_cli.config import load_config as _load_config_for_xai_check
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Athena isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  athena setup")
        print()

        from cortex_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'athena setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from cortex_cli.banner import prefetch_update_check
    ...
    # setup (config, rules files, plugins, MCP servers) or from Athena itself.
    ...
    # cortex_cli/plugins.py and tools/mcp_tool.py).
    ...
    # user's ~/.cortex/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_gateway(args):
    ...
    from cortex_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from cortex_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from cortex_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Athena?")
        print()
    ...
    # has no creds.json.  Every subsequent `athena gateway` then paid a 30s
    ...
    session_dir = get_cortex_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `athena gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Athena Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``athena whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``cortex_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from cortex_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from cortex_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from cortex_cli.config import stamp_install_method
    from cortex_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Athena post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from cortex_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``athena model``) and the setup wizard
    ...
    from cortex_cli.providers import resolve_provider_full
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from cortex_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'athena model' for "
    ...
    # PROVIDER_GROUPS in cortex_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.cortex/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.cortex/.env if the active provider is not 'custom'.
    ...
    """
    from cortex_cli.config import get_env_value, save_env_value, load_config
...
#
# Athena uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `athena model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `athena model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`cortex_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from cortex_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Athena only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``athena model`` flow, then route aux tasks to them here.
    """
    from cortex_cli.config import load_config
    from cortex_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from cortex_cli.auth import _prompt_model_selection
    from cortex_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from cortex_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from cortex_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from cortex_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Athena URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from cortex_cli.curses_ui import curses_radiolist
...
# downstream call sites read `cortex_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `athena` invocation — including
# fast paths like `athena --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from cortex_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``athena setup`` / ``athena model``.
    ...
    recover from a malformed paste without editing ``~/.cortex/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from cortex_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from cortex_cli.config import save_env_value
    from cortex_cli.secret_prompt import masked_secret_prompt
    ...
    from cortex_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `athena setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from cortex_constants import display_cortex_home as _dhh_fn

            print(
                f"    Athena will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Athena CLI with a provider."""
    from cortex_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from cortex_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from cortex_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from cortex_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from cortex_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from cortex_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``athena slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            ...
            "\n"
            "Run `athena slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from cortex_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from cortex_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from cortex_cli.hooks import hooks_command
...
def cmd_security(args):
    """Dispatch `athena security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from cortex_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from cortex_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from cortex_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from cortex_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from cortex_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from cortex_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from cortex_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from cortex_cli.banner import format_banner_version_label
    ...
    try:
        from cortex_cli.banner import check_for_updates
        from cortex_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Athena Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from cortex_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from cortex_cli.uninstall import run_gui_uninstall
    ...
    from cortex_cli.uninstall import run_uninstall
...
# Critical files that every ``athena`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``athena update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "cortex_cli/main.py",
    "cortex_cli/config.py",
    "cortex_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``athena`` startup; if any of them
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``athena update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``cortex_cli/web_dist/`` (per vite.config.ts
    outDir: "../cortex_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "cortex_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``athena`` launcher present but ``cortex_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``athena update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``athena web``).
    ...
    # build path usable on a stock `py -m cortex_cli.main web` invocation.
...
#   - ``athena update`` can unconditionally call ``athena desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``athena desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $CORTEX_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $CORTEX_HOME."""
    from cortex_constants import get_cortex_home
    return get_cortex_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``athena update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Athena.app/Contents/MacOS/Athena"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Athena.exe",
            release_dir / "win-ia32-unpacked" / "Athena.exe",
            release_dir / "win-arm64-unpacked" / "Athena.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "athena",
            release_dir / "linux-unpacked" / "Athena",
            release_dir / "linux-arm64-unpacked" / "athena",
            release_dir / "linux-arm64-unpacked" / "Athena",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Athena'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Athena.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Athena.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Athena.exe: Access is
    ...
    an unrelated "Athena" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Athena is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Athena.app/Contents/MacOS/Athena  ->  app bundle = .../Athena.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Athena Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Athena Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from cortex_logging import setup_logging as _setup_logging_gui
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Athena.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `Athena` rename: unpack-electron
                ...
                if purged:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Athena.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Athena.exe, close any")
                    print("  running Athena desktop window and retry.")
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Athena is
    ...
    if source_mode:
        print("→ Launching Athena Desktop from source build...")
    ...
    print(f"→ Launching packaged Athena Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``athena dashboard`` processes other than ourselves.

    ``athena dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``athena update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Athena Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``athena dashboard`` as
    ...
    patterns = [
        "hermes dashboard",
        "cortex_cli.main dashboard",
        "cortex_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `cortex_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `athena update`.
    ...
    print("  Preview now:  athena curator run --dry-run")
    print("  Pause it:     athena curator pause")
    print(
        "  Docs:         https://athena-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``athena update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``athena update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: athena curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``athena dashboard`` processes.

    Called at the end of ``athena update`` (default ``reason``) and also
    from ``athena dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Athena Desktop Electron app spawns this dashboard as a
...
def _update_via_zip(args):
    """Update Athena Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `athena update "
            f"--branch {branch}`, or update against main with `athena update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/dr-shabana/athena-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="hermes-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"athena-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for athena-agent itself
        ...
        # GitHub ZIPs extract to athena-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"athena-agent-{branch}")
    ...
    from cortex_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from cortex_cli.model_catalog import seed_cache_from_checkout
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Athena behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Athena couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Athena couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Athena behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Athena couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Athena couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `athena update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/dr-shabana/athena-agent.git",
    "git@github.com:dr-shabana/athena-agent.git",
    "https://github.com/dr-shabana/athena-agent",
    "git@github.com:dr-shabana/athena-agent",
}
OFFICIAL_REPO_URL = "https://github.com/dr-shabana/athena-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_cortex_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_cortex_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Athena repository.")
        print("  This means you may miss updates from dr-shabana/athena-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/dr-shabana/athena-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/dr-shabana/athena-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``athena update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumb dropped right before ``athena update`` mutates the
...
# survives and the next ``athena`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $CORTEX_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``athena update`` left half-done.
    ...
    protocol stream (``athena acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `athena update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from cortex_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``athena update`` is still progressing even if pip/uv itself is silent.
    """
...
def _detect_concurrent_hermes_instances(
    ...
    The Athena Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    so during ``athena update`` the user-invoked process and the desktop's
    ...
    chain are excluded so the running ``athena update`` invocation never
    ...
    # file lock) is the parent. Without excluding it, every ``athena update``
    ...
    #      hermes.exe sitting *under* a non-Athena parent (e.g. a Athena
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Athena Desktop, exit any open `athena` REPLs, and")
    lines.append("  stop the gateway (`athena gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `athena update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    when ``athena update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    handles (recovers in <1s), or the Athena Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Athena Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Athena Desktop, exit other `athena` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `athena update`."
        )
...
def _run_quarantined_install(
    ...
    *,
    ...
    is left missing and ``athena`` drops off PATH. This wraps
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``athena update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `athena update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``athena update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``athena update``, not the venv we just wrote
    ...
    # live launcher, which would leave ``athena`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `athena update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `athena update --force` after closing other hermes processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from cortex_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$CORTEX_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from cortex_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `athena update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``athena update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.cortex/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``athena update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``athena update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.cortex/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``athena update --gateway``) the update is already
    ...
    try:
        ...
        # cortex_cli.config.get_cortex_home to simulate setup failure.
        from cortex_cli.config import get_cortex_home as _get_cortex_home

        logs_dir = _get_cortex_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``athena update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from cortex_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``athena update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from cortex_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from cortex_cli.config import recommended_update_command
        from cortex_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``athena update`` without requiring a reinstall.  The
    ...
    ``athena`` prints ``command not found`` even though the symlink lives
    ...
    # /usr/local/bin/hermes, code at /usr/local/lib/athena-agent).
    ...
    path_comment = (
        "# Athena Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of CORTEX_HOME before running the update.
    ...
    CORTEX_HOME directories.  The ``--backup`` flag on ``athena update``
    ...
    try:
        from cortex_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_cortex_home so the user sees ~/.cortex/...
    try:
        from cortex_constants import get_cortex_home, display_cortex_home

        home = get_cortex_home()
        try:
            display_path = f"{display_cortex_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``athena update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Athena Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Athena Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `athena update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Athena via pip (for PyPI installs)."""
    from cortex_cli import __version__
    from cortex_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "athena-agent"]
        ...
        cmd = [pipx, "upgrade", "athena-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "athena-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "athena-agent"]
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # though `git pull` can't touch $CORTEX_HOME, this is cheap
        ...
        try:
            from cortex_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in cortex_cli/config.py bricked
        # every user who ran ``athena update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``athena update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_cortex_home added to cortex_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``athena`` launch finishes the
        ...
        # build.  ``athena desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``athena desktop`` shouldn't be forced into a full
        # Electron build by ``athena update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "cortex_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `athena desktop` to retry)")
        ...
        # current — so copy it straight over ~/.cortex/cache/model_catalog.json
        ...
        # modules in this process.  Reload cortex_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_cortex_home() added since the last release.
        try:
            import importlib
            import cortex_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit CORTEX_HOME so
        # it is not affected by sync_skills()'s module-level CORTEX_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's CORTEX_HOME env var points at the default or a named profile.
        try:
            from cortex_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from cortex_cli.profiles import backfill_profile_envs
        ...
        try:
            from cortex_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `athena curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``athena update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from cortex_cli.tools_config import install_cua_driver
        ...
        # When running as ``athena update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_cortex_home() / ".update_exit_code"
            try:
        ...
        try:
            from cortex_cli.gateway import (
                is_macos,
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `athena update` restart it automatically, allow\n"
                            ...
                            # path in `athena gateway restart`
        ...
        # Warn if legacy Athena gateway unit files are still installed.
        # When both hermes.service (from a pre-rename install) and the
        # current hermes-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `athena update` surfaces the issue until the user migrates.
        try:
            from cortex_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Athena gateway unit(s) detected:")
        ...
        print("  athena model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``athena -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from cortex_constants import display_cortex_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `athena profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_cortex_home()
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.cortex)")
            else:
        ...
        from cortex_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from cortex_constants import get_cortex_home as _hh
        ...
        from cortex_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `athena profile info {name}` for full manifest)")
        if alias_name:
        ...
        from cortex_cli.profiles import profile_exists
        ...
        from cortex_cli.profiles import rename_profile
        ...
        from cortex_cli.profiles import export_profile
        ...
        from cortex_cli.profiles import import_profile
        ...
        import tempfile
        from cortex_cli.profile_distribution import (
            plan_install,
        ...
        from cortex_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `athena profile install` can be updated."
                )
                sys.exit(1)
        ...
        from cortex_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Athena {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from cortex_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.hermes_requires:
        print(f"  Requires: Athena {mf.hermes_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``athena dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``athena dashboard --status``
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No hermes dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `athena update`.
    ...
    # launches the dashboard (`worker dashboard` → CORTEX_HOME points into
    ...
    try:
        from cortex_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "cortex_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        env = os.environ.copy()
        # Drop the profile CORTEX_HOME so the child binds the machine root.
        env.pop("CORTEX_HOME", None)
    ...
    # the same logs directory as every other Athena surface.
    try:
        from cortex_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "cortex_cli" / "web_dist"
        )
    ...
    try:
        from cortex_cli.mcp_startup import start_background_mcp_discovery
    ...
    from cortex_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from cortex_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_completion(args, parser=None):
    ...
    from cortex_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from cortex_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from cortex_cli.logs import tail_log, list_logs
...
# so that in ``athena -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``cortex_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``athena -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``athena`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from cortex_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from cortex_cli.oneshot import run_oneshot
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `athena --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_cortex_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_cortex_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_cortex_home()}/memories/\n")
    else:
        from cortex_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Athena Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from cortex_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from cortex_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from cortex_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from cortex_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from cortex_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from cortex_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from cortex_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from cortex_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from cortex_cli.claw import claw_command
...
def main():
    ...
    # Cosmetic: make the process show up as 'athena' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from cortex_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``hermes.exe.old.*`` quarantine files left by previous
    # ``athena update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # Self-heal a venv left half-built by an interrupted ``athena update``
    ...
    # ``athena skills install update``) merely defers recovery one launch;
    # under-matching (missing ``athena -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in cortex_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from cortex_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://athena-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `athena model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.cortex/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://athena-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from cortex_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from cortex_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in cortex_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in cortex_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in cortex_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in cortex_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `athena whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in cortex_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from cortex_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in cortex_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in cortex_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in cortex_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in cortex_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in cortex_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in cortex_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from cortex_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from cortex_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # hooks command  (parser built in cortex_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in cortex_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in cortex_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in cortex_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in cortex_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in cortex_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.cortex/checkpoints/",
    ...
    from cortex_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in cortex_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in cortex_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in cortex_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in cortex_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from cortex_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in cortex_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``athena --help``, ``athena version``, ``athena logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from cortex_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from cortex_cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # memory command  (parser built in cortex_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in cortex_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `athena computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `athena tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            print("  Run: athena computer-use install")
            return
    ...
    # =========================================================================
    # mcp command  (parser built in cortex_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_cortex_home() / "sessions"
    ...
    # =========================================================================
    # insights command  (parser built in cortex_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in cortex_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in cortex_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in cortex_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in cortex_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in cortex_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in cortex_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in cortex_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Athena-Setup.exe success screen tells users to
    # run `athena desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in cortex_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in cortex_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in cortex_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``athena -c Pokemon Agent Dev`` → ``athena -c 'Pokemon Agent Dev'``
    ...
    from cortex_cli.config import get_container_exec_info

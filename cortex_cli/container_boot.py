...
``$CORTEX_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
(the stage2 hook) has chowned the volume and seeded $CORTEX_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$CORTEX_HOME``,
    not under ``profiles/``). The dispatcher in ``cortex_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``athena gateway start`` (no ``-p``) targets. Without it,
    bare ``athena gateway start`` inside the container would land on
    ...
    ``$CORTEX_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        cortex_home: The container's CORTEX_HOME (typically /opt/data).
            Profiles live under ``<cortex_home>/profiles/<name>/``;
            the default profile lives at ``<cortex_home>`` itself.
    ...
    # ``athena gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_prior_state(cortex_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(cortex_home)
    ...
    profiles_root = cortex_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `athena profile create` (config.yaml
            # is not — that comes later via `athena setup`). Use it as the
            ...
            # via ``athena -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(cortex_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = cortex_home / "gateway_state.json"
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "athena":
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `athena -p <profile> gateway start`
...
def _write_reconcile_log(
    cortex_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $CORTEX_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    cortex_home = Path(os.environ.get("CORTEX_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        cortex_home=cortex_home, scandir=scandir,
    )
    for a in actions:
        print(

"""
Security advisory checker for Athena Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``athena doctor --ack <id>``; the ack is persisted to
...
1. ``athena doctor`` (and ``athena doctor --ack <id>``)
...
run in environments where the rest of Athena failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.cortex/.env (OpenRouter, Anthropic, OpenAI, "
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Athena requires 3.10+ but defensive.
        return None
    try:
...
# ``~/.cortex/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_cortex_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``athena doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `athena doctor` on the gateway host for details.")

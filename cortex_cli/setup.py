"""
Interactive setup wizard for Athena Agent.
...
_DOCS_BASE = "https://athena-agent.nousresearch.com/docs"
...
# display_cortex_home imported lazily at call sites (stale-module safety during hermes update)
...
from cortex_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from cortex_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Athena Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Athena using environment variables or config commands:")
    print_info("  athena config set model.provider custom")
    print_info("  athena config set model.base_url http://localhost:8080/v1")
    print_info("  athena config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'athena setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from cortex_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'athena setup')")


def _print_setup_summary(config: dict, cortex_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'athena setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from cortex_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `athena tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from cortex_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'athena setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'athena setup tts'"))
    else:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'athena setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {cortex_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('athena setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('athena setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('athena setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('athena setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('athena setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('hermes config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('hermes config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('athena config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('athena', Colors.GREEN)}              Start chatting")
...
# between `athena tools` and `athena setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``athena model``)
    ...
    provider added to ``athena model`` is automatically available here.
    ...
    # Delegate to the shared athena model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from cortex_cli.main import select_provider_and_model
    try:
    ...
    # on demand via `athena auth add`, `athena setup` vision, and
    # `athena setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``athena model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from cortex_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.cortex/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Athena auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via athena setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'athena setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Athena runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `athena setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `athena setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `athena setup agent` later to customize.")
...
def _setup_telegram_auto_result():
    ...
    try:
        from cortex_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    ...
    try:
        ...
        if "/profiles/" in cortex_home:
            profile_name = cortex_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Athena delivers cron job results,")
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `athena update`)",
                True,
            ):
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under CORTEX_HOME, and print
    ...
    """
    try:
        from cortex_cli.slack_cli import _build_full_manifest
        from cortex_constants import get_cortex_home

        manifest = _build_full_manifest(
            bot_name="Athena",
            bot_description="Your Athena agent on Slack",
        )
        target = Path(get_cortex_home()) / "slack-manifest.json"
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Athena delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Athena to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from cortex_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://athena-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://athena-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    from cortex_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Athena from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'athena setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     athena config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `athena setup tools` and `athena tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``cortex_cli.auth`` — lists every supported
    ...
    """
    try:
        from cortex_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from cortex_cli.config
    ...
    if section_key == "model":
        ...
        from cortex_cli.gateway import _all_platforms, _platform_status
...
# Config values may have different semantics between OpenClaw and Athena.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Athena to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Athena at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Athena at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Athena at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Athena at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Athena equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Athena config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Athena.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Athena's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(cortex_home: Path) -> bool:
    ...
    print_info("Athena can preview what would be imported before making any changes.")
    print()
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=cortex_home.resolve(),
    ...
    # Execute the migration — overwrite=False so existing Athena configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Athena config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Athena (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``athena setup --portal`` and ``athena portal``. This is the
    ...
    Athena session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``athena setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``athena model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``athena portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Athena Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `athena portal` == quick setup's Nous step.
    try:
        from cortex_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `athena portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `athena portal`: %s", exc)
        print()
    ...
    print_info("  Run `athena portal info` to inspect routing.")
    print_info("  Run `athena` to start chatting.")
...
def run_setup_wizard(args):
    ...
    """
    from cortex_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Athena Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Athena Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Athena Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, cortex_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Athena configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'athena setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(cortex_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, cortex_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {cortex_home}")
    ...
    # Tune later with `athena setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, cortex_home)


def _run_first_time_quick_setup(config: dict, cortex_home, is_existing: bool):
...
def _run_quick_setup(config: dict, cortex_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'athena setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Athena to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'athena setup gateway'.")

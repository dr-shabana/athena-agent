"""CLI handlers for ``athena secrets bitwarden ...``.
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``cortex_cli.main`` as part of building the top-level
    ``athena secrets`` parser.
    """
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every Athena process."
    )
    console.print(
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Athena will skip BSM "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`athena secrets bitwarden setup` first.[/yellow]"
        )
        return 1

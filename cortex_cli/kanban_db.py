...
``<root>`` is the **shared Athena root** (the parent of any active
...
``athena -p <profile>`` joins the same board as the dispatcher that
...
the profile's Athena home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``CORTEX_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``athena kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.cortex``. In Docker / custom
deployments where ``CORTEX_HOME`` points outside ``~/.cortex`` (e.g.
``/opt/hermes``), ``<root>`` is ``CORTEX_HOME``. Legacy env-var
...
``HERMES_KANBAN_WORKSPACES_ROOT``, and ``CORTEX_KANBAN_BOARD`` into
...
# enough that kebab-case names like ``atm10-server`` or ``athena-agent``
...
def kanban_home() -> Path:
    """Return the shared Athena root that anchors the kanban board.
    ...
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``CORTEX_HOME`` is ``<root>/profiles/<name>``, and returns
       ``CORTEX_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``CORTEX_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``athena kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``CORTEX_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``athena kanban boards
    ...
    env = os.environ.get("CORTEX_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``athena kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``athena kanban log`` unambiguous even when multiple
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Athena treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``HERMES_KANBAN_DB`` env → ``CORTEX_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See cortex_state._WAL_INCOMPAT_MARKERS for detection logic.
                from cortex_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``athena kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from cortex_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``athena --skills ...`` alongside the built-in
    ...
    # `athena --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``athena kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `athena kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``athena kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``athena kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``athena kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``athena kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Athena' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Athena-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``athena kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Athena
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``athena kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Athena profile.
    ...
    if not rows:
        return False
    try:
        from cortex_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from cortex_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Athena profile.
        # `_default_spawn` invokes ``athena -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Athena
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `athena kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    ...
    # ``cortex_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``athena`` package — there is no
    # ``athena`` package to import.
    return [sys.executable, "-m", "cortex_cli.main"]


def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Athena shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Athena executable path.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``athena`` invocation as argv parts for ``Popen``.
    ...
    2. ``shutil.which("athena")`` — the console-script shim, normalized to
    ...
    3. ``sys.executable -m cortex_cli.main`` — fallback for setups where
       Athena is launched from a venv and the ``athena`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_hermes_bin`` for the same reason. Kept
    local (not imported from gateway) because ``cortex_cli`` sits below
    ...
    hermes_bin = _safe_which_no_cwd("athena") if _IS_WINDOWS else shutil.which("athena")
    if hermes_bin:
        return _hermes_path_argv(hermes_bin)
    return _module_hermes_argv()


def _kanban_worker_skill_available(cortex_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``athena -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset CORTEX_HOME means the worker falls back to the default root
    # home (``~/.cortex``), which ships the bundled skill.
    base = _Path(cortex_home) if cortex_home else (_Path.home() / ".hermes")
...
def _resolve_worker_cli_toolsets(cortex_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``CORTEX_KANBAN_TASK`` is set.
    """
    if not cortex_home:
        return None
    try:
        from cortex_constants import reset_cortex_home_override, set_cortex_home_override
        from cortex_cli.config import load_config
        from cortex_cli.tools_config import _get_platform_tools

        token = set_cortex_home_override(cortex_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for CORTEX_HOME=%r (%s)",
            cortex_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``athena -p <profile> chat -q ...`` subprocess.
    ...
    ``HERMES_KANBAN_DB`` / ``CORTEX_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject CORTEX_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `athena -p <name>` the
    # _apply_profile_override() runs *before* cortex_constants is imported.
    # If CORTEX_HOME is absent from the child's env, get_cortex_home() falls
    # back to Path.home() / ".hermes" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from cortex_cli.profiles import resolve_profile_env
    try:
        env["CORTEX_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via HERMES_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # CORTEX_HOME never had profiles created.
        pass
    if task.tenant:
        env["HERMES_TENANT"] = task.tenant
    env["CORTEX_KANBAN_TASK"] = task.id
    ...
    # that even when the worker activates a profile (`athena -p <name>`
    # rewrites CORTEX_HOME), its kanban paths still match the
    ...
    env["CORTEX_KANBAN_BOARD"] = resolved_board
    # HERMES_PROFILE is the author the kanban_comment tool defaults to.
    # `athena -p <assignee>` activates the profile, but the env var is
    ...
    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped CORTEX_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("CORTEX_HOME")):
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("CORTEX_HOME"))
    if worker_toolsets:
    ...
    # `athena kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`athena` executable not found on PATH. "
            "Install Athena Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``athena kanban daemon`` is systemd-friendly.
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Athena root exists

    Reads profile paths directly so this module has no import dependency on
    ``cortex_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from cortex_constants import get_default_hermes_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``athena kanban assignees`` for the terminal.

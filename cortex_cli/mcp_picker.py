"""MCP picker — interactive `athena mcp picker` (also the default `athena mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``athena mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `athena plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from cortex_cli.cli_output import prompt_yes_no
from cortex_cli.curses_ui import curses_single_select
from cortex_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Athena session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from cortex_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `athena mcp catalog`."""
    if not rows:
        print()
    ...
    # Surface manifest-version warnings so users know when their Athena is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Athena — run `athena update` "
...
def show_catalog() -> None:
    """`athena mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`athena mcp picker` (and default `athena mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`athena mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `athena mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:

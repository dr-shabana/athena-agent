"""
Baked-in build metadata for Athena Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``cortex_cli/dump.py`` and ``cortex_cli/banner.py``).  That doesn't work inside
...
To make ``athena dump`` and the startup banner identify the exact commit the

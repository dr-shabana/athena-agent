"""``athena setup`` subcommand parser.
...
def build_setup_parser(subparsers, *, cmd_setup: Callable) -> None:
    ...
    setup_parser = subparsers.add_parser(
        ...
        "Run a specific section: athena setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'athena setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",

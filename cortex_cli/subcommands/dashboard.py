"""``athena dashboard`` subcommand parser.

Extracted verbatim from ``cortex_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Athena Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `athena dashboard` cmdlines and SIGTERM them directly — the same
    # path `athena update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
    ...
    # Backward-compat shim: older Athena desktop app shells (<= 0.15.x) spawn the
    # backend as `athena dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Athena couldn't start" with no actionable cause. Accept and silently
    ...
    # `athena dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.cortex/.env.
    # Nested subparser so bare `athena dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "HERMES_DASHBOARD_OAUTH_CLIENT_ID into ~/.cortex/.env, and prints "
            "how to engage the login gate. Requires being logged in (athena setup)."
        ),
    )

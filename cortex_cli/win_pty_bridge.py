"""Windows ConPTY bridge for the `athena dashboard` chat tab.

Drop-in counterpart to ``cortex_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``cortex_cli.web_server`` consumes: ``spawn``, ``read``,

"""
Athena Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``athena desktop`` / ``athena gui``)
     Built inside the agent checkout under ``$CORTEX_HOME/athena-agent/``:
       ...
       - ``$CORTEX_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Athena.app`` or ``~/Applications/Athena.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Athena`` (NSIS per-user)
...
the app name ("Athena"), separate from ``$CORTEX_HOME``:
  - macOS:   ``~/Library/Application Support/Athena``
  - Windows: ``%APPDATA%\\Athena``
  - Linux:   ``$XDG_CONFIG_HOME/Athena`` (default ``~/.config/Athena``)
...
``athena uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(cortex_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return cortex_home / "athena-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Athena"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Athena"
...
def source_built_gui_artifacts(cortex_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``athena desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``athena-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(cortex_home)
    ...
    return [
        ...
        cortex_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Athena" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Athena.app"),
            home / "Applications" / "Athena.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Athena).
            local_base / "Programs" / "Athena",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Athena")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "Athena.desktop",
        ]
    return paths


def agent_is_installed(cortex_home: Path) -> bool:
    """Return True when a usable Python agent install exists under CORTEX_HOME.
    ...
    if (agent_root / "cortex_cli").is_dir():
        return True
...
def gui_is_installed(cortex_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(cortex_home):
        if p.exists():
            return True
...
def gui_install_summary(cortex_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = cortex_home if cortex_home is not None else get_cortex_home()
...
def uninstall_gui(cortex_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``athena-agent/cortex_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$CORTEX_HOME``.

...
Thank you for contributing to Athena Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Athena to work everywhere.
...
Bundled skills (in `skills/`) ship with every Athena install. They should be **broadly useful to most users**:
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.cortex/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(cortex_home, config)` for setup-wizard integration
...
```bash
mkdir -p ~/.cortex/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.cortex/config.yaml
touch ~/.cortex/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.cortex/.env
```
...
│
├── skills/                   # Bundled skills (copied to ~/.cortex/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (athena-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.cortex/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.cortex/`)

| Path | Purpose |
|------|---------|
| `~/.cortex/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.cortex/.env` | API keys and secrets |
| `~/.cortex/auth.json` | OAuth credentials (Nous Portal) |
| `~/.cortex/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.cortex/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.cortex/state.db` | SQLite session database |
| `~/.cortex/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.cortex/cron/` | Scheduled job data |
| `~/.cortex/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.cortex/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
The user may skip setup and keep loading the skill. Athena only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `hermes setup` or update `~/.cortex/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Athena tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Athena Agent" is the secondary collaborator. If the contributor's commit shows "Athena Agent" as author because they used Athena to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Athena tools (`web_extract`, `terminal`, `read_file`).
...
Athena uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.cortex/skins/<name>.yaml`:
...
Athena runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
- Use [GitHub Issues](https://github.com/dr-shabana/athena-agent/issues)
- Include: OS, Python version, Athena version (`hermes version`), full error traceback

"""Centralized logging setup for Athena Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.cortex/logs/`` (profile-aware via ``get_cortex_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``cortex_cli.web_server``,
    ``cortex_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from cortex_constants import get_config_path, get_cortex_home
...
# Used by _ComponentFilter and exposed for ``athena logs --component``.
COMPONENT_PREFIXES = {
    ...
    "cron": ("cron",),
    "gui": (
        "cortex_cli.web_server",
        "cortex_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Athena logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    cortex_home
        Override for the Athena home directory.  Falls back to
        ``get_cortex_home()`` (profile-aware).
    log_level
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from cortex_cli.config import is_managed

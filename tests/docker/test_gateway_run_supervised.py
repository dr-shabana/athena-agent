...
Before the s6 migration, ``docker run nousresearch/athena-agent gateway
...
Every ``docker exec`` runs as ``athena`` per the conftest module
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${CORTEX_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Athena Gateway Starting…"
    ...
    assert "⚕" in combined or "Athena Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Athena Gateway Starting" in file_contents, (

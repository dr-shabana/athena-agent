...
normal tests assume away:
  ...
  - Filesystem: CORTEX_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own CORTEX_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            ...
            os.environ["CORTEX_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("cortex_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from cortex_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "CORTEX_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
    ...
    for bad in bad_metas:
        r = subprocess.run(
            [sys.executable, "-m", "cortex_cli.main", "kanban",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from cortex_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["CORTEX_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE cortex_home: {resolved}")
...
@scenario("cortex_home_with_spaces")
def _(home, kb):
    """CORTEX_HOME at a path with spaces — should work but catches
    ...
    try:
        ...
        print(f"  CORTEX_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("cortex_home_with_unicode")
def _(home, kb):
    """CORTEX_HOME with non-ASCII chars."""
    ...
    try:
        ...
        print(f"  CORTEX_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("cortex_home_via_symlink")
def _(home, kb):
    """CORTEX_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    ...
    try:
        ...
        conn2.close()
        print("  symlinks to same CORTEX_HOME share DB correctly")
    finally:
...
def _(home, kb):
    ...
    try:
        from cortex_cli import web_server as ws  # noqa
    except Exception:
        pass

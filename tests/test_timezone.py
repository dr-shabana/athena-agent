"""
Tests for timezone support (cortex_time module + integration points).
...
def _reset_hermes_time_cache():
    """Reset the cortex_time module cache (replacement for removed reset_cache)."""
    cortex_time._cached_tz = None
    cortex_time._cached_tz_name = None
    cortex_time._cache_resolved = False


# =========================================================================
# cortex_time.now() — core helper
# =========================================================================

class TestAthenaTimeNow:
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="cortex_time"):
            result = cortex_time.now()
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        absolute time when system-local tz != Athena tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_hermes_tz(self):
        """Already-aware datetimes should be normalized to Athena tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Athena tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Athena tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Athena is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Athena tz
        ...
        # Use a Athena timezone far behind UTC so that the numeric wall time
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Athena timezone"
        )

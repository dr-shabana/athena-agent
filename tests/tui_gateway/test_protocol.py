...
def server():
    with patch.dict("sys.modules", {
        "cortex_constants": MagicMock(get_cortex_home=MagicMock(return_value="/tmp/hermes_test")),
        "cortex_cli.env_loader": MagicMock(),
        "cortex_cli.banner": MagicMock(),
        "cortex_state": MagicMock(),
    }):
        import importlib
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "cortex_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "athena-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "athena-agent-dev"

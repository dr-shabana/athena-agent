...
def test_root_session_no_compression(db):
    ...
    assert prov["currentAthenaSessionId"] == "root1"
    assert prov["rootAthenaSessionId"] == "root1"
    assert prov["parentAthenaSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    assert prov["parentAthenaSessionId"] == "old"
    assert prov["rootAthenaSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousAthenaSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootAthenaSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootAthenaSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    assert prov["previousAthenaSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"athena"}
    assert "sessionProvenance" in meta["athena"]
    assert meta["athena"]["sessionProvenance"]["currentAthenaSessionId"] == "root1"

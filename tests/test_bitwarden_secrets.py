...
def cortex_home(tmp_path, monkeypatch):
    """Point Athena at an isolated home directory."""
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("CORTEX_HOME", str(home))
    # Some modules cache get_cortex_home; clear if needed.
    import cortex_constants
    if hasattr(cortex_constants, "_CORTEX_HOME_CACHE"):
        cortex_constants._CORTEX_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(cortex_home, monkeypatch):
...
def test_install_bws_happy_path(cortex_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(cortex_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(cortex_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from cortex_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under cortex_home/cache/."""
    home = tmp_path / ".hermes"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")

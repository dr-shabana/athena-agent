...
class TestWriteDenyExactPaths:
    ...
    def test_hermes_env(self):
        # ``.env`` under the active CORTEX_HOME (profile-aware, not just
        # ``~/.hermes``) must be write-denied. The hermetic test conftest
        # points CORTEX_HOME at a tempdir — resolve via get_cortex_home()
        # to match the denylist.
        from cortex_constants import get_cortex_home
        path = str(get_cortex_home() / ".env")
        assert _is_write_denied(path) is True
    ...
    def test_hermes_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.cortex/.env`` (whose credentials are inherited by every profile)
        ...
        # Sanity check: CORTEX_HOME does point to the profile dir, not the root.
        from cortex_constants import get_cortex_home, get_default_hermes_root
        assert get_cortex_home() == profile_home

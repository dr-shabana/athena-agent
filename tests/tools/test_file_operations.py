...
class TestIsWriteDenied:
    ...
    def test_hermes_control_files_oauth_and_mcp_tokens_denied(self, path):
        """Athena control files, PKCE creds, mcp-tokens, and pairing entries must be write-denied."""
        from cortex_constants import get_cortex_home
        cortex_home = get_cortex_home()
        full_path = str(cortex_home / path)
    ...
    def test_hermes_control_files_and_oauth_traversal_denied(self, path):
        """Path traversal attempts to protected Athena files must be blocked."""
        from cortex_constants import get_cortex_home
        cortex_home = get_cortex_home()
        full_path = str(cortex_home / path)
        assert _is_write_denied(full_path) is True
    ...
    def test_control_files_and_oauth_protected_in_profile_mode(self, tmp_path, monkeypatch, name):
        ...
        global ~/.cortex/{auth.json,config.yaml,webhook_subscriptions.json,
        .anthropic_oauth.json} writable — the same gap PR #15981 fixed
        for .env.
        """
        # Simulate a profile-mode CORTEX_HOME layout:
    ...
    def test_pairing_dir_denied(self, tmp_path, monkeypatch):
        ...
        PR #30383 introduced ~/.cortex/pairing/{platform}-approved.json as the

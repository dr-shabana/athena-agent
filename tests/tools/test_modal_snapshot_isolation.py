...
def _restore_tool_modules():
    original_cortex_home = os.environ.get("CORTEX_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_cortex_home is None:
            os.environ.pop("CORTEX_HOME", None)
        else:
            os.environ["CORTEX_HOME"] = original_cortex_home
        _reset_modules(("tools", "cortex_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    cortex_cli = types.ModuleType("cortex_cli")
    cortex_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["cortex_cli"] = cortex_cli
    cortex_home = tmp_path / "hermes-home"
    os.environ["CORTEX_HOME"] = str(cortex_home)
    sys.modules["cortex_cli.config"] = types.SimpleNamespace(
        get_cortex_home=lambda: cortex_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="athena-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": cortex_home / "modal_snapshots.json",

...
Under a gateway (Telegram, Discord, Slack, ...) Athena passes the
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Athena via Telegram AND Discord
...
class TestPinUserPeerAlias:
    ...
    def test_host_pinUserPeer_false_disables_root_pinPeerName(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "peerName": "eri",
            "pinPeerName": True,
            "hosts": {"athena": {"pinUserPeer": False}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.pin_peer_name is False, (
...
class TestProfilePeerUniqueness:
    """Each Athena profile can pin to its own unique peerName.

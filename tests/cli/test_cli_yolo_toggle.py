...
``os.environ["CORTEX_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``hermes --yolo`` (process-start env), ``CORTEX_YOLO_MODE=1``,
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_does_not_mutate_env_var(self):
        """Toggling /yolo must not write ``CORTEX_YOLO_MODE`` — that path is
        ...
        assert os.environ.get("CORTEX_YOLO_MODE") is None
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``hermes --yolo`` sets ``CORTEX_YOLO_MODE`` before tool imports, so
...
class TestIsSessionYoloActiveAttrSafety:
    ...
    (tests use ``AthenaCLI.__new__(AthenaCLI)`` to skip ``__init__``). It must
    ...
    def test_helper_survives_missing_session_id_attr(self):
        ...
        assert AthenaCLI._is_session_yolo_active(no_attr) is False
...
class TestSessionRotationTransfersYolo:
    ...
    def test_transfer_is_noop_when_ids_match(self):
        ...
        try:
            ...
            AthenaCLI._transfer_session_yolo(stand_in, "same-id", "same-id")

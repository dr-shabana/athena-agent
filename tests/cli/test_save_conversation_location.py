...
The fix writes snapshots under ``~/.cortex/sessions/saved/`` and prints
...
def cortex_home(tmp_path, monkeypatch):
    ...
    # Clear any cached cortex_home computation
    import cortex_constants
    if hasattr(cortex_constants, "_cortex_home_cache"):
        cortex_constants._cortex_home_cache = None
    return home
...
def test_save_conversation_writes_under_cortex_home(cortex_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.cortex/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the CORTEX_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "cortex_constants"]:
    ...
    cli.AthenaCLI.save_conversation(stub)

    # File must NOT be in CWD
    cwd_leak = list(work.glob("hermes_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.cortex/sessions/saved/
    saved_dir = cortex_home / "sessions" / "saved"
...
def test_save_conversation_empty_history_does_nothing(cortex_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "cortex_constants"]:
        sys.modules.pop(mod, None)
    import cli

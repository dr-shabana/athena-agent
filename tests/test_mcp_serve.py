"""
Tests for mcp_serve — Athena MCP server.
...
def _isolate_cortex_home(tmp_path, monkeypatch):
    """Redirect CORTEX_HOME to a temp directory."""
    monkeypatch.setenv("CORTEX_HOME", str(tmp_path))
    try:
        import cortex_constants
        monkeypatch.setattr(cortex_constants, "get_cortex_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking cortex_state schema."""

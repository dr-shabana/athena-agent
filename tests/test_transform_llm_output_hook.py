...
from cortex_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(cortex_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <cortex_home>/plugins/<name> and opt it in."""
    plugin_dir = cortex_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    cortex_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        cortex_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CORTEX_HOME", str(tmp_path / "hermes_empty"))

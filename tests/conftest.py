"""Shared fixtures for the athena-agent test suite.
...
2. **Isolated CORTEX_HOME.** CORTEX_HOME points to a per-test tempdir so
   code reading ``~/.cortex/*`` via ``get_cortex_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".hermes"`` instead of the canonical
   ``get_cortex_home()`` is a bug to fix at the callsite.)
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # the real ~/.cortex/kanban.db instead of the per-test CORTEX_HOME.
    "HERMES_KANBAN_DB",
    "CORTEX_KANBAN_BOARD",
    "HERMES_KANBAN_HOME",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and CORTEX_HOME to per-test tempdirs so code that
    reads ``~/.cortex/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 3. Redirect CORTEX_HOME to a per-test tempdir. Code that reads
    #    ``~/.cortex/*`` via ``get_cortex_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.cortex/*`` via
    #    ``Path.home() / ".hermes"`` instead of ``get_cortex_home()``
    #    is a bug to fix at the callsite.
    fake_cortex_home = tmp_path / "hermes_test"
    fake_cortex_home.mkdir()
    (fake_cortex_home / "sessions").mkdir()
    (fake_cortex_home / "cron").mkdir()
    (fake_cortex_home / "memories").mkdir()
    (fake_cortex_home / "skills").mkdir()
    monkeypatch.setenv("CORTEX_HOME", str(fake_cortex_home))
    ...
    #    ~/.cortex/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import cortex_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_cortex_home(_hermetic_environment):
...
# correlated with a ``tests/cortex_cli/`` pytest run starting up.
#
...
def _live_system_guard(request, monkeypatch):
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m cortex_cli.main".
                if (
                    "athena" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        # Block any subprocess that would run `athena update` (or the
        # equivalent `python -m cortex_cli.main update`).  These commands
        ...
        if "update" in low and (
            ...
            ("athena" in low and "update" in low.split())
            or
            # python -m cortex_cli.main update --gateway
            ("cortex_cli" in low and "update" in low.split())
            or
            ...
            raise RuntimeError(
                ...
                "`athena update` against the real checkout, fetching "

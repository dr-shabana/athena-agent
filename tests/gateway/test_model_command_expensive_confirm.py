...
def _fake_warning():
    return SimpleNamespace(
        message=(
            ...
            "openai/gpt-5.5-pro has known pricing above Athena' safety threshold.\n"
...
def _setup_isolated_home(tmp_path, monkeypatch, *, warn):
    ...
    monkeypatch.setattr(gateway_run, "_cortex_home", cortex_home)
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(
        "cortex_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    monkeypatch.setattr("cortex_constants.get_cortex_home", lambda: cortex_home)
    monkeypatch.setattr("cortex_cli.config.get_cortex_home", lambda: cortex_home)
    monkeypatch.setattr(

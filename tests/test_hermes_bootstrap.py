"""Tests for cortex_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Athena entry point
(hermes, athena-agent, athena-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Athena entry point imports cortex_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported cortex_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("cortex_bootstrap", None)
    import cortex_bootstrap  # noqa: WPS433
    return cortex_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Athena CLI
...
class TestEntryPointsImportBootstrap:
    """Every Athena entry point must import cortex_bootstrap as its
    ...
    # Entry points that invoke Athena as a process.  Each one must
    # import cortex_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "cortex_cli/main.py",   # hermes CLI (console_script)
        "run_agent.py",          # athena-agent (console_script)
        "acp_adapter/entry.py",  # athena-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import cortex_bootstrap' and that
        ...
        ``cortex_bootstrap`` as a top-level module) leaves hermes
        ...
        """
        # Resolve relative to the athena-agent repo root.  Tests live
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hermes start even when cortex_bootstrap hasn't been
        ...
        assert first_import_name == "cortex_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'cortex_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import cortex_bootstrap' line to be the first import."
        )

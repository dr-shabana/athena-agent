...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("cortex_cli.config.get_config_path", return_value=config_path), \
             patch("cortex_cli.config.ensure_cortex_home"):
    ...
    def test_ensure_cortex_home_sets_0700(self):
        home = Path(self.tmpdir) / ".hermes"
        with patch("cortex_cli.config.get_cortex_home", return_value=home):

...
- Path resolution (absolute, relative to CORTEX_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp CORTEX_HOME."""
    cortex_home = tmp_path / ".hermes"
    cortex_home.mkdir()
    (cortex_home / "cron").mkdir()
    (cortex_home / "cron" / "output").mkdir()
    (cortex_home / "scripts").mkdir()
    monkeypatch.setenv("CORTEX_HOME", str(cortex_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "HERMES_DIR", cortex_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", cortex_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", cortex_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", cortex_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.cortex/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script

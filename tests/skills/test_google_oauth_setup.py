...
class TestAthenaConstantsFallback:
    """Tests for _cortex_home.py fallback when cortex_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_cortex_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _cortex_home.py with cortex_constants blocked."""
        monkeypatch.setitem(sys.modules, "cortex_constants", None)
        spec = importlib.util.spec_from_file_location("_cortex_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_cortex_home_env_var(self, monkeypatch, tmp_path):
        """When cortex_constants is missing, CORTEX_HOME comes from env var."""
        monkeypatch.setenv("CORTEX_HOME", str(tmp_path / "custom-hermes"))
        module = self._load_helper(monkeypatch)
        assert module.get_cortex_home() == tmp_path / "custom-hermes"

    def test_fallback_defaults_to_dot_hermes(self, monkeypatch):
        """When cortex_constants is missing and CORTEX_HOME unset, default to ~/.hermes."""
        monkeypatch.delenv("CORTEX_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_cortex_home() == Path.home() / ".hermes"

    def test_fallback_ignores_empty_cortex_home(self, monkeypatch):
        """Empty/whitespace CORTEX_HOME is treated as unset."""
        monkeypatch.setenv("CORTEX_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_cortex_home() == Path.home() / ".hermes"

    def test_fallback_display_cortex_home_shortens_path(self, monkeypatch):
        """Fallback display_cortex_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("CORTEX_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_cortex_home() == "~/.hermes"

    def test_fallback_display_cortex_home_profile_path(self, monkeypatch):
        """Fallback display_cortex_home() handles profile paths under ~/."""
        monkeypatch.setenv("CORTEX_HOME", str(Path.home() / ".hermes/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_cortex_home() == "~/.cortex/profiles/coder"

    def test_fallback_display_cortex_home_custom_path(self, monkeypatch):
        """Fallback display_cortex_home() returns full path for non-home locations."""
        monkeypatch.setenv("CORTEX_HOME", "/opt/hermes-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_cortex_home() == "/opt/hermes-custom"

    def test_delegates_to_hermes_constants_when_available(self):
        """When cortex_constants IS importable, _cortex_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_cortex_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import cortex_constants
        assert module.get_cortex_home is cortex_constants.get_cortex_home
        assert module.display_cortex_home is cortex_constants.display_cortex_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Athena Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "athena-agent[google]" in out

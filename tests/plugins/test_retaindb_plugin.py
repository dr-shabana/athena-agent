...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure CORTEX_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", cortex_home=str(tmp_path / ".hermes"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", cortex_home=profile_home)
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CORTEX_HOME", str(cortex_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", cortex_home=str(cortex_home))
        return p

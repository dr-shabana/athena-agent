...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `cortex_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "athena"
    called = []

    fake_main_module = types.ModuleType("cortex_cli.main")

    def fake_main():
        called.append("cortex_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "cortex_cli.main", fake_main_module)

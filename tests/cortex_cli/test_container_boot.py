"""Tests for cortex_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $CORTEX_HOME under tmp_path; no real
...
from cortex_cli.container_boot import (
    ReconcileAction,
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under cortex_home/profiles/<name>/."""
    p = cortex_home / "profiles" / name
...
def _seed_default_root(
    ...
    *,
    ...
    CORTEX_HOME root (the implicit default profile)."""
    if state is not None:
        (cortex_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (cortex_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (cortex_home / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        cortex_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_reconcile_log_rotates_when_size_exceeded(
    ...
    from cortex_cli import container_boot
...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        cortex_home=tmp_path, scandir=scandir, dry_run=True,
    )
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    """When $CORTEX_HOME/profiles doesn't exist (fresh install), the
...
def test_default_slot_always_registered_on_empty_home(tmp_path: Path) -> None:
    """Bare CORTEX_HOME with nothing under it still produces a
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    ...
    that would resolve to $CORTEX_HOME/profiles/default/ instead of
...
def test_default_slot_autostarts_when_root_state_running(tmp_path: Path) -> None:
    """gateway_state.json at the CORTEX_HOME root with state=running
...
def test_default_slot_cleans_up_stale_runtime_files_at_root(
    ...
    """gateway.pid and processes.json at the CORTEX_HOME root (left

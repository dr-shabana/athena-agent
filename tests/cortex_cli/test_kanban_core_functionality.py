...
Complements tests/cortex_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/cortex_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/cortex_cli/test_kanban_db.py.
    monkeypatch.setenv("HERMES_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.cortex/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_list_profiles_on_disk_custom_root(tmp_path, monkeypatch):
    """list_profiles_on_disk respects a custom CORTEX_HOME root."""
...
def test_parse_duration_accepts_formats():
    from cortex_cli.kanban import _parse_duration
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: cortex_cli.main doesn't propagate sub-command exit codes
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    """First CLI action on an empty CORTEX_HOME must not error with
    ...
    r = _sp.run(
        [_sys.executable, "-m", "cortex_cli.main", "kanban",
...
def test_connect_auto_inits_fresh_db(tmp_path, monkeypatch):
    """Calling connect() on a fresh CORTEX_HOME must create the
...
def test_default_spawn_auto_loads_kanban_worker_skill(kanban_home, monkeypatch):
    ...
    # CORTEX_HOME — the fixture creates an empty tmpdir without the
    ...
    env = captured["env"]
    assert env.get("CORTEX_KANBAN_TASK") == tid
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from cortex_cli import kanban as kb_cli
...
def test_gateway_dispatcher_watcher_respects_config_flag_off(monkeypatch):
    ...
    import cortex_cli.config as _cfg_mod

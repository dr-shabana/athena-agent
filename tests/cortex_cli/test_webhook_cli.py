"""Tests for cortex_cli/webhook.py — webhook subscription CLI."""
...
from cortex_cli.webhook import (
    webhook_command,
    _load_subscriptions,
    _save_subscriptions,
    _subscriptions_path,
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "cortex_cli.webhook._is_webhook_enabled", lambda: True
    )
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("cortex_cli.webhook._is_webhook_enabled", lambda: False)
    ...
    def test_real_check_disabled(self, monkeypatch):
        monkeypatch.setattr(
            "cortex_cli.webhook._get_webhook_config",
            lambda: {},
        )
        monkeypatch.setattr(
            "cortex_cli.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import cortex_cli.webhook as wh_mod

...
``cortex_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from cortex_cli import send_cmd
...
def test_load_hermes_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``athena config set``) lives
    ...
    cortex_home.mkdir()
    (cortex_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_cortex_home() to re-resolve under the patched env.
    from importlib import reload

    import cortex_cli.config as _hc_config
    reload(_hc_config)
...
def test_load_hermes_env_does_not_override_existing(tmp_path, monkeypatch):
    ...
    cortex_home = tmp_path / ".hermes"
    cortex_home.mkdir()
    (cortex_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")

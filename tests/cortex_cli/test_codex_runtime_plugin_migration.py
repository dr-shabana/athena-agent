...
from cortex_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from cortex_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from cortex_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_expose_hermes_tools_writes_callback_mcp_entry(self, tmp_path):
        ...
        into Athena for browser/web/delegate_task/vision/memory tools.
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If Athena appends
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Athena' managed block, re-running
...
class TestStripUnmanagedPluginTables:
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "cortex_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
# ---- Bug C: CORTEX_HOME tempdir leak into ~/.codex/config.toml ----


class TestAthenaHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``CORTEX_HOME``
    ...
    def test_tempdir_detector_accepts_real_cortex_home(self):
    ...
    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when CORTEX_HOME points at a pytest
        ...
        env = entry.get("env", {})
        assert "CORTEX_HOME" not in env, (
            f"pytest-tempdir CORTEX_HOME leaked into codex MCP entry: "
            f"{env.get('CORTEX_HOME')!r}"
        )

    def test_real_cortex_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate CORTEX_HOME (not a tempdir path) DOES propagate so the
        ...
        monkeypatch.setenv("CORTEX_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("CORTEX_HOME") == real_path

    def test_unset_cortex_home_omits_env_key(self, monkeypatch):
        """When CORTEX_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever CORTEX_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "CORTEX_HOME" not in env, (
            f"CORTEX_HOME should not be set when env var is unset, got: "
            f"{env.get('CORTEX_HOME')!r}"
        )

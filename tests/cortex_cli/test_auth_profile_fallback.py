...
When ``CORTEX_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Profile     -> tmp_path/.hermes/profiles/coder   (active, CORTEX_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Athena is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from cortex_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    cortex_home = tmp_path / "classic"
    cortex_home.mkdir()
    monkeypatch.setenv("CORTEX_HOME", str(cortex_home))

    _write(cortex_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (CORTEX_HOME == global root), no fallback path runs.
    ...
    # to the CORTEX_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    cortex_home = tmp_path / "classic"
    ...
    _write(cortex_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from cortex_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: CORTEX_HOME is set to a custom path that is NOT under
    # ~/.cortex/profiles/ — get_default_hermes_root() returns CORTEX_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from cortex_cli.auth import read_credential_pool, write_credential_pool

...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("cortex_cli.config.get_managed_system", return_value=None), \
         patch("cortex_cli.config.get_cortex_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with patch("cortex_cli.config.get_managed_system", return_value="NixOS"), \
         patch("cortex_cli.config.get_cortex_home", return_value=tmp_path):
        from cortex_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from cortex_cli.config import recommended_update_command_for_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    with patch("cortex_cli.config.get_managed_system", return_value=None), \
         patch("cortex_cli.config.get_cortex_home", return_value=tmp_path), \
         patch("cortex_constants.is_container", return_value=True):
        from cortex_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    from cortex_cli import banner

    hh = tmp_path / ".hermes"
    hh.mkdir()
    (hh / ".install_method").write_text("pip\n")

    with patch("cortex_cli.config.get_cortex_home", return_value=hh), \
         patch("cortex_constants.get_cortex_home", return_value=hh):
        buf = io.StringIO()

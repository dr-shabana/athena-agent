...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "hermes-cli",
            "product_id": "nous-athena-agent",
            "nous_client": "athena-agent",
            "exp": int(time.time()) + 900,
            "paid_access": True,
            "subscription_tier": 2,
        }
    )
    monkeypatch.setattr("cortex_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    ...
    assert info.product_id == "nous-athena-agent"
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("cortex_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("cortex_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("cortex_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)

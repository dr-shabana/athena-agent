"""Comprehensive tests for cortex_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and CORTEX_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * CORTEX_HOME  -> tmp_path/.hermes  (so get_cortex_home() agrees)
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["athena", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Athena install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_cortex_home(self, profile_env):
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes athena-agent, .worktrees,
        ...
        (default_home / "athena-agent" / ".git").mkdir(parents=True)
        (default_home / "athena-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "athena-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "athena-agent").exists()
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("cortex_cli.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("cortex_cli.profiles._cleanup_gateway_service"), \
             patch("cortex_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
...
class TestGetActiveProfileName:
    ...
    def test_default_cortex_home_returns_default(self, profile_env):
        # CORTEX_HOME points to tmp_path/.hermes which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom CORTEX_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("CORTEX_HOME", str(custom))
        # With Docker-aware roots, a custom CORTEX_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("athena")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from cortex_cli.profiles import create_wrapper_script
    ...
    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from cortex_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from cortex_cli.profiles import remove_wrapper_script
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from cortex_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from cortex_cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from cortex_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("cortex_cli.profiles.check_alias_collision", return_value="skip"):
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "hermes.ssi_health": {"workspace": "athena", "enabled": True}
            }
        }))
        ...
        assert "hermes.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["hermes_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["hermes_heimdall"]["workspace"] == "athena"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("athena-agent", ".worktrees", "profiles", "bin",
        ...
        excluded_prefixes = [
            "default/athena-agent", "default/.worktrees", "default/profiles",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultAthenaHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_cortex_home()."""
    ...
    def test_default_cortex_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_cortex_home()
        assert home == tmp_path / ".hermes"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (CORTEX_HOME outside ~/.hermes), profiles go under CORTEX_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CORTEX_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_cortex_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_cortex_home() returns CORTEX_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CORTEX_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (CORTEX_HOME under ~/.hermes), profiles root is still ~/.cortex/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under CORTEX_HOME."""
        from cortex_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CORTEX_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under CORTEX_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CORTEX_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CORTEX_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from cortex_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".hermes"

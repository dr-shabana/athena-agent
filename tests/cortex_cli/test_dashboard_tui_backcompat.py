...
Older Athena desktop app shells (<= 0.15.x) spawn the backend as::
...
died before it became ready and the desktop GUI showed only "Athena couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real cortex_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "cortex_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,

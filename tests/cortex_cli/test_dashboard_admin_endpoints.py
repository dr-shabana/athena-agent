...
def _client():
    ...
    # Keep the state DB under the isolated CORTEX_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_cortex_home):
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_cortex_home):
        ...
        (get_cortex_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "cortex_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestDebugShareEndpoint:
    ...
    def test_returns_structured_urls(self, monkeypatch):
        import cortex_cli.debug as dbg
        ...
        monkeypatch.setattr("cortex_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `cortex tools` configurator."""

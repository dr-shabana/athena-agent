"""Regression tests for #34107 — Docker UID/GID handling in ensure_cortex_home.

When Athena runs in Docker with ``HERMES_UID=1000`` / ``HERMES_GID=911``,
the entrypoint chowns the top-level ``CORTEX_HOME`` once at startup. But
...
class TestResolveAthenaUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        ...
        from cortex_cli.config import _resolve_hermes_uid_gid

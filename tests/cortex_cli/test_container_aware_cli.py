...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("cortex_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "athena-agent"
    assert info["exec_user"] == "athena"
...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("cortex_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        cortex_home = Path(tmpdir) / ".hermes"
        cortex_home.mkdir()
        (cortex_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("cortex_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_cortex_home": lambda: cortex_home}), \
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from cortex_cli.main import _exec_in_container

"""Tests for cortex_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateDeepLink:
    ...
    def test_name_url_encoded(self):
        link = generate_deep_link(
            ...
            suggested_name="Athena & Friends",
        )
        assert "Athena+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "deep_link": "https://t.me/newbot/AthenaSetupBot/hermes_abcdefghijklmnop_bot?name=Athena+Agent",
            "qr_payload": "https://t.me/newbot/AthenaSetupBot/hermes_abcdefghijklmnop_bot?name=Athena+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "cortex_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="Athena Agent")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="hermes_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/AthenaSetupBot/hermes_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/AthenaSetupBot/hermes_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        with patch(
            "cortex_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("cortex_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("cortex_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("cortex_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("cortex_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("cortex_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "cortex_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("cortex_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("cortex_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from cortex_cli.setup import _setup_telegram_auto

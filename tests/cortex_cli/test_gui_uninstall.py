"""Tests for cortex_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import cortex_cli.gui_uninstall as gu


def _make_agent(cortex_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = cortex_home / "athena-agent"
    (agent_root / "cortex_cli").mkdir(parents=True)
    (agent_root / "cortex_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(cortex_home: Path) -> None:
    """Create the source-built GUI artifacts a `hermes desktop` run produces."""
    desktop = cortex_home / "athena-agent" / "apps" / "desktop"
    ...
    (cortex_home / "athena-agent" / "node_modules").mkdir(parents=True)
    (cortex_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(cortex_home: Path) -> None:
    (cortex_home / "config.yaml").write_text("x: 1\n")
    (cortex_home / ".env").write_text("KEY=secret\n")
    (cortex_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    cortex_home = tmp_path / ".hermes"
    cortex_home.mkdir()
    assert gu.agent_is_installed(cortex_home) is False
    _make_agent(cortex_home)
    assert gu.agent_is_installed(cortex_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    cortex_home = tmp_path / ".hermes"
    (cortex_home / "athena-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(cortex_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    cortex_home = tmp_path / ".hermes"
    _make_gui_build(cortex_home)
    artifacts = gu.source_built_gui_artifacts(cortex_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(cortex_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(cortex_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    cortex_home = tmp_path / ".hermes"
    agent_root = _make_agent(cortex_home)
    _make_gui_build(cortex_home)
    _make_user_data(cortex_home)
    ...
    removed = gu.uninstall_gui(cortex_home)
    ...
    assert not (cortex_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "cortex_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (cortex_home / "config.yaml").exists()
    assert (cortex_home / ".env").exists()
    assert (cortex_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    cortex_home = tmp_path / ".hermes"
    _make_agent(cortex_home)
    userdata = tmp_path / "Athena-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(cortex_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(cortex_home)
    ...
    assert summary["cortex_home"] == str(cortex_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Athena"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Athena"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Athena"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Athena"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    CORTEX_HOME with all the destructive externals stubbed out.
    """
    import cortex_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_cortex_home", lambda: cortex_home)
    ...
    from cortex_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (cortex_home / "config.yaml").exists()
    assert cortex_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole CORTEX_HOME non-interactively."""
    import cortex_cli.uninstall as uninstall

    cortex_home = tmp_path / ".hermes"
    (cortex_home / "athena-agent" / "cortex_cli").mkdir(parents=True)
    (cortex_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not cortex_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m cortex_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_cortex_home", lambda: cortex_home)
    ...
    assert not (desktop / "release").exists()
    assert not (cortex_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "cortex_cli").exists()
    assert (cortex_home / "config.yaml").exists()

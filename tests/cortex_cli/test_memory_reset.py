...
- Profile-scoped reset (uses CORTEX_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake CORTEX_HOME with memory files."""
    cortex_home = tmp_path / ".hermes"
    memories = cortex_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("CORTEX_HOME", str(cortex_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAthena repo is at ~/.cortex/athena-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
class TestMemoryReset:
    ...
    def test_reset_all_with_yes_flag(self, memory_env):
        ...
        cortex_home, memories = memory_env
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        cortex_home = tmp_path / ".hermes"
        (cortex_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("CORTEX_HOME", str(cortex_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's CORTEX_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_cortex_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"

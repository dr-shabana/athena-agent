...
``cortex_cli.main`` skips eager plugin discovery at argparse-setup time
...
def _live_subcommand_names() -> set[str]:
    ...
    built-in-only set.
    """
    from cortex_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["athena", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["athena", "--tui", "chat"], "chat"),
        (["athena", "-w", "logs"], "logs"),
        (["athena", "chat", "hello world"], "chat"),
        (["athena", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["athena", "-m", "gpt5", "chat"], "chat"),
        (["athena", "--model", "gpt5", "chat", "hi"], "chat"),
        (["athena", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["athena", "-z", "hello world"], None),
        (["athena", "-z", "hello", "chat"], "chat"),
        (["athena", "--model=gpt5", "chat"], "chat"),     # inline form
        (["athena", "--", "chat"], "chat"),               # -- terminator
        (["athena", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["athena", "some-plugin-cmd"], "some-plugin-cmd"),
        (["athena", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["athena"],                          # bare → chat
        ["athena", "--help"],                # top-level help
        ["athena", "-h"],
        ["athena", "version"],               # known built-in
        ["athena", "logs"],
        ["athena", "gateway", "run"],
        ["athena", "--tui"],
        ["athena", "-w", "--tui"],
        ["athena", "chat", "hi"],
        ["athena", "help"],                  # accepted built-in-ish
        ["athena", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["athena", "meet", "join"],          # potential google_meet plugin
        ["athena", "honcho", "status"],      # potential memory plugin
        ["athena", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    assert not missing_from_declaration, (
        ...
        f"cortex_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "

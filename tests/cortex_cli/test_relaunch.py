"""Tests for cortex_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from cortex_cli import relaunch as relaunch_mod


class TestResolveAthenaBin:
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        fake = tmp_path / "athena"
        fake.write_text("#!/bin/sh\n")
        fake.chmod(0o755)
        monkeypatch.setattr(sys, "argv", [str(fake.name)])
        monkeypatch.chdir(tmp_path)
        # Ensure we don't accidentally match a real 'athena' on PATH
    ...
    def test_falls_back_to_path_which(self, monkeypatch):
        ...
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/hermes" if name == "athena" else None
        )
...
class TestBuildRelaunchArgv:
    ...
    def test_falls_back_to_python_module(self, monkeypatch):
        ...
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv == [sys.executable, "-m", "cortex_cli.main", "--resume", "abc"]
...
class TestResolveAthenaBinWindowsPyGuard:
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            ...
            lambda name: r"C:\venv\Scripts\hermes.exe" if name == "athena" else None,
        )

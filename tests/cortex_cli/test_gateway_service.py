...
import cortex_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``CORTEX_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` CORTEX_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir CORTEX_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="CORTEX_HOME=/tmp/pytest-of-alice/pytest-42/'
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E CORTEX_HOME like
        ...
        ``hermes gateway restart`` with a /tmp CORTEX_HOME exported; the
        ...
        must refuse ANY temp-dir CORTEX_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "hermes-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="CORTEX_HOME=/tmp/hermes-e2e-41264"\n'
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="CORTEX_HOME=/var/tmp/hermes-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="CORTEX_HOME={tmp_path}/hermes-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None
    ...
    def test_accepts_real_home(self):
        unit = '[Service]\nEnvironment="CORTEX_HOME=/home/alice/.hermes"\n'
    ...
    def test_accepts_unit_without_cortex_home(self):
        unit = "[Service]\nExecStart=/usr/bin/python -m cortex_cli.main gateway run\n"
        assert gateway_cli._temp_home_in_service_definition(unit) is None
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="CORTEX_HOME=/tmpfs-data/.hermes"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_cortex_home_for_target_user", lambda home: "/home/alice/.hermes")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        ...
        # CORTEX_HOME resolves under the (pytest tmp) test CORTEX_HOME.
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test CORTEX_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>CORTEX_HOME</key>"
                "<string>/Users/alice/.hermes</string></plist>"
            ),
        )
...
class TestSystemUnitAthenaHome:
    """CORTEX_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'CORTEX_HOME=/home/alice/.hermes' in unit
        assert '/root/.hermes' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: CORTEX_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("CORTEX_HOME", "/root/.hermes/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'CORTEX_HOME=/home/alice/.hermes/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_cortex_home(self, monkeypatch):
        # Custom CORTEX_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("CORTEX_HOME", "/opt/hermes-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'CORTEX_HOME=/opt/hermes-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's CORTEX_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        cortex_home = str(gateway_cli.get_cortex_home().resolve())
        assert f'CORTEX_HOME={cortex_home}' in unit


class TestAthenaHomeForTargetUser:
    """Unit tests for _cortex_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("CORTEX_HOME", raising=False)

        result = gateway_cli._cortex_home_for_target_user("/home/alice")
    ...
    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("CORTEX_HOME", "/opt/hermes")
...
class TestProfileArg:
    ...
    def test_default_cortex_home_returns_empty(self, tmp_path, monkeypatch):
        ...
        result = gateway_cli._profile_arg(str(cortex_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.cortex/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".hermes" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CORTEX_HOME", str(tmp_path / ".hermes"))
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile CORTEX_HOME should return empty string."""
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.cortex/profiles/mybot/subdir should NOT match — too deep."""
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CORTEX_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_cortex_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".hermes" / "athena-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".hermes" / "athena-agent")
    ...
    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        ...
        original = str(tmp_path / "alice" / ".hermes" / "athena-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".hermes" / "athena-agent"
        ...
        monkeypatch.setenv("CORTEX_HOME", str(root_home / ".hermes"))
        monkeypatch.setattr(gateway_cli, "get_cortex_home", lambda: root_home / ".hermes")
        ...
        # WorkingDirectory is anchored at the target user's CORTEX_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.hermes/athena-agent" not in unit
...
class TestLegacyAthenaUnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m cortex_cli.main gateway run --replace\n"
    )
    ...
    def test_ignores_unrelated_hermes_service(self, tmp_path, monkeypatch):
        ...
        If a user has some other package named ``athena`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "hermes.service").write_text(
            "[Unit]\nDescription=Some Other Athena\n[Service]\n"
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m cortex_cli.main gateway run
          - python path/to/cortex_cli/main.py gateway run
          - hermes gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m cortex_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/cortex_cli/main.py gateway run",
...
class TestRemoveLegacyAthenaUnits:
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        if project_root is None:
            import cortex_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "cortex_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "cortex_cli.main", "gateway", "status", "-l", "--help"],
...
class TestServiceWorkingDirIsStable:
    ...
    (CORTEX_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """

    def test_stable_working_dir_uses_cortex_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_cortex_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # CORTEX_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".hermes"
        monkeypatch.setattr(gateway_cli, "get_cortex_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_cortex_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_cortex_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_cortex_home(self, tmp_path, monkeypatch):
        import re

...
`CORTEX_REDACT_SECRETS` at module-import time. `cortex_cli/main.py` at
...
Fix: bridge `security.redact_secrets` from config.yaml → `CORTEX_REDACT_SECRETS`
env var in `cortex_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports cortex_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("CORTEX_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import cortex_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('CORTEX_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["CORTEX_HOME"] = str(cortex_home)
    env.pop("CORTEX_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env CORTEX_REDACT_SECRETS takes precedence over config.yaml."""
    cortex_home = tmp_path / ".hermes"
    cortex_home.mkdir()
    (cortex_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (cortex_home / ".env").write_text("CORTEX_REDACT_SECRETS=true\n")

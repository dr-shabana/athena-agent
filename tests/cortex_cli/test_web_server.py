"""Tests for cortex_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_cortex_home):
    """Drop the example-dashboard fixture into the per-test CORTEX_HOME
    ...
    from cortex_cli import web_server
    ...
    #      and start serving requests against a torn-down CORTEX_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Athena vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Athena env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import cortex_cli.web_server as ws
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_cortex_home):
        """Create a TestClient and isolate the state DB under the test CORTEX_HOME."""
        try:
        ...
        import cortex_state
        from cortex_constants import get_cortex_home
        from cortex_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(cortex_state, "DEFAULT_DB_PATH", get_cortex_home() / "state.db")
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("cortex_state.SessionDB", _FakeDB)
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from cortex_cli.web_server import app
    ...
    def test_elevenlabs_voices_unavailable_without_key(self, monkeypatch):
        import cortex_cli.web_server as web_server
    ...
    def test_update_hermes_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        assert "docker pull nousresearch/athena-agent:latest" in data["message"]
        ...
        assert any("docker pull nousresearch/athena-agent:latest" in line for line in status_data["lines"])
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from cortex_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from cortex_cli.web_server import (
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "cortex_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Athena provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from cortex_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from cortex_cli.config import save_env_value
        from cortex_cli.web_server import _SESSION_TOKEN
    ...
    def test_update_messaging_platform_saves_env_and_enablement(self):
        from cortex_cli.config import load_config, load_env
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Athena Agent"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("AthenaDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "deep_link": "https://t.me/newbot/AthenaSetupBot/hermes_pair123_bot",
                "qr_payload": "https://t.me/newbot/AthenaSetupBot/hermes_pair123_bot",
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "deep_link": "https://t.me/newbot/AthenaSetupBot/hermes_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/AthenaSetupBot/hermes_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "deep_link": "https://t.me/newbot/AthenaSetupBot/hermes_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/AthenaSetupBot/hermes_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "deep_link": "https://t.me/newbot/AthenaSetupBot/hermes_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/AthenaSetupBot/hermes_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "deep_link": "https://t.me/newbot/AthenaSetupBot/hermes_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/AthenaSetupBot/hermes_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "deep_link": "https://t.me/newbot/AthenaSetupBot/hermes_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/AthenaSetupBot/hermes_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/cortex_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import cortex_cli.nous_subscription as ns
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from cortex_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from cortex_cli.web_server import _parse_model_ids
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        import cortex_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from cortex_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from cortex_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_cortex_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_list_includes_default(self):
        from cortex_constants import get_cortex_home
        get_cortex_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from cortex_constants import get_cortex_home
        import cortex_cli.profiles as profiles_mod

        cortex_home = get_cortex_home()
        cortex_home.mkdir(parents=True, exist_ok=True)
        (cortex_home / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  name: anthropic/claude-sonnet-4.6\n",
            encoding="utf-8",
        )
        named = cortex_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_cortex_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        default_skill = get_cortex_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = get_cortex_home() / "profiles" / "cloned" / "skills" / "custom" / "new-skill" / "SKILL.md"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_cortex_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_cortex_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_cortex_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_cortex_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_cortex_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_cortex_home() / "profiles" / "builder"
        token = set_cortex_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_cortex_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import cortex_cli.skills_config as skills_config
        import cortex_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import cortex_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from cortex_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from cortex_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from cortex_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.cortex/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``cortex_cli/web_server.py``).
    ...
    def _setup_test_client(self, monkeypatch, _isolate_cortex_home):
        ...
        monkeypatch.setattr(
            cortex_state, "DEFAULT_DB_PATH", get_cortex_home() / "state.db"
        )
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in cortex_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_cortex_home):
        ...
        # Pin the SessionDB to the isolated CORTEX_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_cortex_home, _install_example_plugin):
        ...
        installs it into the per-test ``CORTEX_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into CORTEX_HOME by the class-level
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import cortex_cli.main as main_mod
    ...
    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_cortex_home
    ):
        import cortex_cli.main as main_mod

        config_path = Path(os.environ["CORTEX_HOME"]) / "config.yaml"
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from cortex_cli.pty_bridge import PtyUnavailableError
        ...
        import cortex_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from cortex_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import cortex_cli.main as cli_main
    import cortex_cli.web_server as ws
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_cortex_home, _install_example_plugin):
        ...
        the per-test ``CORTEX_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_cortex_home):
        import threading
    ...
    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_cortex_home):
        import threading
        import cron.scheduler as sched

"""Tests for the Kanban DB layer (cortex_cli.kanban_db)."""
...
from cortex_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated CORTEX_HOME with an empty kanban DB."""
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import cortex_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from cortex_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Athena profile — preserves the
...
def test_scratch_workspace_created_under_cortex_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Athena-managed scratch dir should be cleaned up"
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Athena' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Athena' DB/metadata/logs on task completion.
    """
...
# where `kanban_db_path()` resolved to the active profile's CORTEX_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's CORTEX_HOME."""

    def _set_home(self, monkeypatch, tmp_path, cortex_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_default_install_anchors_at_home_dot_hermes(
        ...
        # Standard install: CORTEX_HOME == ~/.hermes, no profile active.
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.cortex/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.cortex/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.cortex/kanban.db.
        default_home = tmp_path / ".hermes"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # CORTEX_HOME and confirm equality. This is the property the
        ...
        monkeypatch.setenv("CORTEX_HOME", str(profile_home))
    ...
    def test_docker_custom_cortex_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: CORTEX_HOME points outside ~/.hermes.
        # `get_default_hermes_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".hermes"`.
        custom_root = tmp_path / "opt" / "athena"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: CORTEX_HOME=/opt/hermes/profiles/coder;
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CORTEX_HOME", str(default_home))
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # CORTEX_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile CORTEX_HOME and reads.
        monkeypatch.setenv("CORTEX_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # `-p <profile>` flag rewrites CORTEX_HOME.
        default_home = tmp_path / ".hermes"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        assert env["CORTEX_KANBAN_TASK"] == "t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see cortex_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("cortex_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="cortex_state"):
            conn = kb.connect()
...
#
# Workers spawned by the dispatcher must use a `athena` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["athena", ...]`
...
def test_resolve_hermes_argv_prefers_path_shim(monkeypatch):
    """When `athena` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import cortex_cli.kanban_db as kb
...
def test_resolve_hermes_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    assert kb._resolve_hermes_argv() == [sys.executable, "-m", "cortex_cli.main"]
...
def test_resolve_hermes_argv_hermes_bin_bare_name_uses_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HERMES_BIN", "athena")
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m cortex_cli.main`.

    Pins the correct module name (NOT `athena` — there is no top-level
    `athena` package). Regression for #23198: the original PR shipped
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_hermes_argv()
    assert argv == [sys.executable, "-m", "cortex_cli.main"]
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    sufficient — if `cortex_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m cortex_cli.main --version`
    ...
    assert "Athena Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="cortex_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from cortex_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("cortex_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from cortex_cli.kanban_db import connect, write_txn
    import cortex_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from cortex_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from cortex_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("cortex_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("cortex_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("cortex_cli.kanban_db.os.name", "nt")
    with patch("cortex_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("cortex_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("cortex_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "cortex_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("cortex_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "cortex_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("cortex_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("cortex_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("cortex_cli.kanban_db._record_worker_exit"):
            with patch("cortex_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()

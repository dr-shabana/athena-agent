...
files. In Athena the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.cortex/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → AthenaCLI → AIAgent call chain.
"""
...
class TestIgnoreUserConfigEnvGate:
    ...
    When the env var is set, user config at ``<cortex_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._cortex_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_cortex_home", tmp_path)
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``AthenaCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting HERMES_IGNORE_RULES=1 flips AthenaCLI.ignore_rules True."""
        monkeypatch.setenv("HERMES_IGNORE_RULES", "1")

        # Import AthenaCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of AthenaCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.AthenaCLI)
        # Replicate the exact logic from cli.py AthenaCLI.__init__:
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``cortex_cli/main.py`` must set

...
from cortex_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from cortex_cli.auth import _resolve_verify
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="cortex_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from cortex_cli.auth import get_nous_auth_status, invalidate_nous_auth_status_cache
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    cortex_home = tmp_path / "athena"
    _setup_nous_auth(cortex_home, access_token="at-123")
    monkeypatch.setenv("CORTEX_HOME", str(cortex_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    cortex_home = tmp_path / "athena"
    _setup_nous_auth(cortex_home, access_token="at-fresh")
    monkeypatch.setenv("CORTEX_HOME", str(cortex_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("cortex_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_refreshed_jwt_lacks_invoke_scope(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("cortex_cli.auth._refresh_access_token", _fake_refresh_access_token)
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    from cortex_cli import auth as auth_mod
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    cortex_home = tmp_path / "athena"
    _setup_nous_auth(cortex_home, refresh_token="refresh-old")
    monkeypatch.setenv("CORTEX_HOME", str(cortex_home))
    from agent.credential_pool import load_pool
...
class TestLoginNousSkipKeepsCurrent:
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return cortex_home, config_path, auth_path
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from cortex_cli.auth import PROVIDER_REGISTRY, _login_nous

        cortex_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    """
    from cortex_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Athena is not logged into Nous Portal").
    ...
    # doesn't raise "Athena is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Athena must surface an
    ...
    re-authenticate" string led users to report this as a Athena persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from cortex_cli.auth import _refresh_access_token
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    writing to the user's real ``~/.cortex/shared/`` across CI runs.
    """
    from cortex_cli.auth import _nous_shared_store_path
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    ...
    from cortex_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from cortex_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from cortex_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from cortex_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    cortex_home = tmp_path / "athena"
    cortex_home.mkdir(parents=True, exist_ok=True)
    (cortex_home / "auth.json").write_text(
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from cortex_cli.auth import _try_import_shared_nous_state
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different CORTEX_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("CORTEX_HOME", str(profile_a))
    ...
    # Profile B: fresh CORTEX_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("CORTEX_HOME", str(profile_b))

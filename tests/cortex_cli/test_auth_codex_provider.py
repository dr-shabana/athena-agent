"""Tests for Codex auth — tokens stored in Athena auth store (~/.cortex/auth.json)."""
...
def _setup_hermes_auth(cortex_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Athena auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(cortex_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(cortex_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("CORTEX_HOME", str(cortex_home))
    ...
    monkeypatch.setattr("cortex_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    cortex_home = tmp_path / "athena"
    _setup_hermes_auth(cortex_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("CORTEX_HOME", str(cortex_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((cortex_home / "auth.json").read_text())
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Athena auth store, not ~/.codex/."""
    cortex_home = tmp_path / "athena"
    codex_home = tmp_path / "codex-cli"
    cortex_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Athena store
    assert not (codex_home / "auth.json").exists()

    # Athena auth store should have the tokens
    data = _read_codex_tokens()
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from cortex_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from cortex_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "cortex_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "cortex_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("cortex_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("cortex_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",

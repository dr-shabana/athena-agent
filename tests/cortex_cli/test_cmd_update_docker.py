...
host-side Athena, not an update to the running container, so the message
...
from cortex_cli.main import _cmd_update_check, cmd_update
...
@patch("cortex_cli.config.detect_install_method", return_value="docker")
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    assert "docker pull nousresearch/athena-agent:latest" in out
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull nousresearch/athena-agent:latest" in msg
    ...
    assert "CORTEX_HOME" in msg or "/opt/data" in msg, (

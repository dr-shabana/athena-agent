...
import cortex_cli.doctor as doctor
import cortex_cli.gateway as gateway_cli
from cortex_cli import doctor as doctor_mod
from cortex_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "CORTEX_HOME", cortex_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("CORTEX_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_cortex_home(self, tmp_path, provider=""):
        """Create a minimal CORTEX_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_cortex_home(tmp_path, provider)
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from cortex_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("cortex_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "cortex_cli.doctor", raising=False)
        from cortex_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        (cortex_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "CORTEX_HOME", cortex_home)
        monkeypatch.setattr(doctor_mod, "get_cortex_home", lambda: cortex_home)
        ...
        return buf.getvalue(), cortex_home

    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        out, cortex_home = self._run_config_section(
        ...
        assert "HERMES_MAX_ITERATIONS=90" in (cortex_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        env_after = (cortex_home / ".env").read_text(encoding="utf-8")
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    ...
    # left thinking a crashing command means a broken Athena install.

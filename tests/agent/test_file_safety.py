...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_hermes_env(self):
        """Athena' own .env inside CORTEX_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: hermes internal .env is in ~/.cortex/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.cortex/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.cortex/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        cache = cortex_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._cortex_home_path", return_value=cortex_home):
    ...
    def test_hub_directory_blocked(self, tmp_path):
        ...
        cortex_home = tmp_path / ".hermes"
        hub = cortex_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_cortex_home(self, tmp_path):
        """The env basename guard does not depend on CORTEX_HOME resolution."""
        cortex_home = tmp_path / ".hermes"
        cortex_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        cortex_home = tmp_path / ".hermes"
        cache = cortex_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")

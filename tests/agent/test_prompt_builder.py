...
class TestBuildNousSubscriptionPrompt:
    def test_includes_active_subscription_features(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "cortex_cli.nous_subscription.get_nous_subscription_features",
...
class TestBuildContextFilesPrompt:
    ...
    def test_loads_soul_md_from_cortex_home_only(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CORTEX_HOME", str(tmp_path / "cortex_home"))
        cortex_home = tmp_path / "cortex_home"
        cortex_home.mkdir()
        (cortex_home / "SOUL.md").write_text("Be concise and friendly.", encoding="utf-8")
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        ...
        cortex_home = tmp_path / "cortex_home"
        cortex_home.mkdir()
        (cortex_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
    ...
    def test_hermes_md_beats_agents_md(self, tmp_path):
        ...
        (tmp_path / ".hermes.md").write_text("Athena project rules.")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Athena project rules" in result
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_empty_by_default(self, monkeypatch):
        ...
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.delenv("HERMES_ENVIRONMENT_HINT", raising=False)
        monkeypatch.setattr("cortex_cli.config.load_config", lambda: {"agent": {}})
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()

...
engine plugins (e.g. athena-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    ...
    from cortex_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "cortex_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "cortex_cli.plugins.get_plugin_context_engine",

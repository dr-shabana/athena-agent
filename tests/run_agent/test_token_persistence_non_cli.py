...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    cortex_state = ModuleType("cortex_state")
    cortex_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "cortex_state", cortex_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Athena"}, "task-id"))

...
Browser-based dashboard for managing Athena Agent configuration, API keys, and monitoring active sessions.
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `athena-agent/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Athena Agent” — mixed case, no Mondwest, no `text-display` |

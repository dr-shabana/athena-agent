...
Athena pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.cortex/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<cortex_home>/bin/bws`` on
  first use.  Athena pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.cortex/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``athena`` invocations don't
  hammer the API.
* Failures NEVER block Athena startup.  Missing binary, no network,
...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `athena chat -q ...`
...
# <cortex_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.cortex/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under cortex_home/cache/.

    `home_path` is what `load_hermes_dotenv()` already resolved; falling back
    to `$CORTEX_HOME` / `~/.cortex` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("CORTEX_HOME", Path.home() / ".hermes"))
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<cortex_home>/bin/bws``  (our managed copy — preferred)
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``athena secrets bitwarden setup``
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<cortex_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from cortex_cli.env_loader
# ---------------------------------------------------------------------------

...
def _state_file() -> Path:
    return get_cortex_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.cortex/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `athena update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``athena update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``athena curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `athena curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    ...
    "into ~/.cortex/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.cortex/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.cortex/logs/curator/``)
    ...
    authored skill data in ``~/.cortex/skills/``.

    ``ensure_cortex_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_cortex_home() / "logs" / "curator"
    try:
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: athena curator status
        keep an umbrella stable: athena curator pin document-tools
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: athena curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.cortex/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.cortex/skills/.archive/` for "
            "safety and can be restored via `athena curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.cortex/skills/.archive/`. "
            "Restore any via `athena curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `athena curator restore <name>`")
    lines.append("- All archives live under `~/.cortex/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `athena curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `athena curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``athena model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `athena model` → auxiliary

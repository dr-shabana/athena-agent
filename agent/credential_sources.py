"""Unified removal contract for every credential source Athena reads from.

Athena seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.cortex/.env
    claude_code   — ~/.claude/.credentials.json
    hermes_pkce   — ~/.cortex/.anthropic_oauth.json
    ...
    manual        — user ran `athena auth add`
...
unify here is **removal**:
    ...
    ``athena auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.cortex/.env  → clear it
    ...
    """
    from cortex_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Athena.",
            f"  The pool entry is now suppressed — Athena will ignore "
            f"{env_var} until you run `athena auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Athena stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `athena auth add anthropic` to re-enable if needed.",
    ])


def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    """~/.cortex/.anthropic_oauth.json is ours — delete it outright."""
    from cortex_constants import get_cortex_home

    result = RemovalResult()
    oauth_file = get_cortex_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Athena Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `athena auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `athena auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``athena auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``athena auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `athena model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Athena auth store is not enough — _seed_from_singletons() would
    ...
    via ``athena auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `athena auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `athena auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Athena stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `athena auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="~/.cortex/.anthropic_oauth.json",
    ))
    register(RemovalStep(

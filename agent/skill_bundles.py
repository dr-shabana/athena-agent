...
Storage
-------
Bundles live in ``~/.cortex/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CORTEX_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``athena bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CORTEX_HOME.

    Honors ``HERMES_BUNDLES_DIR`` for tests; falls back to
    ``<CORTEX_HOME>/skill-bundles``.
    """
    override = os.environ.get("HERMES_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_cortex_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `athena bundles` CLI subcommand.
# ---------------------------------------------------------------------------

...
# Athena' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Athena/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Athena session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Athena' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Athena' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Athena' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Athena approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Athena returns 'once', 'session', 'always', or 'deny'.

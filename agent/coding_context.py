"""Coding-context awareness — base Athena, every interactive surface.

When the user runs Athena inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Athena shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("cortex.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "athena", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
def is_coding_context(
    *,
    ...
    """Whether Athena should operate in its coding posture right now."""

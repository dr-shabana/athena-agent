...
from cortex_constants import get_cortex_home, get_skills_dir, is_wsl
...
HERMES_AGENT_HELP_GUIDANCE = (
    ...
    "information. Load the `athena-agent` skill with skill_view(name='athena-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.cortex/kanban.db`. Your task id is in "
    "`$CORTEX_KANBAN_TASK`; your workspace is `$HERMES_KANBAN_WORKSPACE`. "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `athena kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Athena "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Athena WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Athena itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Athena runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Athena itself is running. The host OS, home, and cwd "
                f"of the Athena process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Athena "
    ...
    # Athena desktop GUI — any agent running under the desktop app should know
    ...
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Athena desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Athena
...
def _skills_prompt_snapshot_path() -> Path:
    return get_cortex_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.cortex/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Athena Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `athena-agent` skill "
            "first. It has the actual commands (e.g. `athena config set …`, `athena tools`, "
            "`athena setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: athena setup, athena setup tools, athena setup terminal, hermes status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from CORTEX_HOME and return its content, or None.
    ...
    """
    try:
        from cortex_cli.config import ensure_cortex_home
        ensure_cortex_home()
    except Exception as e:
        logger.debug("Could not ensure CORTEX_HOME before loading SOUL.md: %s", e)

    soul_path = get_cortex_home() / "SOUL.md"
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    SOUL.md from CORTEX_HOME is independent and always included when present.
    ...
    # SOUL.md from CORTEX_HOME only — skip when already loaded as identity
    if not skip_soul:

...
from cortex_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.cortex/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from cortex_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.cortex/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from cortex_logging import setup_logging, setup_verbose_logging
    setup_logging(cortex_home=_ra()._cortex_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that cortex_logging.setup_logging() does not
        ...
        # handler level inside cortex_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from cortex_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from cortex_cli.config import load_config as _load_br_cfg
    ...
    # worker (kanban_show tool is present iff CORTEX_KANBAN_TASK is set).
    ...
    # Session logs go into ~/.cortex/sessions/ alongside gateway sessions
    cortex_home = get_cortex_home()
    agent.logs_dir = cortex_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.cortex/sessions/session_{sid}.json)
    ...
    try:
        from cortex_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from cortex_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "athena"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from cortex_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from cortex_cli.plugins import get_plugin_context_engine

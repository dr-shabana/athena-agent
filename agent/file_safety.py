...
def _cortex_home_path() -> Path:
    """Resolve the active CORTEX_HOME (profile-aware) without circular imports."""
    try:
        from cortex_constants import get_cortex_home  # local import to avoid cycles
        return get_cortex_home()
    except Exception:
        return Path(os.path.expanduser("~/.cortex"))


def _hermes_root_path() -> Path:
    """Resolve the Athena root dir (always the parent of any profile, never per-profile)."""
    try:
        from cortex_constants import get_default_hermes_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    return {
        ...
        for p in [
            ...
            str(cortex_home / ".anthropic_oauth.json"),
...
def is_write_denied(path: str) -> bool:
    ...
    # Athena control-plane files: block both the ACTIVE profile's view
    # (cortex_home) AND the global root view. Without the root pass, a
    ...
    hermes_dirs = []
    for base in (_cortex_home_path(), _hermes_root_path()):
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal Athena cache files under ``CORTEX_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under CORTEX_HOME and the global Athena
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.cortex/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active CORTEX_HOME (profile-aware) AND the global
    # Athena root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (CORTEX_HOME points at
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Athena cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Athena credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Athena MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Athena MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Athena profiles are separate CORTEX_HOME dirs under
...
# edited skills under both ``~/.cortex/profiles/hermes-security/skills/``
# AND ``~/.cortex/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under CORTEX_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from CORTEX_HOME.
    ...
    """
    try:
        home_real = _cortex_home_path().resolve()
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Athena entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Athena profile {info['target_profile']!r}, but the "
...
#
#   <CORTEX_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.hermes/...
#
...
# regardless of which Athena profile is active. It does NOT cover the
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    ...
    index where the inner Athena-state portion starts. Returns ``None`` for
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Athena state.
    ...
    Detection is path-shape-only — does not require any Athena resolver to
    succeed, so it works correctly even when called from contexts where
    CORTEX_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Athena process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real CORTEX_HOME. Use the host-side tool for "
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host CORTEX_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Athena state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Athena "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real CORTEX_HOME. Use the "

...
def compress_context(
    ...
    *,
    ...
    # long-lived ``cortex_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `athena update`) to resync.",
    ...
    if agent._session_db:
        try:
            ...
            # from ``cortex_logging._session_context`` (set once per turn in
    ...
    # compression (not a fresh /new). Plugin engines (e.g. athena-lcm) use
    # boundary_reason="compression" to preserve DAG lineage across the
    # rollover instead of re-initializing fresh per-session state.
    # See athena-lcm#68. Built-in ContextCompressor ignores kwargs.
    try:

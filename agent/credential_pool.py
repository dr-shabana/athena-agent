...
from cortex_cli.config import load_env
from agent.credential_persistence import (
    is_borrowed_credential_source,
    sanitize_borrowed_credential_payload,
)
import cortex_cli.auth as auth_mod
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``athena model`` / ``athena auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Athena process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``athena`` invocation that defaults to the active provider
        (e.g. setup wizard, ``athena auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another Athena
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Athena process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Athena profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `athena model` / `athena auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `athena model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``athena auth add``.  Singleton-
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `athena auth remove <provider> <N>` is stable across all source types.
    try:
        from cortex_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Athena PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `athena setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.cortex/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `athena setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `athena auth add nous --label <name>`).  Fall back to the
        ...
        # Athena auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from cortex_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.cortex/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `athena auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `athena auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Athena auth store.  Without this gate the removal is instantly
        ...
        # Athena owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``athena model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``athena auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from cortex_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.cortex/.env over os.environ — the user's config file is the
    # authoritative source for Athena credentials. Stale env vars from parent
    ...
    # Honour user suppression — `athena auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.cortex/.env.
    ...
    try:
        from cortex_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from cortex_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.cortex/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            # Athena PKCE is Athena-owned/persistable while present, but it is

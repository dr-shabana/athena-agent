...
upstream prefix cache warm.  See ``athena-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Athena never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want CORTEX_HOME persona while keeping
    ...
    # Pointer to the athena-agent skill + docs for user questions about Athena itself.
    ...
    # CORTEX_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Athena, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Athena profile the agent is running
    # under so it doesn't conflate ~/.cortex/skills/ (default profile) with
    # ~/.cortex/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Athena profile: default. Other profiles (if any) live "
            "under ~/.cortex/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Athena profile: {active_profile}. This session reads "
            f"and writes ~/.cortex/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.cortex/skills/, ~/.cortex/plugins/, "
            f"~/.cortex/cron/, ~/.cortex/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Athena never rebuilds or reinjects parts of it

...
as ``kind: backend``) or ``~/.cortex/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``athena tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$CORTEX_HOME/cache/videos/``, creating parents as needed."""
    from cortex_constants import get_cortex_home

    path = get_cortex_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$CORTEX_HOME/cache/videos/``.

...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Athena with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.cortex/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Athena
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``athena tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``athena setup`` for

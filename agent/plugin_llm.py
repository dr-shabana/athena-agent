...
Plugins built on Athena Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Athena
...
:class:`~cortex_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Athena' main config
...
handles every provider, fallback chain, and per-task override Athena
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`cortex_cli.plugins.PluginContext`

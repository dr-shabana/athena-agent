...
from cortex_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``CORTEX_KANBAN_TASK`` / ``CORTEX_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("CORTEX_KANBAN_TASK") or os.getenv("CORTEX_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Athena Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
# which becomes the dominant cost of ``athena`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.cortex/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``athena`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against CORTEX_HOME, not cwd
        if not p.is_absolute():
            p = (cortex_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.cortex/skills/`` first, then external.
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Athena metadata, VCS, virtualenv/dependency, and cache

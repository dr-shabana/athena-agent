...
``auth.json``.  They intentionally have no dependency on ``cortex_cli.auth`` so
...
# Sources Athena owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Athena-owned OAuth/device-code state)

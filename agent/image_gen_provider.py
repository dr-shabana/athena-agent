...
as ``kind: backend``) or ``~/.cortex/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``athena tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``athena tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$CORTEX_HOME/cache/images/``, creating parents as needed."""
    from cortex_constants import get_cortex_home

    path = get_cortex_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$CORTEX_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$CORTEX_HOME/cache/images/``.

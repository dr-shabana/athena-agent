...
The PID file lives at ``{CORTEX_HOME}/gateway.pid``.  CORTEX_HOME defaults to
``~/.cortex`` but can be overridden via the environment variable.  This means
separate CORTEX_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting CORTEX_HOME."""
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Athena gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different CORTEX_HOME dirs).
    """
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``athena gateway stop`` on Windows would be misclassified as an

...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_cortex_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Athena does not currently need to route them into the agent loop.
            ...
            # manifest (see `athena slack manifest`). In Socket Mode, Slack
            ...
            from cortex_cli.commands import slack_native_slashes
            import re as _re
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Athena handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Athena session — matching the per-thread behavior channels already
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        if slash_name in {"athena", ""}:
            # Legacy /hermes <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from cortex_cli.commands import slack_subcommand_map

...
# Audio file extensions Athena recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Athena' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Athena-created Telegram private-chat
...
from cortex_constants import get_default_hermes_root, get_hermes_dir, get_cortex_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.cortex/.env manually."
)
...
# Default location: {CORTEX_HOME}/cache/images/ (legacy: image_cache/)
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _CORTEX_HOME / "image_cache",
    _CORTEX_HOME / "audio_cache",
    _CORTEX_HOME / "video_cache",
    _CORTEX_HOME / "document_cache",
    _CORTEX_HOME / "browser_screenshots",
    ...
    _CORTEX_HOME / "cache" / "images",
    _CORTEX_HOME / "cache" / "audio",
    _CORTEX_HOME / "cache" / "videos",
    _CORTEX_HOME / "cache" / "documents",
    _CORTEX_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Athena profile and shared Athena root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for hermes_root in (_CORTEX_HOME, _HERMES_ROOT):
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Athena
    secrets (``~/.cortex/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    Athena-managed cache, under an operator-allowlisted root
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.cortex/.env,
    # ~/.cortex/auth.json, etc. — so the obvious prompt-injection sites
...
class BasePlatformAdapter(ABC):
    ...
    # Athena commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from cortex_cli.commands import should_bypass_active_session

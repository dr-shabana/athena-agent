...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    # Default bridge location relative to the athena-agent install
    ...
    async def connect(self) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `athena gateway`.",
                retryable=False,
            )
            return False
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Run `athena whatsapp` to pair, or remove WHATSAPP_ENABLED from "
                "your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — run `athena whatsapp` to pair.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `athena update` bumps the Baileys pin).  The stamp file records
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `athena update`, so without this check it
            ...
            # hardcodes ~/.cortex/{image,audio,document}_cache, which diverges
            # under CORTEX_HOME overrides, profiles, and the new cache/ layout.
            from gateway.platforms.base import (

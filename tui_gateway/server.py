...
load_hermes_dotenv(
    cortex_home=_cortex_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.cortex/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_cortex_home, "logs", "tui_gateway_crash.log")
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `athena --tui` keeps real _stdio.
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `athena --tui`, so it must NOT count as dead here (doing
...
# ``profile`` on those calls; we open that profile's db and bind its CORTEX_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_cortex_home).resolve():
        return None
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `athena` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # CORTEX_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_cortex_home_override(profile_home)
                try:
                    from cortex_state import SessionDB
            ...
            if home_token is not None:
                reset_cortex_home_override(home_token)
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _cortex_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_cortex_home_override()
        home = override if isinstance(override, str) and override else _cortex_home
...
def resolve_skin() -> dict:
    try:
        from cortex_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from cortex_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    # Coding posture (base Athena): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `athena --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from cortex_cli.tools_config import _parse_enabled_flag
...
def _persist_model_switch(result) -> None:
    from cortex_cli.config import save_config
...
def _apply_model_switch(
    ...
    *,
    ...
    from cortex_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from cortex_cli.config import get_compatible_custom_providers, load_config
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from cortex_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from cortex_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from cortex_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from cortex_cli.config import load_config as _load_full_cfg
...
def _load_fallback_model():
    ...
    stays in parity with ``AthenaCLI.__init__`` and ``gateway/run.py``:
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses cortex_cli.mcp_startup; TUI stdio keeps
    ...
    if isinstance(model_override, dict) and model_override.get("model"):
        ...
        if (
            ...
            from cortex_cli.runtime_provider import find_custom_provider_identity
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind CORTEX_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_cortex_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds CORTEX_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``athena --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``athena gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Athena profile home
    # (~/.cortex/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $CORTEX_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_cortex_home() / "spawn-trees"
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn CORTEX_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_cortex_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from cortex_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from cortex_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: cortex_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from cortex_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _cortex_home / "images"
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("CORTEX_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from cortex_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_cortex_home), "display": display_cortex_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from cortex_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from cortex_cli.runtime_provider import resolve_runtime_provider
        from cortex_cli.auth import has_usable_secret
        from cortex_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Athena provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.cortex/.env`` into the gateway process via
    ``cortex_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `athena` is interactive — use `/hermes chat -q …` or run `athena` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`athena setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`athena gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`athena sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`athena config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m cortex_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "cortex_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from cortex_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from cortex_cli.config import is_managed, save_env_value
        from cortex_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `athena model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.cortex/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from cortex_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from cortex_cli.config import remove_env_value
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from cortex_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from cortex_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from cortex_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from cortex_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from cortex_cli.browser_connect import try_launch_chrome_debug
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_cortex_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from cortex_cli.skills_hub import do_install
        ...
        if action == "browse":
            from cortex_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from cortex_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``athena plugins`` / the dashboard, so the three surfaces

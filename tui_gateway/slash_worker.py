"""Persistent slash-command worker — one AthenaCLI per TUI session.
...
def _run(cli: AthenaCLI, command: str) -> str:
...
def main():
    ...
    # Start before the (hundreds-of-ms) AthenaCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = AthenaCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)

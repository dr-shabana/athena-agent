...
version: 1.0.0
author: arceus77-7, enhanced by Athena Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Athena terminal calls (desktop app flow only)
...
### Service Account (recommended for Athena)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${CORTEX_HOME:-~/.hermes}/.env` (the skill will prompt for this on first load).
...
## Athena Execution Pattern (desktop app flow)

Athena terminal commands are non-interactive by default and can lose auth context between calls.

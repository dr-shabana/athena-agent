...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CORTEX_HOME", os.path.expanduser("~/.cortex")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CORTEX_HOME", Path.home() / ".hermes")) / "skills" / "red-teaming" / "godmode"
...
CORTEX_HOME = Path(os.getenv("CORTEX_HOME", Path.home() / ".hermes"))
CONFIG_PATH = CORTEX_HOME / "config.yaml"
PREFILL_PATH = CORTEX_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Athena models — already uncensored, just needs clean prompt
    "athena": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "athena" in model_lower or "nous" in model_lower:
        return "athena"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Athena config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.cortex/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Athena for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Athena for changes to take effect.")

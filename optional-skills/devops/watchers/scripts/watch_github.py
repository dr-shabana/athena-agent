...
Usage (via cron with --no-agent):
    ...
    hermes cron create hermes-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$CORTEX_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hermes-issues --repo dr-shabana/athena-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Athena .env file
(``${CORTEX_HOME:-~/.cortex}/.env``) to avoid the 60 req/hr

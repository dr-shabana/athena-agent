...
- smoke-test a server with the FastMCP CLI before wiring it into Athena or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Athena. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.cortex/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.cortex/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Athena integration, either:

- configure the server in `~/.cortex/config.yaml` using the `native-mcp` skill, or
...
### Athena cannot see the deployed server

The server-building part may be correct while the Athena config is not. Load the `native-mcp` skill and configure the server in `~/.cortex/config.yaml`, then restart Athena.

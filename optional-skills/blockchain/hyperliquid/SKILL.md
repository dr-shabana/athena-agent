...
version: 0.1.0
author: Hugo Sequier (Hugo-SEQUIER), Athena Agent
license: MIT
...
The script reads `${CORTEX_HOME:-~/.hermes}/.env` for two optional defaults:
...
Helper script: `~/.cortex/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${CORTEX_HOME:-~/.hermes}/.env`.

...
Maps each role archetype to the Athena skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public athena-agent
...
## Athena skills relevant to video production

### Visual / rendering skills (`athena-agent/skills/creative/`)
...
### Design / pre-production skills (`athena-agent/skills/creative/`)
...
### Audio / media skills (`athena-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`athena-agent/skills/devops/`)
...
These are **not** Athena skills but external CLIs / APIs that profiles invoke.
...
## Built-in Athena tools for media review

These are native Athena tools — not invoked via terminal but through their own
...
key is present in `${CORTEX_HOME:-~/.hermes}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Athena profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${CORTEX_HOME:-~/.hermes}/.env` → environment variable.
...
When a new Athena-public video skill ships:

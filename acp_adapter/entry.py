"""CLI entry point for the athena-agent ACP adapter.

Loads environment variables from ``~/.cortex/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from CORTEX_HOME (default ``~/.cortex``)."""
    from cortex_cli.env_loader import load_hermes_dotenv

    cortex_home = get_cortex_home()
    loaded = load_hermes_dotenv(cortex_home=cortex_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", cortex_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="athena-acp",
        description="Run Athena Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Athena version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Athena provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.cortex/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import AthenaACPAgent  # noqa: F401

    print("Athena ACP check OK")


def _run_setup() -> None:
    from cortex_cli.main import main as hermes_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "athena", "model"]
        hermes_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``athena postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting athena-agent ACP adapter")
    ...
    import acp
    from .server import AthenaACPAgent

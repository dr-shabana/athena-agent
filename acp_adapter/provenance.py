...
This is an additive Athena extension surfaced under ACP ``_meta.hermes`` so
...
compression rotates the internal Athena head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    Args:
        ...
        current_hermes_session_id: The live internal Athena DB session id
    ...
    Returns:
        A dict suitable for ``{"athena": {"sessionProvenance": <dict>}}`` under
    ...
    provenance: Dict[str, Any] = {
        ...
        "currentAthenaSessionId": current_hermes_session_id,
        "rootAthenaSessionId": root_id,
        "parentAthenaSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_hermes_session_id:
        provenance["previousAthenaSessionId"] = previous_hermes_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    if prov is None:
        return None
    return {"athena": {"sessionProvenance": prov}}

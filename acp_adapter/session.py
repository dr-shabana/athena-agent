...
Sessions are persisted to the shared SessionDB (``~/.cortex/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Athena itself is running in WSL.

    Windows ACP clients can launch ``athena acp`` inside WSL while still sending
    ...
    """
    from cortex_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Athena from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["athena-acp"]):
...
class SessionManager:
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Athena runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.cortex/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``CORTEX_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_cortex_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=cortex_home / "state.db")
            return self._db_instance
        except Exception:

...
Jobs are stored in ~/.cortex/cron/jobs.json
Output is saved to ~/.cortex/cron/output/{job_id}/{timestamp}.md
"""
...
HERMES_DIR = get_cortex_home().resolve()
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Athena configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Athena timezone.
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall athena-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.cortex/scripts/; ``.sh`` / ``.bash`` files run via bash,

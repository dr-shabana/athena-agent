...
A *suggestion* is a ready-to-run cron job spec that Athena surfaces to the
...
Storage mirrors ``cron/jobs.py``: ``~/.cortex/cron/suggestions.json``, atomic
...
CRON_DIR = get_cortex_home().resolve() / "cron"

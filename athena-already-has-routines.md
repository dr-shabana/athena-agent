# Athena Agent Has Had "Routines" Since March
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Athena implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Athena Agent |
|---|---|---|
...
## Things Athena Does That Routines Can't
...
hermes cron create "every 1h" \
  ...
  --script ~/.cortex/scripts/watch-site.py \
...
Athena has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Athena Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
pip install athena-agent
hermes setup
```
...
Full automation blueprints gallery: [athena-agent.nousresearch.com/docs/reference/automation-blueprints-catalog](https://athena-agent.nousresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [athena-agent.nousresearch.com](https://athena-agent.nousresearch.com)

GitHub: [github.com/dr-shabana/athena-agent](https://github.com/dr-shabana/athena-agent)

---

*Athena Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*

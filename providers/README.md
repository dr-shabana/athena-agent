...
Registry and ABC for every inference provider Athena knows about.
...
`$CORTEX_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
under `$CORTEX_HOME/plugins/model-providers/` for a private plugin).

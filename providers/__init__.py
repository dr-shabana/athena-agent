...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with athena-agent)
2. User plugins: ``$CORTEX_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$CORTEX_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$CORTEX_HOME/plugins/model-providers/`` if it exists."""
    try:
        from cortex_constants import get_cortex_home

        d = get_cortex_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple CORTEX_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$CORTEX_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with athena-agent.
    ...
    # 2. User plugins — under $CORTEX_HOME/plugins/model-providers/<name>/.

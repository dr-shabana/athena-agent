"""Windows UTF-8 bootstrap for Athena entry points.
...
should be imported at the very top of every Athena entry point
(``athena``, ``athena-agent``, ``athena-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import cortex_bootstrap``
# (or ``from cortex_bootstrap import apply_windows_utf8_bootstrap``) at

# OpenAI-Compatible API Server for Athena Agent
...
SSE streaming. By exposing this endpoint, athena-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  athena-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.cortex/.env`
...
4. Chat with athena-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns athena-agent as a model) |
...
But athena-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "athena-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured athena-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of athena-agent's core prompt
...
Once implemented, athena-agent works as a drop-in backend for:

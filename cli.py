#!/usr/bin/env python3
"""
Athena Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Athena Agent, inspired by Claude Code.
...
# IMPORTANT: cortex_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See cortex_bootstrap.py for full rationale.
try:
    import cortex_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when cortex_bootstrap isn't registered in the venv
    # yet — happens during partial ``athena update`` where git-reset landed
...
from cortex_cli.fallback_config import get_fallback_chain
from cortex_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from cortex_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from cortex_cli.pt_input_extras import (
...
from cortex_cli.banner import _format_context_length, format_banner_version_label
...
from cortex_cli.browser_connect import (
...
_cortex_home = get_cortex_home()
_project_env = Path(__file__).parent / '.env'
load_hermes_dotenv(cortex_home=_cortex_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.cortex/.
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from cortex_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.cortex/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If HERMES_IGNORE_USER_CONFIG=1 is set (via ``athena chat --ignore-user-config``),
    the user config at ``~/.cortex/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({CORTEX_HOME}/config.yaml)
    user_config_path = _cortex_home / 'config.yaml'
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Athena, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Athena - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see cortex_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from cortex_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and AthenaCLI.__init__ checks "default" first.
    ...
    from cortex_cli.config import _expand_env_vars
    defaults = _expand_env_vars(defaults)

    # Apply terminal config to environment variables (so terminal_tool picks them up)
    terminal_config = defaults.get("terminal", {})
    
    # Normalize config key: the new config system (cortex_cli/config.py) and all
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["CORTEX_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.cortex/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from cortex_logging import setup_logging
...
try:
    from cortex_cli.config import print_config_warnings
...
try:
    from cortex_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``athena --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from cortex_cli.mcp_startup import wait_for_mcp_discovery
...
from cortex_cli.banner import build_welcome_banner
from cortex_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from cortex_cli.callbacks import prompt_for_secret
...
def _notify_session_finalize(
    *,
    ...
    try:
        from cortex_cli.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_finalize",
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`cortex_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from cortex_cli.config import load_config as _load_full_config
        from cortex_constants import get_cortex_home as _get_cortex_home
        _cortex_home_maint = _get_cortex_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_cortex_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`cortex_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_orphaned_branches(repo_root: str) -> None:
    ...
    These are auto-generated by ``athena -w`` sessions and PR review
...
#   6. Default: assume dark (matches the legacy Athena assumption)
#
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from cortex_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from cortex_cli.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Athena output it's common to display cron schedules verbatim.
    #
...
from cortex_constants import is_termux as _is_termux_environment
...
# ASCII Art - Athena Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "Athena Agent") if _skin else "Athena Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        from cortex_cli import __release_date__ as _release_date
        from cortex_cli import __version__ as _version
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from cortex_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.cortex/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.cortex/config.yaml on first use)
...
class AthenaCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Athena Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from cortex_cli.runtime_provider import _auto_detect_local_model
        ...
        # by `athena chat --ignore-rules` in cortex_cli/main.py. When true we
        ...
        # it's shared across all Athena processes for this CORTEX_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.cortex/checkpoints/.  Opt-in via
        ...
        self._history_file = _cortex_home / ".hermes_history"
    ...
    def _claim_active_session(self, surface: str = "cli", *, stderr: bool = False) -> bool:
        ...
        if self._active_session_lease is not None:
            return True
        try:
            from cortex_cli.active_sessions import try_acquire_active_session
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from cortex_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Athena'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from cortex_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from cortex_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from cortex_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from cortex_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from cortex_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Athena")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - AthenaCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``athena doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``athena doctor`` so the banner stays
        small.
        """
        try:
            from cortex_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Athena needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous Athena LLM (not agentic)
        from cortex_cli.model_switch import is_nous_hermes_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Athena Agent 3 & 4 models are NOT agentic and are not "
                "designed for use with Athena Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.cortex/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from cortex_cli.clipboard import save_clipboard_image

        img_dir = get_cortex_home() / "images"
    ...
    def _fast_command_available(self) -> bool:
        try:
            from cortex_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from cortex_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from cortex_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Athena!{_RST}")
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from cortex_cli.main import _relative_time
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``athena sessions list``. Delegates the
        ...
        if getattr(self, "conversation_history", None):
            return False
        try:
            from cortex_constants import get_cortex_home as _ghh
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `athena sessions list` (gemini-cli#27770 port).
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.cortex/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``athena --resume <id>``
        ...
        saved_dir = get_cortex_home() / "sessions" / "saved"
        try:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from cortex_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Athena session."),
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from cortex_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `athena model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from cortex_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from cortex_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from cortex_cli.model_switch import switch_model, parse_model_flags
        from cortex_cli.providers import get_label
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from cortex_cli.commands import resolve_command
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_cortex_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_cortex_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in cortex_cli/commands.py instead of touching every dispatch site.
        from cortex_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from cortex_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from cortex_cli.skin_engine import get_active_skin
            ...
            if len(parts) > 1:
                ...
                if raw_title:
                    if self._session_db:
                        ...
                        from cortex_state import format_session_db_unavailable
            ...
            from cortex_cli.main import _print_version_info
            ...
            from cortex_cli.config import reload_env
            count = reload_env()
            ...
            try:
                # Discover from disk (bundled + user), matching `athena plugins
                ...
                # catalog lives behind `athena plugins list`.
                ...
                if not user_entries:
                    ...
                    print(f"  Or drop a plugin directory into {display_cortex_home()}/plugins/")
                    if bundled_count:
                    ...
                    loaded: dict = {}
                    try:
                        from cortex_cli.plugins import get_plugin_manager
            ...
            # Check for plugin-registered slash commands
                ...
                from cortex_cli.plugins import (
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from cortex_cli.commands import COMMANDS
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from cortex_cli.goals import GoalManager
            from cortex_cli.config import load_config
        except Exception as exc:
    ...
    def _toggle_verbose(self):
        ...
        from cortex_cli.colors import Colors as _Colors
        labels = {
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``CORTEX_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``CORTEX_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from cortex_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by cortex_logging not
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("  Run `athena portal` to log in, then /credits.")
            return
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from cortex_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.cortex/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_cortex_home / "config.yaml", TOOL_PROGRESS_FLAG)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _cortex_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard CORTEX_HOME, so no -p needed.
            try:
            ...
            try:
                from cortex_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from cortex_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from cortex_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Athena Agent! Type your message or /help for commands.")
        ...
        try:
            from cortex_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            _redact_raw = os.getenv("CORTEX_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(CORTEX_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Athena migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from cortex_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in cortex_cli.pt_input_extras
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_cortex_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Athena binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from cortex_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Athena Agent")
        ...
        # Dynamic prompt: shows Athena symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Athena needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Athena needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Athena needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = AthenaCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `athena sessions list`
...
def _run_kanban_goal_loop_q(cli: "AthenaCLI", first_response: str) -> None:
    ...
    task_id = (_os.environ.get("CORTEX_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from cortex_cli import kanban_db as _kb
    from cortex_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    if toolsets:
        ...
        # Coding posture (base Athena): with no explicit --toolsets, collapse
    ...
    # mode registers its own inside AthenaCLI.run(), but `-q` runs
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("CORTEX_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``athena chat -q "work kanban task <id>"``;
            ...
            _kanban_task_id = os.environ.get("CORTEX_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
                    from cortex_cli import kanban_db as _kb
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Athena" box, no tool-gen
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("CORTEX_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from cortex_cli.kanban_db import (
                ...
                # Single-query mode (`athena chat -q "…"`): skip the welcome
